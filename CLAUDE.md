...
- **StatCardGrid is always one row (living rule, updated 2026-07-22):** `StatCardGrid` (`components/ui/stat-card.tsx`) is always a single horizontal row of equal-width cards (`gridTemplateColumns: repeat(N, minmax(10rem, 1fr))` from child count; `[&>*]:min-w-0 [&>*]:h-full`). Never multi-row responsive breakpoints (`grid-cols-1` / `sm:grid-cols-2` / `xl:grid-cols-*`). On narrow viewports the row scrolls horizontally (`overflow-x-auto scrollbar-hide touch-pan-x`, snap optional) so cards keep a readable min width — never compress/truncate labels by shrinking below ~10rem; do not wrap to a second row. Desktop (`md+`) keeps equal `1fr` fill with no overflow.
...
- **Large multi-section popovers/dropdowns → Drawer on mobile (living rule, 2026-07-22):** tall filter/display panels (Display options, view prefs, multi-section Add filter, notifications View/Category/Priority filter) use `ResponsivePopover` (`components/ui/responsive-popover.tsx`) — **Drawer** below `md`, **Popover** (or existing desktop Selects) on desktop. Applies to tall scrollable multi-section `DropdownMenu`s of this size as well as Popovers. Content + trigger stay the same; do not convert tiny menus, 1–3 item popovers, or date pickers. Specialized drill-down filters (e.g. ticket `filter-command-menu`) may keep a custom mobile Drawer.
