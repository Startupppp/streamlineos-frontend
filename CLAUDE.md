...
- **Employee self-service is platform core, not a paid-module entitlement (living rule, 2026-08-05):** every active member keeps access to their own time off, attendance, expenses, payslips/pay, employment documents, announcements, and people directory even when their only enabled product is Build/CRM/etc. Canonical frontend routes live under `/me/*`; backend handlers use `self:*`, derive the subject from `@CurrentUser()`, never accept a self `userId`, and never carry `@RequireModule`. The HR/payroll/finance administration endpoints remain separately permission- and module-gated. Never expose an admin endpoint merely to make self-service work.
