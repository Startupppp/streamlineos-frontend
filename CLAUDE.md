...
- **Administration follows the HRMS treatment, not ink (living rule, 2026-08-11).** `/settings/*` uses the same rich-surface kit as HR — hero band, tinted quick-action tiles, `rounded-2xl` translucent panels, section headers, status pills, icon wells. The kit is therefore **shared, not HR-local**: it lives in `components/shared/rich-surface.tsx` and `features/hr/shared/hr-ui.tsx` re-exports it under the `Hr*` names so HR's importers are untouched. Administration must import the shared module — never reach into `features/hr/**` (§24 forbids feature→feature). Consequence: for HR and Administration only, `rounded-2xl` and `backdrop-blur` are sanctioned and override `UI-UX-SYSTEM.md` DS-001/AP-4. Every other module stays ink-first at `rounded-xl`.
...
- **A text index is unusable under RLS, and `LEAKPROOF` cannot fix it here (living rule, 2026-08-12).** Every search operator is `proleakproof = false` (`ts_match_vq`, `similarity_op`, `textlike`, `texticlike`), and a non-leakproof user qual may not be evaluated before the RLS security qual — so it never becomes an index condition and the GIN index is skipped entirely. Measured on `tickets` (203k rows): owner `Bitmap Index Scan` 16 buffers / 0.34ms vs `streamline_app` **`Seq Scan` 12,036 buffers / 134.8ms**. **`ALTER FUNCTION … LEAKPROOF` is impossible on Neon** — it needs a true superuser and `neondb_owner`/`neon_superuser` are both `rolsuper = false`, so it fails `42501` even as owner. Never propose it. The one working escape is a **`SECURITY DEFINER` function owned by the BYPASSRLS owner** (Postgres never inlines these, so the body runs outside the security barrier). Canonical: `app.search_ticket_ids` (migrations `0424`/`0425`). It is only acceptable with all five: org from `app.current_org_id()` and **never a parameter** (so it fails closed `42501` with no GUC) · returns **ids only, never row data** · the caller's real query still runs under RLS and still applies its DataScope clause · `REVOKE ALL … FROM PUBLIC` then `GRANT EXECUTE` to the app role only · a `LIMIT` argument, with the caller requesting `cap + 1` and falling back to plain `ILIKE` when the cap is hit — an unbounded set-returning function is materialised in full and is *slower* than the seq scan for a broad term (measured 434ms vs 1ms), because a seq scan under `LIMIT` stops early.
- **Benchmark as `streamline_app` with the tenant GUC, never as the owner** — the owner has `BYPASSRLS`, so its plans hide every problem above.
- **All list endpoints paginated**, hard cap **100/page** (public included). **Soft-delete** with `deleted_at`.
- **Soft delete is the default; hard delete is the exception (living rule, 2026-08-11).** Every **business entity** — contacts, deals, leads, quotes, products, employees, tickets, invoices, projects, documents, roles, org units — deletes by setting `deleted_at`, and **every read filters it** (`isNull(x.deletedAt)`); a `deleted_at` column that reads ignore is worse than none, because it silently resurrects deleted rows. Indexes on those tables are **partial**, excluding deleted rows. Physical `DELETE` is permitted only for: join/link table rows, unsent drafts, unaccepted or terminal invitations with no retention duty, session/token revocation, and explicit DPDP/GDPR erasure. Organization hierarchy is stricter still — archive/restore only (§16). Watch for a cascade crossing the boundary: if a parent soft-deletes but a child FK is `onDelete: "cascade"`, the cascade never fires and the children are orphaned-but-visible.
...
- **AI is barred from the authorization decision path (living rule, 2026-08-10).** Runtime decisions are made only by the deterministic resolver; AI may never write to a permission table; AI explanations must render a deterministically computed resolution trace, not reason about policy; and permission data (role names, group membership, org hierarchy, grant history) must not egress to a model provider — tenant free-text (custom role names, grant justifications) is data, never instruction.
...
- **Org admin implies module admin; per-user module denies do not override an org-level grant (living rule, 2026-08-10).** Test org-admin structurally via `organizationMembers.role` / `isOwner`, never by checking whether a principal holds `settings:manage` or `settings:rbac:manage` — the permission-derived path is AC-04 and creates an accidental parallel superuser bypass.
...
- `DataScope`: `all` · `team` (same dept) · `own` · `none`. `team` scope is approved to go live once materialised — eliminate the correlated `org_unit_members` subquery in `apply-scope.ts` before exposure; until then do not surface it to callers (living rule, 2026-08-10).
