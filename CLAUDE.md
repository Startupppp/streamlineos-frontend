...
- **Credit and machine control:** one concrete outcome per assignment. Run only tests for the changed code and directly affected callers/contracts, with explicit file paths or a bounded test filter. One test batch at a time, one worker/in-band where supported; coordinate builds/typechecks so heavy processes never overlap. Do not run the whole repository test suite or watch mode without the user's explicit request. Stop expanding after two unsuccessful repair hypotheses: report the missing discriminator and continue independent work. Broader coverage is collected from sequential targeted batches, not an all-tests launch.
...
- **Comments explain invariants and tradeoffs.** Remove unnecessary, stale, misleading, duplicate, commented-out implementation and code-restating comments in the affected flow. Preserve useful security, concurrency, migration, compatibility and non-obvious design explanations, licences, generated markers and tool directives. Remove a TODO only after its work is verified complete or retained in the completion plan; never bulk-strip comments or delete protective directives to silence a gate.
