...
- Full-page backgrounds: the shell owns the background (`--background`). Pages NEVER repaint it — no `min-h-screen`, no page-level gradients inside `(authenticated)`.
- **Light-mode surface separation (living rule, 2026-07-15):** the light canvas is `#f4f6fa` (between slate-50 and slate-100) with `--border #d7dee8` — cards stay white so every card/panel visibly lifts off the page in ALL accent themes (the themes.css light tint block mixes over these same bases). Never drift `--background` back toward white, never let card ≈ background, and keep translucent panel fills ≥75% card opacity so they don't wash out against the canvas.
