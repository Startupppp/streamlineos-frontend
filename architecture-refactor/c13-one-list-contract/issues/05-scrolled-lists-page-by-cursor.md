...
### Design decision — the Build ticket list stays page-numbered, and this is why

S4 was asked to resolve this rather than route around it, so the call is recorded with its reasoning.

**The surface is real.** `useProjectBoardTickets` (`frontend/hooks/api/build/ticket-queries.ts:37`) is a `useInfiniteQuery` that walks `GET /build/:projectId/tickets` by **page number**, auto-loading up to 500 rows into one flattened array. That is precisely the scrolled-and-concurrently-written shape this ticket exists for: a ticket created while the board loads shifts every row down, and the flattened array gets a duplicate and a gap. It is also the only paging consumer of that endpoint — `useTickets` never sends a page.

**Converting it was weighed and declined.** It needs a cursor mode on the endpoint, a keyset predicate per sortable column (`rank` is a string, so `keyset.ts` — which coerces its sort value to a `Date` — cannot serve it), a `total` that is only correct on the first page, and a matching frontend change. Against that: no database is reachable to measure the new plan against (see the blocker in c13-06), four sessions share this checkout, and it is the product's busiest screen. **And it closes no box** — this ticket's first criterion names `notifications` too, which is in another lane, so the criterion stays open whether or not the ticket list is converted.

Declining an unverifiable change to the busiest screen, for no box, is the trade S4 took. It is recorded as open work in one lane's territory rather than left implied.
...
- [ ] Set **Status** to `done` and update this ticket's row in [`../README.md`](../README.md) — held open by the first acceptance criterion. `notifications/broadcasts.service.ts` is outside every lane that has run so far; the Build ticket list is inside S4's territory and was declined on the record above.
