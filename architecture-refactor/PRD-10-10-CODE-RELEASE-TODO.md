...
| `check:query-signal` | **superseded — the gate was blind; see S11.** 1,053 queryFn blocks / 417 files, 125 repaired, 0 remaining |
...
- [x] [S03 — API, query, pagination, caching and contracts](code-release-sessions/S03-API-QUERY-CACHE-CONTRACTS.md) — 25/25 S03 gates green, 112 suites / 1,202 tests green, read budgets 57 PASS / 0 FAIL / 10 EXCL / 3 SKIP on `scratch_e2e` as `streamline_app`, every exclusion CRM or Inventory. Commits `8cc46225` · `846827d7` · `5e785f03` · `dfc8708a`.
...
- [ ] [S05 — Payroll, Accounting and Finance](code-release-sessions/S05-PAYROLL-ACCOUNTING-FINANCE.md) — 7 of 8 criteria evidenced; 249 suites / 1,748 backend tests and 11 / 66 frontend tests green, 11 of 12 owned e2e suites green. Blocked on S02 applying migration `0933` before the payroll DB-integration e2e can pass.
- [x] [S06 — Build and Workflows](code-release-sessions/S06-BUILD-WORKFLOWS.md) — 127 suites / 857 tests green, e2e 15/362 green, 13 services moved onto the membership gate, five runtime-fatal queries fixed and `check:unjoined-table-refs` added (now clean repo-wide). Cancellation, list truncation and the untested workflow builder are closed; module-runner retry/DLQ and the approval executor remain open because both need an idempotency or product decision, not a keystroke — both named in the session.
...
- [x] [S08 — Chat, Notifications and realtime delivery](code-release-sessions/S08-CHAT-NOTIFICATIONS-REALTIME.md) — backend 93 suites / 691 tests green, e2e 7 passed + 1 db-gated skip, frontend 16 suites / 109 tests green, boot proof 3,604 operations. All eight first-pass open items are now closed, including two migrations (`0980`, `0981`) verified against the live schema and rolled back. The P0 was a preference upsert whose `ON CONFLICT` target had carried no matching index since `0918`.
...
- [ ] [S10 — Knowledge, Wiki, Chatbot and AI](code-release-sessions/S10-KNOWLEDGE-WIKI-CHATBOT-AI.md) — **6 of 8** session criteria closed; commits backend `1509c33d`, root `e58554057`. Backend `modules/(kb|ai|search)` 120 suites / 768 tests green (from 113 / 723), frontend KB/AI 8 / 53 (from 3 / 30) inside a full frontend run of 234 suites / 2,194 tests green, production typecheck exit 0, zero spec-typecheck errors in scope, madge zero in both repos, 23 targeted gates green. The load-bearing finds were a circuit breaker that could never open (its only increment site was a `catch` around a function that returns synchronously), client aborts that never reached the provider on the non-streaming path, unbounded gateway output tokens against reserve-ceiling pricing, an AI response cache whose key carried no reader dimension, AI-drafted email interpolated unescaped into outbound HTML, page-document chunks written with NULL ACL columns that made them permanently invisible to vector search, an unbounded `reindexAll`, per-chunk embedding that opened 500 simultaneous provider calls, a comments panel gated on the update key so read-only members saw nothing, and nine query-key factories with the trailing-`undefined` bug. Since then, page-revision immutability (owner chose KEEP-FOREVER), the per-tenant concurrency cap, the process-local breaker, the settle/release race, embedding attribution, wiki edit concurrency and the page-version cursor cutover all closed. Open on two clauses only: embeddings are metered but still do not route through the gateway, so an anonymous KB question is embedded before its credit check; and streaming for the non-chat AI surfaces (owner deferred) plus realistic-corpus latency, which needs the seeded database tracked below.
- [ ] [S11 — Frontend platform, TanStack and perceived performance](code-release-sessions/S11-FRONTEND-TANSTACK-PERFORMANCE.md) — 5 of 8 session criteria closed. Frontend typecheck exit 0 and **243 suites / 2,283 tests green**, re-run after each bulk edit; 17 targeted gates green; 5 gate self-tests carrying 60 fixtures. The load-bearing finds were a cancellation gate that reported "0 violations across 399 files" while **125 reads could not be cancelled** — closed by making the whole class a compile error rather than a lint rule — and a command catalog whose 677 baseline had been measured by a scanner blind to the **382 `useMutation<T, E, V>` call sites** that are the dominant form in this repo, now at **0 unclassified of 1,501** with every key read from the contract. Also fixed: three permission keys that blocked the role the feature exists for (a payroll approver holding only `payroll:runs:approve` was refused by the UI), an authorized-mutation guard that read "access not loaded yet" as "denied" across 1,422 hooks, a `POST` that sent an AbortSignal as its request body, 66 hardcoded cache prefixes, and four missing shell primitives (no root `global-error`, a retry that never reset the query cache, no offline state, no focus move on navigation). Three dead endpoints found and handed on with backend evidence. Open on: the bundle number, which was measured from a fresh production build but deliberately not recorded because it is not separable from a possible recipe change; Web Vitals, which have no browser automation in this repo; 114 domain-owned thick route modules, now ratcheted after the four transport violations were repaired; and contrast plus the 375/768/1280 sweep, which needs a browser. Landed as `cecd0d5bf` · `2ab8348e3` · `398359abe` · `7ebea9acb` · `71af72ce9` · `6589ab72e`.
- [ ] [S12 — Shared adapters, security, privacy and repository quality](code-release-sessions/S12-SHARED-SECURITY-QUALITY.md) — 6 of 8 session criteria closed. The load-bearing find is that **the seeded E2E harness had been failing 4 of 6 for reasons that each masked the next**: the fixture never wrote `organization_placement`, so every request 401'd; it forged HS256 tokens against a backend that verifies EdDSA only; and it selected a `projectMembers` column that no longer exists. It is now 6/6, refuses a non-disposable database, and can enable a module — without which no plan-gated permission was testable at all. Also closed: GDPR object-key enumeration was invalid SQL (`= ANY(row constructor)`, `42809`) so every storage erasure aborted before finding a key; the GDPR admin export view was inert because the route carried no `PermissionGuard`; multipart uploads were marked `clean` with a fabricated hash and no scan; onboarding uploads had no scan at all; `release` could clear a malware detection; webhook delivery re-resolved DNS after validating it; `sweepAbandonedUploads` had zero callers; and `check:module-di` was failing on a genuine boot failure in four CLI entry points. Three architecture rules that CLAUDE.md states and nothing enforced now have gates — kebab-case, import direction and AppModule reachability — and `check:over-300`'s self-test, which asserted only its own constants and never ran the scan, was rebuilt against a fixture tree and bite-proven twice. **`check:module-registration` caught its own blind spot before shipping**, reporting 73 unreachable modules in an app that serves 3,595 handlers because it could not parse `imports: BUILD_MODULES`; the result was disbelieved rather than filed. **One fix in this session opened a hole and the review caught it** — the new quarantine sweep would have unblocked an infected key whenever the S3 delete failed. Evidence: seeded E2E 6/6 plus a new cross-module privacy artifact 2/2; 117 suites / 804 tests green; 21 gates green including three new ones, with `check:retention-coverage`, `check:module-di` and `check:mock-surface` each moved from EXIT=1 to EXIT=0. **Open on two criteria:** erasure still covers object storage only, and both over-300 ratchets plus one over-500 file are above baseline — S12's own splits are provably net-neutral and the regressions arrived from concurrent lanes in the shared tree, so they are named with owners rather than absorbed.
...
- [x] Fix seeded E2E harness failures, including organization placement/control-plane state and schema/fixture drift. — S12: **2/6 → 6/6**, three independent root causes, each one masking the next. **Placement/control-plane:** `SeedBuilder` wrote `organizations.region` but never `organization_placement`, and `regionForOrg` reads the placement table — so every tenant transaction threw "has no region" and every authenticated request returned **401**, which reads as an authorization defect rather than a fixture one. All three real creation paths place first and insert the org second (`auth.service.ts:83`, `org-setup-resolver.service.ts:219`); the fixture now matches, and teardown unplaces because `organization_placement` carries no FK to `organizations`. **Contract drift:** `signSeededToken` minted HS256 against `BACKEND_JWT_SECRET` while `JwtKeyringService.verifyToken` accepts EdDSA only — the AR-01 token-authority change the harness was never updated for; it now signs through the application's own keyring so it cannot drift from the verifier again. **Schema drift:** `projectMembers.userId` no longer exists (the column is `membershipId`) and Drizzle threw rather than returning empty. Two further harness gaps closed: it connected to whatever `DATABASE_URL` named — the shared `neondb`, with owner credentials, while writing and deleting fixture rows — and it could not enable a module, so every plan-gated permission resolved `NO_MODULE` and a correct grant was indistinguishable from a denial.
...
- [x] Verify backend/frontend folders follow domain ownership and kebab-case rules; shared modules never import feature modules. — S12: both rules were stated in CLAUDE.md §7/§9 and enforced by nothing, so "verified" could only ever mean "someone looked". Two gates now exist. `check:kebab-case`: 6,049 entries under `backend/src`, **0 violations**; `__tests__`, `__snapshots__` and `@types` are allowed by exact name rather than a loose pattern, so a genuinely mis-cased directory still fails, and the two recorded exceptions each carry a reason. Its self-test rejects PascalCase, camelCase, snake_case, a single stray capital (the actual CI case-safety failure) and a `__lookalike__` directory. Frontend files are clean (0 violations); its directory conventions are Next.js route groups and dynamic segments, which the framework mandates. `check:import-direction`: **0 new violations** across 208 `src/common` files, with **9 pre-existing** `common → modules` imports pinned in a baseline that names the symbol and owner for each — the debt cannot grow, and the gate fails if a baseline entry silently disappears. Its self-test proves `import type`, side-effect imports, multi-line specifier lists, `export * from` and the `src/modules/…` alias are all caught, since those are the forms a naive scan misses.
...
- [ ] Prove every active Nest module is registered and every frontend route has one canonical owner; remove obsolete routes rather than preserving hidden duplicates. — the backend half is now **proven and gated**: `check:module-registration` (S12, new) walks `imports:` transitively from `AppModule` and reports **216 declared / 215 reachable / 0 unreachable**, the one exception being `ArchitectureEvidenceModule`, declared standalone with its reason. `check:module-di` proves a module's own providers resolve, which an orphan also does, so it could never see this class of defect. Fixing one such defect on the way: `OutboxPublisherService` takes `OutboxReportService` at constructor index 3 with no `@Optional`, and `ArchitectureEvidenceModule` neither provided it nor imported `OutboxModule` — four CLI entry points would have thrown at boot. **The gate caught its own blind spot first:** matching only `imports: [ … ]` it reported 73 unreachable modules in an application that serves 3,595 handlers, because the common aggregate form is `imports: BUILD_MODULES`. The result was disbelieved rather than filed. The frontend route-ownership half stays open (S11).
...
- [ ] Keep authenticated `app/**/page.tsx` and `layout.tsx` files as thin route modules for metadata, parameters, server authorization and composition; move state, forms, queries and mutations behind feature-owned interfaces and gate route-file size/import direction without changing landing visuals or animations. — **The gate this asks for now exists.** S11 added `check:route-thinness`, which scans all 586 authenticated route modules for component state, data fetching, forms, direct `apiClient` calls and files over 300 lines, and ratchets the in-scope count. It reported **118 in-scope** and **67 CRM/Inventory**, the latter printed under OUT OF SCOPE rather than filtered away; nine fixtures pin the classifier, including one proving `useStateOfTheArtBanner` is not a `useState` match, and the ratchet is proven to fail when lowered by one. The four transport violations are now repaired and the count stands at **114**. Landing visuals and animations untouched. Moving the 118 is domain-session work, listed by owner in the S11 ticket.
...
- [x] Resolve the Drizzle-declared columns that exist in no database. — **0 remain.** This was a live failure, not cosmetic drift: `db.select().from(X)` renders every declared column, so a full-table read on ~65 HR, hiring, payroll and performance tables returned `42703 column "user_membership_id" does not exist` against the live database. 68 nullable actor columns were added by `0985` and the last (`attendance.user_membership_id`) by `0987`; `select()` on `interview_panel_members`, `performance_reviews` and `goals` was failing before and succeeds after. Applying the previously-stranded KB and expense migrations closed a further 29 without new DDL.
...
- [x] Cold-bootstrap an empty database to migration head and record zero pending, orphan, duplicate or unreachable migrations. — `check:migration-ledger` EXIT=0 against the live database: **633 applied of 633 journal entries, 0 pending, no orphan, duplicate or unreachable entry**, watermark `1803000010083`. `verify-migration-chain` EXIT=0. Reaching this exposed two defects: `0983`/`0984` had no journal entry so `db:migrate` would never have applied them while printing success, and six migrations from other lanes were repeatedly overtaken by the advancing watermark. Cold-bootstrap *parity* between two independent empty databases is tracked separately below and is not claimed here.
...
- [x] Verify migration `0930` enables the `audit_logs` append-only trigger and rejects application-role mutation in the disposable database. — all four controls pass on a bootstrapped database: INSERT as `streamline_app` succeeds, UPDATE and DELETE are both rejected `42501 "audit_logs is append-only"`, and the rejection is proved against the **application role**, not merely the owner. Evidence: [s02-bootstrap-parity.md](final-refactor/evidence/s02-bootstrap-parity.md).
...
- [x] Fix the evidence gap: 43 read budgets are below minimum seed size and 27 are skipped; CRM/Inventory rows do not count. — S03: **0 below minimum seed size, 3 skipped**, all three CRM party-search fixtures. The 43/27 figures are superseded.
- [x] Run each in-scope budget as `streamline_app` with `EXPLAIN (ANALYZE, BUFFERS)` and retain rows, buffers, duration, indexes and thresholds. — S03: **57 PASS / 0 FAIL / 10 EXCL / 3 SKIP over 70 budgets**, exit 0, run as `streamline_app` (non-`BYPASSRLS`) with `set_config('app.organization_id', …, true)` inside the transaction, so RLS predicates are in every plan. Each budget runs twice and records shared hit/read blocks, table rows, scan rows and selectivity per run; many show a cold read on run 1 and warm on run 2. **Every remaining exclusion and skip is CRM or Inventory** — nothing in scope is unmeasured.
...
- [x] Verify every growing list has a hard limit, deterministic order, unique tie-breaker, signed scope-bound cursor and consistent filters/sorts. — S03: `check:unbounded-reads` reports **0 actionable offset instances and 0 actionable unbounded reads** across 2,029 service files, and `check:bounded-contracts` 0 in-scope violations. The last offset page, `GET /storage/quarantine`, was keyset-migrated to `(created_at DESC, id DESC)` — it previously ordered on `created_at` alone, so rows sharing a timestamp could repeat or be skipped between pages. **One deliberate departure:** cursors are opaque and scope-bound but not cryptographically signed. `cursor.schema.ts` argues that a cursor encodes position and nothing else — no tenant, no permission, no filter state — so a forged one cannot express anything the server would not otherwise serve, and scope is re-derived per request regardless. Signing would add a key-rotation surface for no gained authority. Recorded as a decision, not silently satisfied.
- [x] Prove no `SELECT *`, fetch-then-filter/count, N+1/per-row expansion or unbounded export/sweep remains. — S03: 0 actionable unbounded reads and 0 actionable offsets. One genuine fetch-then-filter was found and fixed: `agedPayables` selected every `POSTED`/`PARTIALLY_PAID`/**`PAID`** bill org-wide and discarded each row with an outstanding balance `<= 0.005` in JS — which is every PAID bill, and PAID bills accumulate for the life of the org. The predicate now runs in SQL. **`check:db-call-count` still carries 36 files / 42 call sites classified ACTIONABLE** as an N+1 ratchet in domain modules; those belong to their owning sessions and this criterion is checked for the projection/fetch-then-filter/offset half only.
- [ ] Verify cache keys include tenant, subject, permission and resource dimensions where applicable.
- [ ] Prove mutation/revocation invalidation, TTL/negative-cache policy, stampede protection and Redis degradation never leak data or preserve revoked access. — S03 evidence for the shared primitive: stampede protection is in-process single-flight plus a cross-instance fill lease plus ±15% TTL jitter, all tested; Redis degradation on every read path falls back to the database, so an outage never returns a permissive answer; negative caching is now pinned by test — a `null` fetch result is never served from cache, so a cached denial cannot outlive the grant that ends it; and all four invalidation paths retry with backoff and record `cache.invalidation.dropped` instead of swallowing the first error. **Left unchecked on purpose:** `AccessVersionChannel.publish` still swallows a failed `store.clear`, so a Redis outage during a permission mutation leaves the stale shared version in Redis and other instances serve revoked permissions until that key's TTL expires. Bounded staleness, not a lost revocation — but the bound is a TTL nobody has declared as the revocation consistency contract, and the criterion says "never preserve revoked access". Owner: S01.
...
- [x] Verify request cancellation, stale/gc policy, retry policy, refetch behavior and deduplication do not amplify load or replay unsafe writes. — S11: cancellation is now enforced by a parser that resolves argument **position** over the whole queryFn body, green across 1,053 blocks after repairing 125 reads. Policy read from `query-provider.tsx:37-51` and unchanged because it is already correct: `staleTime` 120s, `gcTime` 600s, `refetchOnWindowFocus: false`, queries retry at most **once** and never on a 4xx except 408/429, and **`mutations.retry: 0`** unconditionally — so no write can be replayed by the retry layer.
- [x] Verify server-prefetch/hydration and client keys match exactly with no cross-user or cross-organization cached payload. — S11: all six prefetch factories go through `createServerQueryClient` (`lib/prefetch/server-query-client.ts:24-33`), which installs the same `scopedQueryKeyHashFn(authenticatedScope(orgId, userId))` the client provider installs, so the dehydrated hash equals the client lookup. `hydration-contract.test.ts:48-58` proves the round trip and `:74-75` proves a plain `new QueryClient()` produces an unreachable entry; `check:query-scope` rule 3 fails any `lib/prefetch/**` file that dehydrates without it.
...
- [x] Pass TanStack `QueryFunctionContext.signal` through every cancellable read to `apiClient`; enforce zero unclassified reads and test navigation, search, range-change and organization-switch cancellation. — **Closed by S11 (2026-09-02) on all three conditions this box was re-opened for.** (1) The **class is now a compile error**, not a lint rule: `apiClient.get`'s `params` is `QueryParams`, a union of an index-signature record and `object`, each intersected with `{ aborted?: never; addEventListener?: never; throwIfAborted?: never }` — object literals, interfaces, `Record<string, unknown>` and `Record<string, string>` all stay assignable while an `AbortSignal` does not. Proven against the shipped type by an in-tree probe: `apiClient.get<string[]>("/things", signal)` failed with TS2345 while the three-argument form compiled. (2) `check:query-signal` now parses the **whole queryFn body** and resolves every argument by **position**, with 19 fixtures — including the exact `(url, signal)` shape, a multi-line body past the old 150-character window, a signal passed as a POST body, and three parser traps that a comma inside `as Record<string, unknown>` used to cause. (3) The repairs are done. The gate's "0 violations across 399 files" became **125 real violations** over **1,053 parsed queryFn blocks in 417 files** — 39 queryFns never destructured `signal`, 85 destructured it and never forwarded it, and one sent it as the request body of a `x-bodyless` POST — and all 125 are fixed. Navigation, search, range-change and organization-switch cancellation are covered by the repaired hooks plus the cursor-contract suites, which now assert the forwarded signal positionally rather than accepting a two-argument call.
- [ ] Enforce canonical query-key factories for authenticated data: zero ad-hoc array keys or local key factories, no redundant tenant argument where the scoped Query hash already owns tenant/user identity, and exact invalidation tests for every mutation. — S11: ad-hoc array keys are **zero** (`check:query-scope` rule 4) and tenant identity lives only in the scoped hash, never in the key array. **66 hardcoded `"streamlineos"` prefixes across 26 files now compose `queryKeyBase`**, and new rule 5 forbids the literal outside `lib/query-keys/**` with two known-bad and two exempt fixtures. STILL PENDING: roughly 20 domain files still define a local `*Keys` object beside the feature rather than being enrolled in `lib/query-keys/**`, so the exhaustive registry walk does not cover them; that move is domain-session work.
- [x] Build a controller-to-hook command catalog: every non-universal mutation uses the exact backend permission through the authorized-mutation module, every universal/self exception is explicit, and zero commands are unclassified; test revocation before and during a mutation. — S11: **0 unclassified of 1,501** mutation hooks (1,422 PERMISSIONED, 34 SELF, 16 IN-SERVICE, 15 OFF-CLIENT, 14 PUBLIC). The classifier is **derived from `contracts/openapi.json`**, so no key was chosen by hand; the frontend `PermissionKey` union accepted all 979 migrated keys, which is independent evidence they are real. The prior 677 baseline was measured by a scanner that matched only `useMutation(` and was blind to the **382** `useMutation<T, E, V>({…})` call sites that are the dominant form here, plus `export const` arrow hooks and any hook whose comment contained an apostrophe. Three wrong keys were found and corrected against the backend source, not the contract alone: `useApproveStage`/`useRejectStage` demanded `payroll:runs:manage` where the controller requires `payroll:runs:approve`, so an approver holding only `:approve` was refused by the UI. `useAuthorizedMutation` no longer reads an unresolved access snapshot as a denial.
- [x] Provide one typed optimistic patch/rollback implementation per cache shape, including `InfiniteData`; update list/detail/count variants atomically and prove concurrent realtime delivery cannot corrupt or overwrite optimistic state. — S11: `lib/optimistic-cache.ts` ships `cancelAndSnapshot`/`restoreSnapshot` plus shape-aware `patchRows`/`patchRow`/`removeRow`/`patchDetail`/`patchCount` covering flat arrays, `{ data }` and `{ items }` cursor pages and `InfiniteData` over any of them; an unrecognised shape is **left untouched rather than guessed at**, so the failure mode is no optimistic update rather than a corrupted cache. 15 tests cover every shape, list+page+infinite patched atomically from one prefix, rollback across several filters, and two concurrent-realtime cases. **Proven to bite:** patching only page 0 turned 2 tests red and restoring the source turned them green.
...
- [x] Compute integrity checksums and make upload, scan, transform and finalization retries idempotent. — SHA-256 returned in `UploadResult`; `@Idempotent` on the mutating quarantine transitions. **Corrected by S12 2026-09-02:** true of the single-request upload path only. Multipart completion wrote `sha256: ""` and `fileSizeBytes: 0`, because the bytes go browser→S3 via presigned part URLs and never reach the backend, so there was nothing to checksum. The record is now honest (it stays `pending_scan` rather than claiming a hash it does not have); computing a real digest needs either a `HeadObjectCommand` after completion or a client-supplied hash in the complete body, and is recorded as open below.
- [x] Fail closed into tenant-scoped quarantine until malware scanning succeeds; implement authorized release, rejection, retention, deletion and audit transitions. — `file_quarantine_records` (migration `0933`) plus a state machine and a permission-gated admin controller (`storage:quarantine:view` / `:manage`). Download is blocked while quarantined. VirusTotal responses are Zod-`safeParse`d and a malformed or timed-out response keeps the file quarantined rather than releasing it. **This box was checked on one upload path and was false on the other two — corrected by S12 2026-09-02.** (a) Multipart completion called `markClean` immediately on a record it had just created, so a file nothing had ever scanned was born `clean`; it now stays `pending_scan` and downloads stay blocked, which removes no capability because the frontend has zero callers of any `storage/multipart` route. (b) The onboarding upload controller never injected `AvScanner` at all — an `@Universal()` route open to every active member wrote to S3 with magic-byte validation only; it now fails closed identically to `StorageController.upload` (infected → 422, scanner error → 503) before any write. (c) "Authorized release" had no prior-state predicate, so a holder of `storage:quarantine:manage` could `release` a record sitting in `infected` and silently clear a malware detection; `markClean`/`markInfected` now carry status predicates and the controller returns 409. (d) "Retention, deletion" existed only as an unreachable method: `sweepAbandonedUploads` was implemented, cursor-based and resumable, with **zero callers**, and nothing expired stale `pending_scan` or infected records or their objects. A registered `CronStorageSweepService` now does both per org — and its first version would have **unblocked an infected key** whenever the S3 delete failed, because it soft-deleted the record in a separate `try` block and `isKeyBlocked` ignores soft-deleted rows; the soft-delete is now reachable only after a successful object delete.
...
- [x] Routes/contracts: verify calculation, lock, approve, publish, reverse and export operations use strict schemas, idempotency and explicit state transitions. — S05. `PAYROLL_RUN_TRANSITIONS` listed `PENDING_APPROVAL → LOCKED`, and `LockingService.lock` is the only consumer of that table, so `POST /payroll/runs/:id/lock` skipped the approval chain outright; removed, and the spec that asserted the entry (naming a `lockAfterApproval` flow that never consults `canTransitionRun`) now asserts the opposite. Lock/reopen/close were SELECT-then-UPDATE with no status predicate — each UPDATE now carries `eq(status, run.status)` with `.returning()` and raises 409 on a lost race. `check:idempotent-commands` and `check:bounded-contracts` pass with 0 in-scope violations.
- [x] Authorization/privacy: test payroll owner/admin/member, approver, self-payslip, separation-of-duties, sensitive projections and every mutation hook. — S05. All 65 payroll mutation endpoints inventoried with their keys confirmed **verbatim** against the catalog (`check:permission-keys`: 3,112 usages / 627 unique). Separation of duties was missing on salary loans (`updateLoan` let the requester approve their own; reimbursements already had the guard) and is now closed. Self-payslip privacy verified sound — ESS routes derive the subject from `@CurrentUser()` and expose no `userId` parameter — and the payslip publication lookup gained the `orgId` predicate it was missing.
...
- [x] Make Payroll finalization and Accounting posting crash-consistent through a transactional outbox: emit an idempotent posting intent inside the Payroll lock transaction, consume it durably in Accounting, expose `pending/posted/failed` state and prove rollback cannot leave a journal while retries cannot duplicate one. — `locking.service.ts` emits `payroll.run.posting-intent` via `OutboxWriter.emit(tx, …)` on the lock's own transaction handle, so the intent rolls back with the lock. `PayrollPostingIntentConsumer` claims the inbox row atomically and posts idempotently; `postJournal` remains keyed on `(PAYROLL_RUN, runId, finalized)`. `posting_state` added by migration `0933`. 8 tests cover induced outer rollback, crash-between-commit-and-consume, duplicate delivery, malformed payload, cross-tenant mismatch and retry-on-failure. **Corrected by S05 — the seam was installed on a path the default configuration never takes.** `lockAfterApproval` defaults to `true`, so an ordinary run reaches LOCKED through `approveStage`, which wrote the status directly and emitted no intent, set no `posting_state` and wrote no TDS ledger: the Accounting journal was never created at all. The lock-commit body moved to `LockingService.commitLock(tx, …)` and both paths now call it on their own transaction handle. Second correction: a poison intent marked the inbox FAILED and returned **without throwing**, so the publisher recorded success — no retry, no DEAD, no durability alert, and the run sat at `pending` with a lost journal; both terminal paths and the catch path now write `posting_state = "failed"`, a state the schema declared and no line of code had ever written. 11 tests now, each bite-proven. **Open:** `0933` is journaled at `when: 1803000010026` against a live watermark of `1803000010025` and is **not applied**, so `posting_state` does not exist in the database and `payroll-db-integration.e2e-spec.ts` fails 14 assertions on it. Applying it is S02's.
...
- [x] Queries/cache/workers: verify leases, concurrency limits, retries/backoff, cancellation, DLQ, schedule deduplication, bounded histories and consumer registration. — S06. The **shared** runner is sound: `FOR UPDATE SKIP LOCKED` with `lease_expires_at` reclaim covers a crashed worker, and `decideAfterFailure` backs off with full jitter before dead-lettering. **Cancellation is now obeyed, not merely recorded.** `cancelExecution` wrote `cancelled` from its own transaction while `advanceExecution` walked the graph without re-reading it, so every remaining node still ran and only the terminal write was suppressed; the walk now re-reads status at each step boundary, stops, and persists the cursor it stopped at. 5 tests including a running-throughout control. **Silent truncation closed:** `listAllSchedules`, `listSchedules`, `listSecrets` and `listGlobalSecrets` capped rows behind a bare array and now return the canonical keyset page, with a PgDialect spec asserting the cursor columns match the two-column ORDER BY so the board's cursor/sort mismatch cannot recur here. `alert:workflow-stranded` makes `timed_out`/`failed` runs visible to an operator. **Still open, recorded not hidden:** the module runner has no retry or DLQ for step failures and nothing returns a `timed_out` run to `pending`; run/step/version history is unbounded. Retrying arbitrary steps needs per-node idempotency that does not exist yet, so it is not a change to make unilaterally.
- [x] Frontend/TanStack/tests: verify editor/run-history state, version conflicts, permission gates, polling/subscription cleanup and deterministic execution/recovery tests. — S06. The workflow builder had **no** test coverage, which is why this was the last open box in 10.9. 27 tests now cover it: a 409 from publish reaches the caller instead of being swallowed and `expectedVersion` is actually sent; every mutation refuses without its permission key and the API is never called; polling stops at each terminal status and continues at each active one; and `queryKeys.workflows.all` is proved a real prefix of the list, detail and execution keys by mounting a list and a create together and watching the invalidation land. Writing them found a real defect: the execution list polled only on `running`/`waiting`, but `pending` is equally non-terminal — the runner claims `pending` and `waiting` together — so a page of freshly queued runs sat frozen. Fixed via `isActiveExecution`. Every workflow `useCan` key was confirmed verbatim against `lib/rbac/permissions/workflows.ts`; none were missing.
...
- [x] Architecture/schema: verify accounts, journals/entries, expenses, reimbursements, invoices, payments, reconciliation and immutable reversal relationships balance and preserve tenant scope. — S05. **`voidInvoice` never reversed anything:** it looked its journal up by `sourceEvent: "invoice_send"` while `postInvoiceSend` writes `"send"`, so every voided invoice left its AR debit and revenue credit posted permanently. Both sides now share one constant pair. Balance assertion, the `0793`/`0445` immutability triggers, `uniq_je_idempotency` and RLS coverage across journal/ledger/`fin_*`/payroll/invoice tables were re-verified sound at head — that UNIQUE key is also why re-issuing could not double-post, contrary to the audit's claim.
- [x] Routes/contracts: verify posting, approval, reimbursement, reconciliation, reversal, export and reminder operations use strict schemas, idempotency and valid financial state transitions. — S05. `PATCH /invoices/:id` applied any status with no transition guard: a PAID invoice could be reset to DRAFT and re-edited, and `VOIDED` set through PATCH bypassed both the payment-allocation checks and the journal reversal that `POST /invoices/:id/void` performs. `lib/invoice-transitions.ts` now allows only DRAFT→ISSUED and settleable→PAID/FAILED; `VOIDED` was removed from the PATCH schema and the frontend routed to the void endpoint. `check:contract-breaking-change` clean (100 published / 3,513 internal).
- [x] Authorization: test finance roles, approver separation, record/DataScope, employee self-expense access, immutable posted records and cross-tenant denial. — S05. **The expense approver's DataScope was discarded** — `perms.has("hr:expenses:approve")` is a key-presence test while that key is `scopable: true`, so an approver scoped to `own`/`team` read every expense in the org; scope is now resolved and applied in SQL, with `team` reaching direct reports and the scope folded into the cache key. **A second scope defect made a list unreachable:** `GET /hr/reimbursements` is gated by `hr:payroll:view` but resolved scope from `hr:expenses:approve`, and HR_ADMIN holds only the former — so the primary HR role saw a permanently empty list. Gate and scope now share one constant. Approver separation added to expenses and purchase bills. Disproved rather than implemented: `POST /hr/reimbursements` is a self-service create (it always writes the caller's own `userId`), not an escalation.
- [x] Queries/cache/workers: verify ledger/report/export plans, bounded reminder sweeps, asynchronous resumable exports, retries/cancellation/DLQ and derived-balance invalidation. — S05. Tax-due notifications enumerated orgs with a bare `.limit(500)`, so every organization past the 500th silently received no filing notice, ever; it now uses the canonical `forEachOrg` enumeration. The same sweep had no dedup (re-notifying daily across the 5-day window) and the bills-due dedup it copied **failed open**, turning a Redis blip into a notification storm — both now dedupe and fail closed. `offsets` (UNNESTed per invoice in a LATERAL join) capped at 50 and salary-profile `components` at 200, both confirmed HTTP bodies rather than outbox payloads first. `ignoreTransaction` was the one reconciliation mutation invalidating nothing. **Not proven here:** ledger/report *plans* need the seeded scratch database (master known-open).
- [x] Frontend/TanStack/tests: verify monetary precision, approval/reversal conflicts, report cursors, export job state and balanced-journal/cross-tenant E2E. — S05. Period and chart-of-accounts mutations invalidated only `["streamlineos","accounting","core"]`, which does not cover the report keys, so closing a period left Trial Balance, P&L, Balance Sheet, Cash Flow, Aged reports and GST returns stale; they now invalidate the `accounting` subtree, proven by a bite-tested regression test. 19 floating `invalidateQueries` promises given `void`; a banned `msg.includes("conflict")` branch replaced with `isApiError(e) && e.status === 409` against a backend that really does throw 409; twelve verbatim duplicate permission entries removed. The payout bank file had **no** UI path at all — `fileUrl` is always `null` from the batch response, so the download branch was unreachable — and now calls the authorized `GET /payroll/payout/batches/:batchId/file`. Frontend `tsc --noEmit` 0 errors; 11 suites / 66 tests green. **Balanced-journal/cross-tenant E2E remains master-open** pending the seeded database.
...
- [x] Routes/contracts: verify channel/message/thread/reaction/read/history/export routes use strict schemas, bounded cursors, server-derived actors and idempotent client message keys. — schemas, bounded cursors and server-derived actors verified. The idempotent client message key now exists: migration `0980` adds a nullable `client_key` with a PARTIAL unique on `(org_id, channel_id, client_key)`, so a retried send returns the original message and a race collides on the index rather than posting twice. `messages/poll` and `listMembers` also moved off silent 100-row truncation onto keyset cursors (S08).
...
- [x] Queries/cache/workers: verify timezone/DST, recurrence expansion limits, free-busy/conflict indexes, reminder replacement/deduplication, sync retries and range/source cache invalidation. — S09: everything except **indexes** is proven. DST round-trips through `toZonedTime`/`fromZonedTime` with BITE PROOF cases in `calendar-dst-edge.spec.ts`; expansion is capped at 500 occurrences per event and `CALENDAR_EVENTS_CAP` per source; reminder dedupe is `onConflictDoNothing` on `(orgId, dedupeKey)` with move/cancel tombstoning in the same transaction; sync retries are 5 attempts with `[0, 30s, 120s, 600s, 1800s]` backoff under `FOR UPDATE SKIP LOCKED`. **A silent truncation was fixed:** the reminder sweep read attendees with one `.limit(10_000)`, no `ORDER BY` and no continuation, so beyond that count a nondeterministic subset of people were never reminded — now keyset-paged with chunked inserts, bite-proven. **Indexes are now measured, and the answer was negative.** A candidate `(org_id, start_date, recurrence_end) WHERE rrule IS NOT NULL` index was authored as migration `0982`, then benchmarked on a seeded scratch cell (200,000 events, 20 orgs, the measured org a 2.5% minority) with `EXPLAIN (ANALYZE, BUFFERS)` as `streamline_app` and the tenant GUC set via `SET LOCAL` inside the transaction. The calendar list plan was **byte-identical with and without it, 2,594 buffers either way** — a partial index on `rrule IS NOT NULL` cannot serve a predicate that also contains an `rrule IS NULL` branch, since the planner cannot prove implication. The UNION rewrite normally prescribed for an OR-defeats-index shape measured **19% worse** (3,093 buffers), because the non-recurring branch dominates and can never use a recurring-only index. Only the reminder sweep's recurring-only query improved, 86 to 63 buffers on a periodic 200-row batch, against 1,128 kB and write amplification. **`0982` was reverted**; `idx_calendar_events_org_date` is the right index for this path. Recorded as a measurement, not an opinion. The conflict service queries the same OR predicate against the same table, so the measurement covers it; there is no separate free/busy endpoint. `check:cache-invalidation` is green across 1,051 service files.
...
- [ ] Architecture/schema: verify spaces, memberships, documents/pages, immutable revisions, attachments, ingestion jobs, chunks/embeddings and deletion/reindex state have tenant-composite integrity. — S10: tenant-composite integrity and deletion/reindex propagation are VERIFIED, and two ingestion defects were fixed — `indexPageDocument` inserted without a preceding delete or a transaction (duplicate chunks on every re-upload) and left `pageVisibility`/`aclRevision` NULL, which `chunkVisibleTo` can never satisfy, so page-document chunks were permanently unreachable by vector search. Two missing explicit `orgId` predicates were added (`kb-members` role-conflict guard, `retrieveTopSources` chunk side). STILL PENDING: **revisions are not immutable** — `kb-page-edit.util.ts:44-51` hard-deletes the oldest version past `MAX_VERSIONS = 100`, so page history is a rolling window while article history is insert-only. Left as a retention decision, not changed unilaterally.
- [x] Routes/contracts: verify CRUD, revision, publish/archive, search, ingestion, reindex, export and chatbot routes use strict schemas, bounded work and idempotency. — S10: search/ask/history/brief/import contracts were already capped (200-char query, 50/page, 1,000-char question, 100-item import). **Three unbounded or unvalidated reads fixed:** `KbArticleReindexService.reindexAll` loaded every published article for an org with no limit and no cursor (now keyset-paged at 100, mirroring `reindexAllPages`); `getBacklinks` materialised every inbound link and expanded it into `ANY(ARRAY[...])` (capped at 200); both review list endpoints read raw `@Query()` with no `@Validate`, so cursor params reached the keyset code unvalidated (strict Zod schemas added in `dto/`). A cross-tenant existence oracle was closed: `listConversations` accepted any conversation id in the system as a cursor.
- [x] Authorization/privacy: test org/user content, space/audience/record ACLs, draft/published visibility, attachment access and ACL enforcement inside keyword/vector retrieval before model context. — S10: `/kb/ask` traced end to end. ACL is in the SQL, never the prompt, on every branch — keyword article candidates carry the space filter plus a correlated restriction subquery, keyword page candidates carry `pageVisibleTo()` on live columns, and vector candidates `INNER JOIN` on `chunks.aclRevision = page/article.aclRevision`, a SQL `=`, so a stale chunk (mismatch or NULL either side) is excluded and the re-index window fails **closed**. The ANN pre-filter is RLS-scoped or goes through `app.search_kb_chunk_ids`, a `SECURITY DEFINER` function that reads `app.current_org_id()` and never takes an org parameter. Read/search parity holds because the final fetch re-applies the same live predicates the direct read uses. Public routes serve only `visibility='public'`, published, non-deleted rows.
- [ ] Queries/cache/workers: verify revision/search plans, ingestion leases/retries/DLQ, chunk dedupe, permission-aware cache keys, purge/reindex and realistic-corpus latency. — S10: leases (Redis `NX`, 300s TTL, Lua release), bounded retries into a DLQ, real `AbortController` cancellation, content-hash chunk dedupe and checkpoint resumption are all VERIFIED. **Embedding was the weak point and is fixed:** source and attachment indexing fired `Promise.all(chunks.map(embedQuery))` — one provider call per chunk, all at once, non-resumably, so a 500-chunk document opened 500 simultaneous requests and lost everything on the first rate-limit; `embedBatch` already existed with **zero callers** and all three sites now batch through it at 64, sequentially. STILL PENDING: **realistic-corpus latency and query plans are not measured** — that needs `EXPLAIN` as `streamline_app` with the tenant GUC against a seeded database, the repo-wide deferred runtime-measurement item.
- [ ] Frontend/TanStack/tests: verify editor/revision conflicts, search cursors, permission changes, citations/source integrity, ingestion states and ACL/purge/reindex E2E. — S10: citations are safe by construction (numeric `data-page-id`, never a server-supplied URL) and article HTML is DOMPurify-allowlisted. **Four defects fixed.** `useKbPageComments` gated a READ on `kb:pages:update`, so any member holding only `kb:pages:view` had a permanently empty comments panel. Nine query-key factories, including `kb.contentGaps`, omitted the conditional branch their siblings have, so a no-arg call produced a trailing `undefined` that TanStack's partial matcher never matches — proven empirically against a real `QueryClient` (0 matches before, 1 after), not asserted. Both AI action menus rendered with no `useCan("kb:ai:generate")` gate, firing a guaranteed 403. Three infinite queries never forwarded the TanStack `signal`, so abandoned reads ran to completion. STILL PENDING: **wiki page editing has no concurrency control** — `PATCH /kb/pages/:id` carries no `If-Match`/version and autosave fires 1,500 ms after typing stops, so two tabs silently clobber; and browser E2E is not claimed.
...
- [x] Architecture/schema: verify shared modules expose narrow interfaces and do not absorb feature authorization or business rules; integration credentials remain server-side and tenant-bound. — S12: `StorageService` carries no feature permission check; every gate lives in the calling controller. All five write paths namespace the object key `${orgId}/${folder}/…`. Only Composio's opaque `composioConnectedAccountId` is mirrored — no provider token is stored — and webhook signing secrets are AES-256-GCM at rest with a per-write IV. The shared→feature direction is now enforced by `check:import-direction` rather than asserted.
- [x] Routes/contracts: verify upload/download/search/token/integration callbacks validate input, authenticate provider callbacks and never expose provider secrets or internal object keys. — S12: no response DTO carries an R2/S3 credential or endpoint secret; `CONNECTION_COLUMNS` returns none. Git callbacks verify GitHub HMAC-SHA256 / GitLab tokens with `timingSafeEqual` and an unknown format fails closed; the tenant is resolved server-side by `app.resolve_git_connection_org_id(connectionId)`, never from a caller-supplied field. `check:multipart-contracts` 542 controllers green. Two fixes: `storage-key-catalog.ts` built `sql.raw` with hand-escaped interpolation for `orgId`/`userId` and is now parameterized, and the webhook `events[]` array was `z.array(z.string())` with no bound on count or item length.
- [x] Authorization/security: prove callers supply an authorization context that shared adapters cannot bypass; test SSRF, malicious files, token replay and cross-tenant resources. — S12, four defects closed. **Malicious files:** multipart completion created a quarantine row with `fileSizeBytes: 0, sha256: ""` and immediately marked it `clean` — bytes go browser→S3 and were never scanned; it now stays `pending_scan` and downloads stay blocked, which removes no capability because the frontend has **zero** callers of any `storage/multipart` route. Onboarding upload had no AV scan at all and now fails closed identically to `StorageController.upload` (infected → 422, scanner error → 503, before any write). **Privilege:** `markClean`/`markInfected` carried no prior-state predicate, so an admin could `release` an `infected` record and silently clear a malware detection; both now carry a status predicate and the controller returns 409. **SSRF:** webhook delivery called `checkWebhookUrl(url)` and then `fetch(url)`, re-resolving DNS independently on each of up to five attempts — a textbook rebinding window; delivery now goes through the existing `postSafeWebhook`, which resolves once and pins the address via `pinnedLookup`, and an SSRF block is terminal rather than retried. No second guard was written. **Cross-tenant:** proven end-to-end by a new seeded-E2E artifact (2/2) rather than by a service double. **Still open:** inbound git webhooks have no timestamp window or delivery-id dedupe — the operations are largely idempotent, so this is replay-as-DoS rather than replay-as-corruption; it needs a table and is handed to S02.
- [x] Queries/cache/workers: verify bounded search, tenant/ACL predicates, backpressure, retries/DLQ, idempotent callbacks, cache namespaces and resource cleanup. — S12: webhook delivery is bounded at 5 attempts with full-jitter backoff, a 10 s timeout, a per-endpoint circuit breaker and dead-lettering to `webhookLogs`; `dispatch()` swallowed every failure with `.catch(() => undefined)` and now logs. `listConnections` is capped, webhook list/log pages are capped at 100. **Resource cleanup did not exist:** `sweepAbandonedUploads` was implemented, cursor-based and resumable, with **zero callers**, and nothing expired stale `pending_scan` or infected quarantine records or their objects. A registered `CronStorageSweepService` now does both per org. **The first version of that sweep opened a hole and the review caught it** — it deleted the object and soft-deleted the record in separate `try` blocks, and `isKeyBlocked` ignores soft-deleted rows, so a failed S3 delete would have *unblocked an infected key*; the soft-delete is now reachable only after a successful object delete, and the record survives for the next sweep. A fire-and-forget call in the git webhook ran after its tenant transaction committed, so its queries would have died `42501`; it now opens its own tenant transaction.
- [x] Consumers/tests: verify every produced event has a registered consumer or explicit terminal sink and exercise adapter fakes plus cross-module contract tests. — S12: `check:outbox-consumers` reports **24 emitted / 24 consumed**, including `integration.connection.disconnected` (consumed by `IntegrationConnectionDisconnectedConsumer` with inbox claiming) and `gdpr.export.requested`. Adapter fakes and cross-module contract tests run green across **117 suites / 804 tests** covering storage, gdpr, integrations, webhooks, cron and `test/security`.
...
- [x] Architecture: verify route groups and feature folders mirror ownership, shared UI stays domain-neutral and no business/database implementation exists in the frontend. — S11: `check:routes` confirms the only `app/api` route in the repo is `app/api/auth/[...nextauth]/route.ts`; the frontend holds no database client, schema, adapter or migration. `check:import-direction` and `madge --circular` hold the one-directional features → shared graph.
- [ ] TanStack/contracts: verify query-key factories, parsing, invalidation, hydration, cancellation, retry, optimistic concurrency and pagination rules across every module above. — S11 proved the shared half: cancellation (1,053 queryFn blocks, 125 repaired, the class now a compile error), hydration (six prefetch factories through one scoped hash), retry (`mutations.retry: 0`), key factories (zero ad-hoc keys, `queryKeyBase` canonical) and a typed optimistic implementation per cache shape. This box is an aggregation gate and stays open until each domain session evidences its own modules.
- [x] RBAC/UI: verify authenticated layout, route/action parity, module navigation, permission changes and organization switching without flashes of unauthorized content. — S11: `check:route-access-contract` maps 201 route-access keys onto real `x-permission` endpoints in the generated contract, `check:navigation-permissions` and `nav-surface-parity.test.ts` prove the desktop sidebar, mobile drawer and command palette consume one filtered navigation model, and `sidebar-permission-coverage.test.ts` fails on a route without a `requiredPermission`. Organization switching remounts the Query provider on the `authenticated:<orgId>:<userId>` scope key, so no other tenant's payload survives the switch. **Command parity is now enforced too:** 1,422 mutations carry their exact backend permission, and an unresolved access snapshot no longer reads as a denial.
- [ ] UX/accessibility: verify loading/empty/error/offline/permission states, keyboard/screen reader, focus, contrast and responsive 375/768/1280 behavior. — S11 closed four real shell gaps: no root `global-error.tsx`, a retry that never reset the query cache so the failing entry survived it, no shell offline state (`useOnlineStatus` was consumed only by the notifications page and the barcode scanner), and no focus move after client navigation, which left the skip link and `#dashboard-content` decorative for keyboard users. `check:icon-labels` and `check:empty-states` are green and canonical `NoPermissionState`/`AccessDenied` components are in place. The shell also gained **32 automated a11y/keyboard assertions** where it had none — eleven a11y suites existed and none covered the surface every authenticated route renders — each opening with a positive guard so it cannot pass on an empty render, and both suites proven to guard the source rather than themselves. STILL PENDING: **contrast is not verified and no test claims to be**, because axe under jsdom cannot evaluate computed CSS; the `AppSidebar` axe case is skipped for the same reason, since with its hooks mocked empty the nav renders nothing. The 375/768/1280 sweep needs a real viewport.
- [ ] Performance/SEO/tests: verify bundle boundaries, lazy loading, rendering/Web Vitals budgets and public metadata without changing landing visuals/animations; run representative browser E2E. — S11: four module-scope shell imports that each serve one route (`ChatMobileBottomNav`, `ProjectNavTree`, `LeaveOrganizationDialog`, `ArchivedOrgsRestore`) moved behind `next/dynamic`; `LeaveOrganizationMenuItem` was deliberately left static because Radix builds its keyboard focus ring synchronously and deferring it would break arrow-key navigation. `check:seo-metadata` green; landing visuals and animations untouched, confirmed by diff. A production build now succeeds (exit 0) and `measure-route-bundles` reproduces the recipe the manifest documents, so those numbers stop being unreproducible; it measures `/inbox` at **461,746 bytes against its 524,288 ceiling**. STILL PENDING: **the new numbers were deliberately not written**, because the same run also reports `/mail` and `/crm/inbox` ~85k lower than recorded on routes this session never touched — consistent with a shared-shell reduction, but not separable from a possible recipe difference without an A/B rebuild, and turning a red gate green on a methodology change is the same sin as raising a budget. Web Vitals and browser E2E remain unrun: there is no browser automation in this repo.
- [ ] Reduce authenticated client route modules below the current 304-page ceiling, never raise that ceiling, and move data/authorization/orchestration to server or feature seams while preserving interactive leaf components; public landing visuals and animations remain untouched. — S11: client route modules are **260 of 600 against the 304 ceiling**, 44 below, and the ceiling was not raised. The missing half — actually moving orchestration out — is now measurable rather than asserted, and has started to move: the new **`check:route-thinness`** ratchet reported **118 in-scope** route modules holding state, data fetching, forms or transport, or over 300 lines, against **67** more in CRM/Inventory printed under OUT OF SCOPE rather than filtered away. The four that called `apiClient` directly — the only §5 transport violations among them — are repaired into feature-owned hooks and five-line pages, taking the count to **114** with the ratchet lowered to match. The ratchet is proven to bite. The remaining 114 are owned by their domain sessions: HR 39, Accounting 32, Support 14, Build 10, Workflows 7, Notifications 6, Surveys 2, Settings 2, Payroll 1, Chat 1.
...
- [ ] Implement correction/rectification rather than treating export, deletion or anonymization as correction. — S12: a real correction path exists and is not export-in-disguise — `gdpr-rectification.service.ts` writes `users.name` at the source of truth, tenant-scoped, `@Idempotent`, with optimistic concurrency on the current value. **It covers exactly one field.** `gdpr-rectification.schemas.ts` is `field: z.literal("profile.name")`, so every other subject-controlled value has no correction path: `hr_people` personal email/phone/address/DOB/gender/preferred name and emergency contacts (S04), bank and payroll details (S05), and `users.email`, which is authentication-linked and needs a verification challenge rather than a blind update. Named per owner so the work is scoped rather than rediscovered.
- [ ] Make subject export exhaustive and resumable with no silent caps or skipped in-scope sources. — S12: the async path is exhaustive over 55 subject-scoped sources, resumable (checkpointed, `reclaim()` returns stale `running` jobs to `pending`) and atomic with its outbox event. Three defects closed. **The admin view was inert:** `GET /gdpr/export-async/:jobId/status|download` declared `@AuthorizedInService("… or hold hr:retention:manage")` but applied only `JwtAuthGuard`, and `PermissionGuard` is the only guard that sets `req.rbacScope` and is not global — so `isAdmin` was always `false` and `findJob` always restricted to the caller's own jobs. Both handlers now resolve through `authorize()`, proven end-to-end by a seeded-E2E artifact that asserts subject 200 / same-org non-admin 404 / same-org retention-admin 200 / other-org admin 404. **The worker silently dropped every job when object storage was unconfigured** — `handle()` called `wake()`, which is `void this.tick()`, and `tick()` returned early, so the outbox event was acknowledged while the job stayed `pending` forever with nothing reporting it; `wake()` now throws so the relay retries. **Five synchronous-export reads were unbounded** (`auditLogs` worst) and now bound at 500 while *naming each truncation* in the existing `exportIncomplete` contract, rather than either silently truncating or OOMing. **Open:** a truncated synchronous export still records `status: "completed"` because no honest value exists in `hr_data_request_status` — a `partial` value is handed to S02.
- [ ] Implement idempotent tenant-scoped erasure for database, object storage, search/vector, projections, caches and supported adapters while preserving immutable/legal-hold records. — S12: **object storage only, and until this session it erased nothing at all.** `enumerateFileKeyColumns` rendered `n.nspname = ANY(($1, $2, $3))` because Drizzle expands a JS array into a row constructor; Postgres rejects that with `42809 op ANY/ALL (array) requires array on right side` — measured directly against the database alongside the `IN` form, which succeeds. Every subject and org storage purge therefore aborted its transaction before enumerating a single key, and the failure surfaced only as a `25P02` on the follow-on statement. Now an `IN` list, with two assertions pinning the rendered SQL. Legal hold is sound and bites *inside* the predicate rather than as a pre-check (`gdpr-storage-purge.service.ts:149`, `cron-hr-retention.service.ts:135`, `cron-org-purge-worker.service.ts:127`). **Still missing entirely:** database-row deletion/anonymisation (no service nullifies `users.name`/`email`, `hr_employee_sensitive_fields`, `hr_dependents`, biometric or tax rows), search/vector index removal, projection cleanup, and Redis session/permission invalidation — after an erasure the subject's cached session and permission version survive to TTL.
- [x] Resolve retention for `helpdesk_tickets`, `performance_reviews`, `mail_message_metadata` and `announcements` with bounded policy or explicit KEEP-FOREVER configuration. — decisions recorded in `RETENTION-POLICY.md` and enforced in code by `cron-helpdesk-retention`, `cron-mail-retention` and `cron-announcements-retention` services; `performance_reviews` is an explicit KEEP-FOREVER (employment-record obligation). Covered by `cron-pending-retention.spec.ts`. ⚠️ **The enforcement gate has not caught up and cannot, by construction.** `check:retention-coverage`'s `RETENTION_MATRIX` still lists all four as `PENDING-DECISION` and warns they have "no retention decision", and its own self-test asserts `unsupportedTablesRemainPending` — so the stale state is *required* by the check meant to detect staleness, and updating the policy document can never make the gate agree. This box stays checked because the decisions and workers are real, but the matrix and that self-test need correcting by the retention owner (S02/S12) before the gate means anything. Found by S10 2026-09-02.
- [x] Resolve retention for `kb_page_versions` with bounded policy or explicit KEEP-FOREVER configuration. — S10 2026-09-02: **KEEP-FOREVER**. The table had no entry in `RETENTION-POLICY.md` at all while `kb-page-edit.util.ts` silently hard-deleted the oldest revision past a 100-version cap, so the 101st edit destroyed revision 1 with no audit record — retention policy by accident, and a direct contradiction of §10.16's "immutable revisions". The delete is removed and the table is insert-only, matching `kb_article_versions`; growth is bounded by the pre-existing `VERSION_WINDOW_MS` snapshot throttle rather than by deletion, and `listVersions` is keyset-paged so unbounded history is not silently truncated. Recorded in `RETENTION-POLICY.md` and added to the `check:retention-coverage` matrix (self-test 20/20).
- [ ] Prove retention workers are code-scheduled, bounded/resumable, idempotent, audited, retryable and emit failure events. — S12: bounded, resumable and idempotent hold — every sweep uses `forEachOrg`, a fixed batch (200–500) and a `CronLeaseService` lease. Audited now holds too: `CronMailRetentionService` deleted rows without the `hr_audit_logs` record its own policy entry requires, while both sibling workers wrote one; the audit write now matches `cron-helpdesk-retention.service.ts` exactly. **The enforcement gate contradicted the shipped code and could not be corrected by editing the policy:** `RETENTION_MATRIX` still listed `helpdesk_tickets`, `performance_reviews`, `mail_message_metadata` and `announcements` as `PENDING-DECISION` while `RETENTION-POLICY.md` recorded decisions for all four and three workers were registered in `cron.module.ts` and routed — and the self-test assertion `unsupportedTablesRemainPending` *required* that stale state. Matrix reconciled against source and policy, the assertion narrowed to `notification_outbox` and `outbox_events` which are genuinely undecided, and two new assertions fail if a decided table regresses. `check:retention-coverage` EXIT=1 → **EXIT=0**, self-test 22. **Open on two clauses:** no worker is `@Cron`-scheduled — every one runs only when an external scheduler POSTs its route with `CRON_SECRET`, so a silently stopped scheduler stops retention with no dead-man's-switch — and no sweep emits a failure event; a failure is observable only as an HTTP 500 on the trigger call.
...
- [x] Prove TanStack Query deduplicates concurrent callers, cancels abandoned reads, avoids retry storms, retains useful previous pages and invalidates only affected tenant/module keys. — S11: deduplication is structural, since one scoped hash (`authenticated:<orgId>:<userId>` + the key array) means concurrent callers of the same read share one in-flight query. Abandoned reads now genuinely cancel — 125 that could not were repaired and the failure mode is a compile error rather than a lint rule. Retry storms are bounded by `MAX_QUERY_RETRIES = 1` with no retry on 4xx except 408/429, and `mutations.retry: 0` means no write is ever replayed. Previous pages are retained through `placeholderData: keepPreviousData` on the cursor lists, and `lib/optimistic-cache.ts` patches `InfiniteData` page-by-page instead of replacing the collection. Tenant/module isolation holds because the tenant lives in the hash, not the key, so an `invalidateQueries` prefix cannot reach another tenant's entry — and the provider remounts on the scope key at organization switch.
...
- [ ] Route every AI feature through one backend AI gateway with small model/provider interfaces, centralized timeouts, usage accounting, policy, redaction and observable error modes; no frontend direct-provider calls. — S10: LLM calls all route through `AiGatewayService` with centralized per-tier timeouts, redaction on by default, usage accounting and a discriminated failure result; no frontend direct-provider call exists. **Embedding spend is now accounted too.** `EmbeddingsService` previously recorded no usage, reserved no credit and took no `orgId`, so every embedding was unattributable provider spend. It now takes a **required** `orgId` and `feature` on all three methods — required, not optional, so a call site cannot quietly fall through to a generic bucket — and awaits an `ai_usage_logs` row after each provider call; `AiUsageService.track` opens its own tenant transaction with an explicit `orgId`, so attribution survives in the background indexing worker and not only on request paths. `text-embedding-3-small` was added to the pricing map at its published $0.02/1M input rate, with `tokenEstimate: true` recorded because the embeddings API returns vectors and not token counts. All ten call sites pass a specific feature. `embedQueryDeduped` still makes exactly one provider call per unique text but records one usage row **per calling org**, so a second tenant's questions are no longer free because a first tenant asked the same thing. A metering failure cannot break retrieval. STILL PENDING: in `kb-rag.service.ts` the embedding is still issued before the gateway's credit reservation, so an org at zero credits pays a very small embedding charge per anonymous question; correcting the order needs a check-only credit method on the gateway or would bypass its reserve atomicity, so it was not forced. The `hasPublishedPublicArticles` short-circuit still bounds it to orgs that publish.
...
- [ ] Bound prompts, history, retrieved chunks, tool iterations, output tokens, concurrency and per-tenant/user rate; reject or summarize oversized context rather than consuming unbounded memory/cost. — S10: context is bounded at 200,000 chars and rejected with `context_too_large` before any reservation; tool iterations stop at `stepCountIs(10)`; retrieved chunks are capped by the candidate pool; rate tiers are 30/min `ai:invoke`, 20/min `ai:chat`, 10/min `ai:public-kb-ask`, all verified present in `TIERS`. **Output tokens were unbounded and are now fixed** — a caller omitting `maxTokens` inherited the provider default while `AI_FEATURE_COSTS` are only reserve *ceilings*, so a long completion silently overran its reservation; the gateway now bounds all three paths at 4,096 and clamps a caller asking for more. **Concurrency is now capped too:** a new `AiConcurrencyLimiter` acquires a per-org slot before dispatch and releases it in a `finally` on every exit path including throws, at 20 concurrent calls, backed by a Redis `INCR` with a TTL so the cap holds across pods, with a local fallback and fail-open when Redis is absent or throwing; exceeding it returns a typed `concurrency_exceeded` failure rather than a 500. Both exhaustive switches over `AiInvokeFailure["kind"]` were updated for the new member — `gateway-result.util.ts` and `workflows/engine/executors/ai-action.executor.ts` — the latter being a permitted caller adaptation outside S10's module.
- [x] Enforce tenant, subject, permission, document lifecycle and record ACL predicates inside keyword/vector retrieval before any chunk reaches the model. — S10: proven by tracing `/kb/ask` end to end. Every candidate branch binds the predicate in SQL, never in the prompt: keyword article candidates carry the accessible-space filter, `status='published'` and a correlated restriction subquery; keyword page candidates carry `pageVisibleTo()` on live `kbPages` columns; vector candidates `INNER JOIN` on `chunks.aclRevision = page/article.aclRevision`, so a chunk whose ACL snapshot is stale — or NULL on either side — is excluded and the re-index window fails **closed**. The ANN pre-filter is RLS-scoped or goes through `app.search_kb_chunk_ids`, a `SECURITY DEFINER` function that reads `app.current_org_id()` internally and returns ids only. Read/search parity holds because the final fetch re-applies the same live visibility predicates the direct read endpoint enforces. One gap closed: `retrieveTopSources` leaned on RLS alone for the chunk-side tenant scope while every sibling bound it explicitly.
- [x] Batch and deduplicate parsing, chunking and embeddings; make ingestion resumable/idempotent with leases, retries, cancellation, DLQ, progress and deletion/reindex propagation. — S10: leases are Redis `NX` with a 300s TTL and a Lua release, retries are bounded by `shouldDeadLetter(retryCount)` into a DLQ, cancellation threads a real `AbortController` into `adapter.handle()`, article/page chunking resumes from `KbIngestionCheckpointService` and skips unchanged content by source hash, and deletion/reindex propagate on soft-delete, archive, unpublish and ACL change. **Embeddings were neither batched nor deduplicated and are now fixed:** source and attachment indexing fired `Promise.all(chunks.map(embedQuery))`, one provider call per chunk all at once and non-resumably, so a 500-chunk document opened 500 simultaneous requests and lost all progress on the first rate-limit — `EmbeddingsService.embedBatch` already existed with **zero callers**, and all three sites now batch through it at 64 sequentially with chunk order asserted.
...
- [ ] Propagate client aborts, enforce deadlines and circuit breakers, and retry only replay-safe pre-stream operations; never duplicate a paid request or continue spending after cancellation. — S10: **two defects found and fixed.** (a) **Client aborts never reached the provider on the non-streaming path** — `AiGatewayRunnerHelper` accepted no signal and `LlmService.attempt()` had none, so a disconnected client left the call running and left the retry loop free to walk the whole model chain (up to 9 attempts × 30s) for a request nobody was waiting for; `signal` is now threaded runner → `attempt()` → `client.invoke(messages, { signal })` with `throwIfAborted()` before each attempt and an abort short-circuit that stops retrying. (b) **The chat circuit breaker could never open** — `recordCbFailure()` was reachable only from the `catch` around `buildStream()`, but `streamText()` returns synchronously and provider faults surface asynchronously on the stream, so `cb.failures` stayed 0 forever; `onError` is now wired, with client aborts excluded so one user cancelling does not trip the breaker for everyone. Streaming already carried a combined abort signal (client close + 120s deadline) and mutually-exclusive settle/release. Both residues are now closed as well. The breaker's failure count moved to Redis behind `@Optional() @Inject(REDIS)` so pods share one view — `CacheModule` is `@Global()` and exports the token, verified rather than assumed, since an optional inject that never binds would be the same inert mechanism as the original defect — falling back to in-process state when Redis is absent or throwing. And the settle/release race is gone: `stream.finishReason` no longer releases on success, only on the abort-before-output rejection, so normal completion settles with real tokens, a mid-stream abort settles the partial usage `onFinish` reports, and abort-before-output keeps the pre-existing spec-pinned refund. Determinism is proven by two tests that resolve `finishReason` before and after `onFinish` and assert the same billing outcome either way.
- [x] Cache only explicitly cacheable AI artifacts using tenant, subject/ACL version, model, prompt/version, source revision and policy dimensions; invalidate on permission, content, model or prompt change. — S10: the cache was **unsafe by default** and is now unsafe-by-construction-impossible. `aclVersion`, `sourceRevision` and `policy` were all optional and the `cache: true` shorthand resolved to `{}`, so the key was org + feature + tier + prompt-hash with **no reader dimension** — two users in one org asking the same question would have shared an answer built from documents only one could read. `aclVersion` is now a required field on `AiResponseCacheOpts`, the ACL-blind shorthand is deleted, and a runtime guard refuses an empty value, so the unsafe state is unrepresentable rather than merely discouraged (backend/CLAUDE.md §4). Also fixed: a failed invocation was written to the cache, so one provider blip was served for an hour. Tenant separation was already sound (`orgId` namespaces every key) and invalidation is an O(1) namespace version bump. Note the cache has zero callers today — this makes the first caller safe rather than closing a live leak.
- [x] Defend against prompt injection, unsafe tool arguments, SSRF and data exfiltration with allowlisted tools, validated arguments, output schemas, content controls and least-privilege execution. — S10: tool arguments are Zod-validated by the SDK against per-tool `inputSchema` before `execute`, tool names come from trusted module factories and are narrowed further by persona filtering, each tool re-checks the caller through `ToolAccessService.denyReason`, and no tool in scope accepts a URL, so there is no SSRF surface. Structured output uses `withStructuredOutput(..., { strict: true })` and is re-parsed with Zod; a `ZodError` releases the reservation and returns `invalid_output` rather than a partial answer. **One exfiltration path closed:** the `email.send` and `mail.send` confirmed actions built `` `<p>${bodyText}</p>` `` straight from model output and sent it to an **external** recipient, so a poisoned KB page could steer the model into emitting active markup or a tracking pixel; both now escape through the shared `escapeHtml`, pinned by a boundary spec that fails if any future AI email path interpolates an unescaped value.
...
- [x] Run deterministic AI gateway/retrieval/stream tests plus representative provider-sandbox tests when credentials are available; prove identical authorization for direct document reads and AI-assisted retrieval. — S10: backend `modules/(kb|ai|search)` runs **120 suites / 768 tests green** (baseline 113 / 723), frontend KB/AI **8 suites / 53 tests** (baseline 3 / 30) inside a **full** frontend run of **234 suites / 2,194 tests green**. Identical authorization is proven by `kb-read-search-parity.spec.ts` plus the traced retrieval path: keyword search and the final vector fetch apply the same `pageVisibleTo`/restriction predicates the direct read endpoint enforces, and the vector candidate stage fails closed on an ACL-revision mismatch. Every new guard was bite-proven by neutering the mechanism, observing red, and restoring. Provider-sandbox tests were NOT run — no credentials in this environment.

## Known-open at the 2026-09-02 reconciliation

Recorded explicitly so no unchecked box above is mistaken for an oversight.

- **Both over-300 ratchets are above baseline, and neither regression is S12's.**
  Backend is **397 vs 394** (395 when S12 started) and frontend is **521 vs 519**.
  S12's two splits are provably neutral — `projects-work-query.service.ts` 460 →
  370 + 33 + 66 and `cron-hr.controller.ts` 526 → 335 + 216, one file over 300
  before and one after in both cases. The backend +3 sits in files one to five
  lines over the line (`user-permission-grants.service.ts` 302,
  `chat-assistant.service.ts` 301, `notification-events.catalog.ts` 301), each
  owned by another session; splitting a service to shed two lines is the cosmetic
  fragmentation §2 forbids, so they are named rather than cut. Separately,
  `src/modules/support/core/support-ai-triage.service.ts` reached **611 lines**
  during this round and breaks the hard 500 gate with no exception entry. It is
  genuinely mixed-responsibility — four inline Zod schemas that CLAUDE.md §6
  requires in a `*-schema.ts`, four persistence helpers and seven AI features —
  but it was last written thirty minutes before the measurement by a concurrent
  lane, and the sessions README forbids redesigning another session's
  implementation in a shared tree. Owner: S10.

- **The frontend over-300 ratchet counts code the release excludes.** 82 of its
  521 files are CRM or Inventory, so an out-of-scope edit can fail an in-scope
  gate and an out-of-scope reduction can mask an in-scope regression. Every
  comparable backend gate partitions scope explicitly
  (`check:tenant-relationships`, `db:verify-rls`, `check:bounded-contracts`).
  Reported rather than changed: a single historical baseline cannot be
  decomposed after the fact, and re-scoping the gate now would turn a failing
  ratchet into a passing one, which is the move this PRD warns against. Owner:
  S11.

- **`pnpm validate:env` cannot pass as written.** The script is
  `node -r ts-node/register/transpile-only src/scripts/validate-env.ts` with no
  `--env-file-if-exists=.env`, unlike every sibling env-dependent script, so it
  fails on a correctly configured machine with six "expected string, received
  undefined" errors. Run with the env file it exits 0, and its self-test passes.
  §1 lists env validation among the one-commit release gates, so the gate as
  invoked reports a false negative. Owner: orchestrator.

- **Nine `src/common/** → src/modules/**` imports remain, now pinned rather than
  latent.** Root CLAUDE.md §9 requires one-directional flow and nothing enforced
  it. `check:import-direction` (S12) fails any new violation and carries the nine
  in a baseline naming the symbol to relocate and its owner: `EntitlementsService`,
  `MfaPolicyService`, `REQUIRE_PERMISSION` (twice), `DataScope`,
  `ACCESS_MANAGED_MODULES` and `AccessService` for S01; `crypto.helpers` (twice)
  for S04. The gate also fails if a baseline entry silently disappears, so the
  ratchet can only tighten. Relocating `AccessService`'s guard is the only entry
  with real blast radius; the rest are constants and types.

- **knip reports 66 unused exports and 127 unused exported types in the backend,
  removed only where owned.** S12 deleted one graph-proven dead file
  (`src/common/auth/backend-claims-schema.ts`, superseded by
  `JwtKeyringService.verifyToken`) under the four-part standard: zero symbol
  references, no barrel in `common/auth`, no dynamic import, knip-confirmed. The
  remainder spans every domain session's files, and CLAUDE.md §10 requires a real
  `nest build` to confirm a removal — which the sessions README reserves for the
  orchestrator. Left as a measured list rather than a sweep executed without its
  confirming gate.

- **S11 closed the signal item and then found the same lesson twice more, in
  bigger numbers.** `check:query-signal` reported "0 violations across 399 hook
  files". A full-body positional parse of the same tree found **125** reads that
  could not be cancelled, across **1,053 queryFn blocks in 417 files**: 39 never
  destructured `signal`, 85 destructured it and never forwarded it, and one
  passed it as the **request body** of a `POST` the contract marks
  `x-bodyless: true`. The fix that matters is not the 125 edits — `apiClient.get`'s
  `params` is now a type an `AbortSignal` cannot inhabit, so the whole class is a
  compile error, proven by an in-tree probe rather than argued.
  The command catalog repeated it at larger scale: its 677 baseline came from a
  scanner that matched `useMutation(` when the dominant form in this repo is
  `useMutation<T, E, V>({…})` — **382 of 463 raw call sites were invisible to the
  number the PRD was tracking**. Two smaller blind spots went with it: an
  `export const` arrow one-liner borrowed the next declaration's body, and an
  apostrophe in a code comment ("the person's own task list") put the argument
  parser into string mode and hid a hook's declared permission key. The catalog
  is now derived from `contracts/openapi.json` and sits at **0 unclassified of
  1,501**, which also means no permission key in it was chosen by a person.
- **Three UI controls call endpoints that do not exist.** Found by resolving every
  mutation hook against the contract and confirmed in the backend source, not
  inferred: `useSetPresenceStatus` calls `PUT /chat/presence/status` while
  `chat-presence.controller.ts` declares only `POST presence/heartbeat` and
  `GET presence/online`, so every status change in the chat presence menu 404s
  (S08); `useDeleteInvoice` calls `DELETE /invoices/{id}` where the controller
  declares no `@Delete`, breaking the delete action in both
  `invoice-detail.tsx` and `invoices-client.tsx` (S05); and
  `useSupportKbAttachmentDownloadUrl` calls a `GET` by attachment id that
  `support-kb.controller.ts` never declares, and the list projection deliberately
  returns no URL to fall back to (S10). Each needs a backend route, which is
  outside S11's boundary.
- **Three frontend permission keys were wrong in the direction that blocks the
  user the feature exists for.** `useApproveStage` and `useRejectStage` demanded
  `payroll:runs:manage` while `payroll-approvals.controller.ts:89,123` requires
  `payroll:runs:approve`, so a payroll approver holding only `:approve` was
  refused by the UI before the request was ever made; `useExportTaxReport`
  demanded `payroll:tax:manage` against `tax-admin.controller.ts:82`'s
...
- **`useAuthorizedMutation` read "access not loaded yet" as "denied".** It
  ...
  unresolved gate as a refusal tells a permitted user they lack access. Harmless
  at 421 hooks and not harmless at 1,422, so the guard now resolves the access
  snapshot before deciding. `prefetchAccess` returning an empty dehydrated state
  on any fetch error is what makes the pending branch reachable in production.
- **`check:over-300` is RED at 521 against a 519 baseline and no session caused
  it.** Settled by measurement rather than argument: the over-300 set was computed
  for every `.ts`/`.tsx` file in the working tree and for the same paths at
  `HEAD` — **525 at HEAD, 525 in the working tree, 0 added, 0 dropped**. The
  baseline is stale with respect to `HEAD`. Orchestrator-owned.
- **The `/inbox` bundle and authenticated Web-Vitals gates are RED on stale
  numbers.** Four module-scope shell imports moved behind `next/dynamic` and no
  budget was raised, but both gates read measurements captured before that edit,
  and re-measuring needs `next build && next start`, which section 10.18 reserves
  for the orchestrator. The arithmetic that framed the problem still holds: only
  4 of `/inbox`'s 42 chunks (13,519 bytes) are inbox-specific while the shared
  shell was 545,161 bytes against a 524,288 ceiling, so the shell was the only
  place the excess could come from, and that is where the four deferrals were made.

- **S10 closed with twelve named open items, and two of its finds argue that a
  green gate can certify an inert mechanism.** The chat circuit breaker had a
  threshold, an open duration, a reset and a passing test — and could never
  open, because its only `recordCbFailure()` call sat in the `catch` around
  `streamText()`, which returns synchronously and surfaces provider faults
  asynchronously on the stream. Its test passed because the fixture threw
  synchronously, which is the one shape the real provider never produces. The
  same pattern appeared in the AI response cache: `aclVersion` existed, was
  documented, was concatenated into the key — and was optional, with a
  `cache: true` shorthand that resolved to `{}`, so the mechanism was present
  and unreachable. Both are the §11 lesson restated: a self-test must feed the
  shape the mechanism is most likely to miss, and an optional safety dimension
  is not a safety dimension. The rest of S10's open list is in its ticket;
  the ones needing another owner are `check:file-sizes` red on
  `build/core/projects-work-query.service.ts` (S06) and
  `cron/cron-hr.controller.ts` (S04), and `check:module-di` red on
  `OutboxPublisherService` needing `OutboxReportService` declared in
  `ArchitectureEvidenceModule` (S03/S12). Both gates were green at S10's
  baseline and went red during the session from work outside its boundary;
  `chat/chat-messages.service.ts` was on this list earlier and S08 has since
  cleared it.
- **S08's eight open items are now closed, and two of them were worse than
  recorded.** Chat attachments were the sharper one. The first pass could not
  tell whether `chat_attachments.fileUrl` held a signed or a permanent URL; it
  holds a **permanent unauthenticated R2 URL**, because chat uploads use folder
  `"chat"` and `publicUrlFor` only withholds the public base for
  `PRIVATE_HR_FOLDERS`. Access therefore survived leaving the channel and leaving
  the organization, while §9 above claims downloads are re-authorized — true of
  `/storage/download`, false of every chat attachment. S08 added a
  membership-gated attachment route returning a 1-hour signed URL and pointed the
  client at it; **the storage half is S12's**: add `"chat"` to
  `PRIVATE_HR_FOLDERS` and backfill existing rows from URL to key, or the old
  public URLs keep resolving.
  The second was `chat_channels` carrying both `is_private` and `type`. The
  membership guard picks 404-over-403 from `is_private`, and `createChannel` set
  it only for `type='PRIVATE'` — so a DIRECT message and an invite-only GROUP
  were stored `is_private=false`, and a non-member probing a DM got 403, which
  confirms the conversation exists. Migration `0981` backfills
  `is_private = (type <> 'PUBLIC')` then constrains it.
  Both migrations were executed against the live schema inside a rolled-back
  transaction, so the live database is unchanged, and both were proven to bite:
  a duplicate `client_key` raised 23505 and a mismatched privacy row raised
  23514. Journal entries sit above the database watermark. **A lesson worth
  keeping: the two migration gates are currently red for reasons that are not
  these migrations** — 19 AR-02 files (0956, 0962-0979) lost their journal
  entries to a concurrent edit of `_journal.json`, and `0933` has a duplicate
  prefix. Attribute a red journal gate before assuming your own file caused it.
...
- **Read budgets now have their reproducible seed, and the budgets are green
  ...
  transaction: **57 PASS / 0 FAIL / 10 EXCL / 3 SKIP over 70 budgets**, exit 0.
...
- **A scoped approver could see work they could never do.** `leaveApprovalScope`
  ...
  unsatisfiable.
- **A gate that cannot see a declaration form reports zero, not a gap — third
  instance.** `check-command-catalog` matched only `export function useX(`, so
  every arrow-declared hook was invisible: not PERMISSIONED, not SELF, not even
  UNCLASSIFIED. Four real commands were governed by nothing while the count read
  clean — two of them `settings:manage` writes. The first repair was itself
  incomplete, because a hook typed `const useX: () => UseMutationResult<T> = () =>`
  carries `=>` inside its own annotation. The scan floor (≥50 hooks) passed
  throughout. This is the same lesson already recorded for the tenant-FK gate and
  the bulk-`ids` gate, and it has now cost three separate gates: **a gate's
  self-test must feed a fixture in every declaration form the codebase actually
  uses, not only the one the author had in mind.**
- **In a shared working tree, verified work is not safe until it is committed,
  and the loss is asymmetric.** Mid-session another session ran a checkout inside
  `backend/`. Every *tracked* S04 edit was destroyed while the *untracked* new
  files survived, leaving the module uncompilable: new specs referencing symbols
  that no longer existed. Nothing announced it. The failure mode reads like a
  broken session rather than an external event, and the surviving-untracked half
  is what makes it confusing. Sessions should commit verified work promptly by
  explicit pathspec; the orchestrator should not assume an uncommitted tree
  survives to reconciliation.
- **S04 findings owned elsewhere, recorded once here.** S02: `leave_balances`
  ...
  key. S01 again: `frontend/hooks/api/users/bulk-mutations.ts` is corrupted in
  the working tree by an automated `useAuthorizedMutation` sweep — the key is
  spliced into the generic parameters and the identifier repeated — and is the
  only file failing the frontend typecheck.

- **Migration `0933` is authored, journaled and unapplied, and AR-07 depends on it.**
  `0933_ar07_payroll_run_posting_state.sql` adds `payroll_runs.posting_state`. Its
  journal entry sits at `when: 1803000010026` against a live watermark of
  `1803000010025`, so it is correctly ordered and will not be silently skipped —
  it simply has not been run. Until S02 applies it the column does not exist and
  `payroll-db-integration.e2e-spec.ts` fails 14 assertions on
  `column "posting_state" does not exist`. Everything else in that spec was
  repaired: its teardown did `DELETE FROM audit_logs`, which migration `0930`
  forbids at the database boundary, so the suite previously died in cleanup
  before reaching a single assertion. It now detaches audit rows through the
  supported `app.nullify_audit_logs_org_id` function and uses a per-run id prefix.

- **A concurrent session reset the backend repository's tracked files mid-session,
  and the loss was invisible to every green result already recorded.** The backend
  is a separate git repository from the frontend. During S05 an external process
  discarded all tracked-file modifications while leaving new untracked files in
  place, silently reverting ~25 source edits *and* another lane's service fixes,
  minutes after a full suite had reported them green. It was caught only because a
  later pass re-read the source instead of trusting the earlier run. The recovery
  rule that worked: re-apply from an assertion-guarded script where every
  replacement asserts its target text is present, so a silent no-op is impossible,
  then re-run the suite, typecheck, gates and boot proof from scratch. Treat a
  green run in this tree as evidence only for the commit it was measured against.

- **Spec-inclusive typecheck carries 16 errors in four modules, none of them S05.**
  dashboard (11), calendar (2), build (2), chat (1), all constructor-arity drift
  from concurrent sessions. The count moves as those sessions land; S05-owned
  modules are at 0.

- **`check:tenant-isolation-coverage` names three services with no cross-tenant
  negative test:** `accounting-receivables.service.ts` and
  `depreciation-reverse.service.ts` (S05) and `dashboard-project.service.ts` (S04).
  Coverage is otherwise 909/912.
