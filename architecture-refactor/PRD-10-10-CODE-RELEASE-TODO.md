...
- [ ] [S10 — Knowledge, Wiki, Chatbot and AI](code-release-sessions/S10-KNOWLEDGE-WIKI-CHATBOT-AI.md) — 5 of 8 session criteria closed. Backend `modules/(kb|ai|search)` 120 suites / 768 tests green (from 113 / 723), frontend KB/AI 8 / 53 (from 3 / 30) inside a full frontend run of 234 suites / 2,194 tests green, production typecheck exit 0, zero spec-typecheck errors in scope, madge zero in both repos, 23 targeted gates green. The load-bearing finds were a circuit breaker that could never open (its only increment site was a `catch` around a function that returns synchronously), client aborts that never reached the provider on the non-streaming path, unbounded gateway output tokens against reserve-ceiling pricing, an AI response cache whose key carried no reader dimension, AI-drafted email interpolated unescaped into outbound HTML, page-document chunks written with NULL ACL columns that made them permanently invisible to vector search, an unbounded `reindexAll`, per-chunk embedding that opened 500 simultaneous provider calls, a comments panel gated on the update key so read-only members saw nothing, and nine query-key factories with the trailing-`undefined` bug. Open on: page-revision immutability (a retention decision), embedding metering through the gateway, a per-tenant concurrency cap, streaming for the non-chat AI surfaces, and realistic-corpus latency, which needs the seeded database tracked below.
