...
- [ ] **[PRD-C006]** **Frontend speed:** complete v2 ticket 29's production-build Web Vitals, bundle, rendering and interaction budgets while preserving completed lazy-loading, virtualization and hydration gains.
      **OPEN — measured, and the dominant term is now attributed to transit rather than to the frontend.** The capture itself is sound: 68 Core Web Vitals violations and 13/13 route-byte breaches at HEAD with verified build provenance (`WEB-VITALS-CAPTURE-2026-09-04.md`). What is new is the cause. The recorded dominant term is `GET /me/access` at **p50 503 ms**, issued on every authenticated server render. Measured on this machine, a single **empty** tenant transaction costs **447.9 ms p50**, leaving roughly 55 ms for all of that handler's own work — and `AccessService.getAccessSnapshot` opened one unconditionally, even when every input was already cached. Since LCP ≥ FCP ≥ TTFB, that one round trip propagated into all three metrics; the same server returns the public landing page at 9–13 ms TTFB, so none of it was bundle, route composition or render. **Mitigation shipped:** the assembled snapshot is now cached in Redis under a key carrying `(userId, permissionsVersion, isOrgOwner)` and returns **without opening a tenant transaction**, replacing a 448 ms Neon round trip with a ~120 ms Redis read on the hot path. Correctness was verified rather than assumed — the version is the invalidation signal and is in the key; token-attenuated requests skip the cache entirely rather than risk serving a full-session snapshot; `cachedForOrgWith` scopes by tenant via `redisForOrg` + `scopedKey`; TTL is 30 s, no longer than the existing in-process permissions cache; and `MfaGuard` resolves MFA independently through `MFA_POLICY` rather than reading this snapshot, so the cached `mfa` field never gates an action. Three unit tests pin the hit path, version-bump invalidation and the token-scoped bypass. **Still OPEN:** the budgets are not met, the capture has not been re-run since the fix, and even a perfect backend leaves a ~120 ms cache round trip from this machine — so the honest close needs a co-located capture. No budget was changed. Evidence: [LATENCY-FLOOR-2026-09-04.md](final-refactor/evidence/42-production-ops/release-authority/LATENCY-FLOOR-2026-09-04.md).
...
- [ ] **[PRD-C018]** Run disposable-database E2E for Organization/RBAC, Home, Settings, HRMS, Payroll, Build, Billing, Payments, Accounting, Chat, Calendar, Notifications, Knowledge, Workflows and Inbox/mail.
      **OPEN — source coverage is complete; a clean, complete recorded execution is pending.** All fifteen named domains now have seeded specifications, including `test/billing/billing-entitlement-and-seat-isolation.seeded-e2e-spec.ts`. Earlier statements that ten domains lacked specifications are superseded. Historical executions and focused failure repairs are recorded in [CODE-LANES-2026-09-05.md](final-refactor/evidence/42-production-ops/release-authority/CODE-LANES-2026-09-05.md) and [E2E-DISPOSABLE-2026-09-04.md](final-refactor/evidence/42-production-ops/release-authority/E2E-DISPOSABLE-2026-09-04.md); mixed-head runs, skipped coverage and separate focused reruns do not establish release completion. Run the entire in-scope seeded corpus and BOLA cross-tenant sweep against an explicitly identified disposable database at the current migration head, with application-role grants and complete source/prober organization fixtures. Require zero in-scope failures and zero unexpected skips, explicitly exclude CRM/Inventory, and retain raw logs, artifact hashes, fixture counts, database identity and the frontend/backend commit pair. PRD-C158 remains open until all final verification passes at that same commit pair.
...
- [ ] **[PRD-C151]** Record route-level JavaScript, CSS, server payload, image/font and third-party budgets; lazy-load module editors, charts, calendars, chat media and AI interfaces not required for first render.
      ...
      **UPDATED 2026-09-05 — one identifiable owner-chunk found and removed from the pre-load budget; the rest of the breach is still the shared-shell tail.** The largest single first-load script on the worst route was not a Next chunk at all: `/feedbucket-widget.js` at **62,379 B** on `/notifications`, a self-hosted feedback widget mounted from `app/(authenticated)/layout.tsx` and therefore on the critical path of **every** authenticated route. It carried `strategy="afterInteractive"`, which loads after hydration but **before the load event**, so `splitByLoadPhase` charged it to `measuredScriptBytes` rather than to the speculative tail. Changed to `strategy="lazyOnload"` in `components/feedbucket/feedbucket-embed.tsx`; Next's own documentation names "chat support plugins" as this strategy's case, and it is not same-origin-exempt weight — `measuredThirdPartyBytes` is 7,168 on that route because the widget is served from our origin, so no third-party budget ever saw it. Re-measured over the wire against the new production build: **60,459 B** compressed (`curl -H "Accept-Encoding: gzip, br"` against `next start`). Frontend `next build` exit 0, `tsc --noEmit` exit 0.
      **This is NOT recorded as a measured close, and the numbers above are unchanged.** By arithmetic on the 2026-09-04 capture it brings `/mail` (+50,689), `/support/inbox` (+44,465) and `/parties` (+54,107) inside the 524,288-byte ceiling and leaves the other eight in-scope routes over, `/chat` by ~42 KB and `/notifications` by ~247 KB. **Arithmetic is not a capture.** Recording it needs a fresh authenticated `measure:web-vitals` run against the current build, which this session did not perform. One hypothesis was tested and killed: the capture is not measuring uncompressed bytes against a compressed budget — `next start` does gzip static chunks (1,819,635 → 734,771 B on the largest chunk), and the harness reads CDP `Network.loadingFinished.encodedDataLength`, so the recorded figures are genuine compressed weight. The remaining breach has no second owner-chunk: `@excalidraw/excalidraw` (734 KB gzip, the largest chunk in the build) is already behind a `next/dynamic` boundary in `whiteboard-page.tsx`, and `GlobalCreateTicketDialog` is already `dynamic` + closed-by-default.
      **FOUR shell-eviction experiments were run and measured on 2026-09-05. All four were reverted. Do not re-run them.** Against the 301,635 B gzip(9) / 34-chunk `/dashboard` baseline: removing `MotionProvider` from the root layout **−319 B, 34 → 34 chunks** (it imports only `MotionConfig`, a context wrapper); lazifying `product-switcher-menu` **−513 B**; removing all three eager `@animateicons/react` shell imports (`quick-create-button`, `mobile-shell-fab`, `mobile-module-bottom-nav`) **−6,416 B, 34 → 35 chunks**; a coordinated cut **+338 B**. Two earlier analyses contradicted each other on `MotionProvider` and both were wrong — the engine is anchored by `product-switcher-menu.tsx` importing `motion`/`AnimatePresence` **directly** from `framer-motion`, and separately by `@animateicons/react`'s built dist re-exporting `LazyMotion`/`m`/`domMin` from it (its `package.json` lists only React as a peer dep, which is what misled the analysis). **framer-motion never left the shared chunk in any experiment.** Best case is ~7 KB of 301,635 B (2.3%), so the shell breach is structural: the remaining mass is Radix UI 73,681 B and shell navigation 41,571 B, both load-bearing. Closing C151 by cutting is not available; it needs a re-based budget or an architectural change, and that is an owner decision.
      **The "long tail" claim is now MEASURED, not asserted — do not spend another session looking for a second big win.** `node scripts/measure-route-bundles.mjs` at this build: the `/dashboard` shared-shell baseline is **301,634 B gzip(9) across 34 chunks**, and every in-scope route's own additive is small — `/settings` 6,546, `/inbox` 20,956, `/build/inbox` 22,998, `/notifications` 31,894, `/mail` 37,866, `/calendar` 61,575, `/chat` 76,547, `/build/my-work` 82,587. Ranking those 34 baseline chunks individually, the largest is **27,025 B gzip of 301,634 (9.0%)** and the top four are minified app code with no `node_modules` marker (`productionBrowserSourceMaps: false`, so they cannot be attributed further without a sourcemap build). There is no library left to lazy-load out of the shell: the one identifiable big library, `@animateicons/react/lucide`, is already tree-shaken to 147,628 B raw / 14,288 B gzip, and sidebar navigation already carries `prefetch={false}` with intent-gated prefetch pinned by `nav-intent-prefetch.test.tsx`. **Every route is already inside `maxFirstLoadJsBytes` (524,288) — the breach is `measuredScriptBytes` alone**, i.e. what the browser pulls during hydration before the load event, which first-load accounting does not see. Closing the remaining eight routes is sustained distributed work against a measurement loop, not another single fix.
      **That measurement loop does not currently exist on this machine, and that is the real blocker.** `measure:web-vitals` refuses an unauthenticated run and needs `--cookie-file` holding an `authjs.session-token` (minted by `scripts/mint-session-cookie.mjs`), a frontend production server, and a running backend on `:1500`. At this session: the backend was not running, `frontend/.capture-cookie` no longer exists on disk (untracked in `920ab734d`), and **`SCRATCH_DATABASE_URL` is configured nowhere** — `backend/.env` resolves only `neondb` and `cell2`. The 2026-09-04 capture was deliberately scoped to "a scratch-seed user on a disposable database"; reproducing it against `neondb` instead would drive 11 routes x 6 repeats x 2 profiles of authenticated navigation through the live database, which is a different risk decision and was not taken unilaterally. **Restoring a disposable capture database is the prerequisite for any further PRD-C151 or PRD-C149 movement.**
      Owner: the repository owner. Recorded in the release record; not waived.
