...
- [x] [S03 — API, query, pagination, caching and contracts](code-release-sessions/S03-API-QUERY-CACHE-CONTRACTS.md) — backend `8cc46225` · `846827d7` · `5e785f03`; 25/25 S03 gates green, 112 suites / 1,202 tests green, read budgets 55 PASS / 0 FAIL on `scratch_e2e` as `streamline_app`.
...
- **S10 closed with twelve named open items, and two of its finds argue that a
  ...
  `build/core/projects-work-query.service.ts` (S06),
  `chat/chat-messages.service.ts` (S08) and `cron/cron-hr.controller.ts` (S04),
  and `check:module-di` red on `OutboxPublisherService` needing
  `OutboxReportService` declared in `ArchitectureEvidenceModule` (S03/S12).
  Both gates were green at S10's baseline and went red during the session from
