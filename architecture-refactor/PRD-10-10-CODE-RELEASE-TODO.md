...
- **Re-run tenant-relationship integrity against an explicitly authorized disposable current-head database.** The latest attempt was inconclusive because the gate correctly refused an unapproved remote Neon database. Acceptance: every tenant-owned relationship has its canonical composite constraint, zero actionable redundant constraints remain, and the artifact records database identity, SHA and catalog hash. **Accountable criterion: PRD-C003, PRD-C001.**
- **Replay the current migration chain on a blank disposable database.** The ledger currently reports 697/697 with zero pending, but `check:replay-ledger` is inconclusive without `COLD_DATABASE_URL`. Acceptance: blank replay, interrupted/resumed replay and catalog comparison pass at final head with exact ledger/catalog evidence. **Accountable criterion: PRD-C001.**
