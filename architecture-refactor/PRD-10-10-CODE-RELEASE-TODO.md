...
Status: historical requirements and evidence ledger as of 2026-09-09. Active work is assigned in [`prd/README.md`](prd/README.md).
...
This file preserves the complete numbered requirements vocabulary and historical evidence. It is not the current task list: unchecked boxes and embedded `CLOSED` prose have drifted and must not be used to assign work. The module files routed by [`prd/README.md`](prd/README.md) are the sole active backlog. A task closes only after its module completion evidence is recorded. Code-level and deployed production readiness remain separate.
...
- Current task ownership: [module backlog](prd/README.md)
...
## Session record - 2026-09-09 (post-audit remediation)

Closing the 2026-09-09 audit blockers. Every gate below was run at root `759e1e927` / backend `8a6737df2` and its exit code read directly, not inferred from a pipeline.

**Now green, having failed at the audit:** `check:unbounded-reads`, `check:db-call-count`, `check:cache-invalidation`, `check:vacuous-assertions`, `check:type-assertions`, `check:file-sizes`, `check:contract-registry`, `check:gated-reads` (frontend), `check:migration-ledger`, plus `check:migration-discipline` and `check:migration-rollback` which this work turned red and then fixed. All twelve self-tests pass.

**Now green, having been INCONCLUSIVE (exit 2) at the audit:** `check:replay-ledger` (708/708 CLEAN) and `check-tenant-relationships` (0 actionable). Both were inconclusive for want of an authorized target, not for want of correctness.

**Now run, having been skipped at the audit:** dependency cycles - backend 0 over 6,472 files, frontend 0 over 5,929. The audit refused these because the scripts shelled to `npx --yes madge@8`, fetching the analyser from the network; madge 8 is now a devDependency in both repos and the invocations use the local binary.

**One-commit pair:** backend typecheck (build + test configs) 0, frontend typecheck 0, `nest build` 0, `next build` 0. Focused suites 169 files / 1,096 tests pass.

**Authentication P0 fixed.** `jwt-auth.guard.ts` treated a Redis tombstone MISS as "not revoked" without consulting `user_sessions.is_revoked`, and resolved a Redis+database double failure as live. Both now fail closed. The spec that asserted the old behaviour was rewritten, and each new negative was observed failing against the old guard before being restored.

**PRD-C149 / Web Vitals: NOT closed, and the blocker moved.** Two real defects were found and fixed in the measurement path: a default `next build` here inlines `NEXT_PUBLIC_API_URL=https://api.streamlineos.in` from `.env.production.local` into 6 chunks, so a local capture measured a browser calling PRODUCTION and every authenticated route 307d to `/signin?session=expired`; and `CORS_ORIGINS` omits `http://127.0.0.1:1000`, so serving on that spelling rendered an error boundary. With both fixed, authenticated routes return 200 and render. The capture is still REFUSED by its own driver for two reasons: desktop samples fall under the 10-word content floor, and the host measured 77% busy before launch and 90% median during, against a 50%/75% ceiling - contention from processes outside this work. **Mobile INP did reproduce locally at 728 ms and 1,152 ms against the 200 ms budget**, which is the first such reading taken against a local API rather than production, so the standing INP breach is corroborated rather than explained away. No Web Vitals evidence is claimed.

## Current 10/10 delta checklist — reverified 2026-09-07
...
The 2026-09-09 current-source audit reopens PRD-C128 and PRD-C129 because their relevant executable tests were not rerun at the current root/backend pair. PRD-C017 remains pending until ticket state is synchronized with the restored unchecked criteria and the traceability gate is rerun. All numbered criteria remain pending until equivalent current-source proof exists. Historical percentages and old checkmarks are not completion evidence.

### A. Query bounds, pagination and database-call cost

- ~~**Eliminate all actionable unbounded reads.**~~ **CLOSED 2026-09-09.** `check:unbounded-reads` reports **0 actionable, 0 unclassified**; suppression 642 against a ceiling of 649. The two unclassified paths were `build/core/build-ticket-mutation-policy.ts:25` and three selects in `kb/retrieval/kb-citation-visibility.service.ts`. Both are exact-id `inArray` reads, not scans, and every producer caps the id list: bulk mutation rejects outside 1..100, rank passes at most 3, ticket update passes 1, KB ask retrieves MAX_CONTEXT_ARTICLES=6 plus 4 sources, a research brief (PLAN_MAX_QUESTIONS=5)+1 queries x5 and x3 deduplicated. Pinned by `build-ticket-mutation-bounds.spec.ts` and `kb-citation-visibility-bounds.spec.ts`, each bite-proved by widening the cap and observing failure. **Accountable criterion: PRD-C005.**
- ~~**Eliminate loop-internal database calls and N+1 service-call loops.**~~ **CLOSED 2026-09-09.** `check:db-call-count` reports **ACTIONABLE: 0 files, 0 call sites** (4 files/7 sites at the 09-09 audit; 51/32 when this row was written). The last in-scope N+1 was org archive/purge opening one `withIdentity` transaction per member, unbatchable because policy `tenant_isolation` on `organization_members` admits one principal at a time. Migration 1081 adds `app.next_active_org_ids`, a SECURITY DEFINER resolver, so the whole departing cohort is one statement. The two surviving `/party/` sites are EXCLUDED-MODULE **on caller evidence** - every caller (contacts.service, leads-import.service, leads.service, public/crm.service) is CRM, which this release excludes; `/party/` was deliberately NOT added to EXCLUDED_MODULE_PREFIXES because party is a shared seam in-scope code also uses. **Accountable criterion: PRD-C005.**
...
- **Meet the current performance budgets.** Require LCP <= 2.5 s, INP <= 200 ms and CLS <= 0.1 for every in-scope route; record raw samples, p75/p95 where applicable, frontend SHA, BUILD_ID, device/viewport, CPU contention measurement and commands. Historical Inbox, Build Inbox, Parties, Dashboard and Support Inbox failures must be remeasured rather than assumed fixed or still failing. **Accountable criterion: PRD-C006, PRD-C156.**
- **Close PRD-C149, then its roll-ups PRD-C006 and PRD-C156.** Closure requires current reproducible performance evidence, not a stale capture or a raised budget. **Accountable criterion: PRD-C149.**

### E. Tenant, migration and final release proof

- **Execute tenant-isolation tests, not only declaration coverage.** Static coverage is 943/943 services, but that proves test presence only. Acceptance: the executable cross-tenant negative suite passes at the final backend commit for reads, writes, caches, jobs, exports, search/vector retrieval and realtime delivery. **Accountable criterion: PRD-C003.**
- ~~**Re-run tenant-relationship integrity against an explicitly authorized disposable current-head database.**~~ **CLOSED 2026-09-09.** The gate was INCONCLUSIVE (exit 2) because it correctly refused an unapproved remote Neon target. Re-run against the loopback database built by the cold replay: `check-tenant-relationships` reports **0 actionable** single-column tenant FKs over 214 total (79 CRM, 134 Inventory, 1 platform-global excluded), with the ledger at 708/708 on the measured target. **Accountable criterion: PRD-C003, PRD-C001.**
- ~~**Replay the current migration chain on a blank disposable database.**~~ **CLOSED 2026-09-09.** `replay-chain-cold.mjs` replayed all 708 journal entries into a blank PostgreSQL 18.6 database (`scratch_cold_20260909`): 707 applied, 943 tables, 46s, and one failure - migration 1081, authored the same day, declared `next_org_id uuid` where the schema is `text`. No static gate could see it; SQL is a string until a server parses it. After the fix, re-running **resumed** from `drizzle.__replay` (707 skipped, 1 applied), exercising the interrupted/resumed path. `check:replay-ledger` reports **LEDGER STATE: CLEAN** - 708/708 applied, 0 orphans, 0 duplicates. Live Neon ledger separately reconciled from 733 rows/34 orphans/2 duplicates/11 pending to **708/708, 0 pending**, watermark unchanged during the row deletion. **Accountable criterion: PRD-C001.**
- **Pass the entire code-release suite at one clean frontend/backend commit pair.** Include builds, typechecks, focused tests, disposable E2E, architecture gates, tenant execution, migration replay, current Web Vitals and the recorded Build scope/isolation scenario. No skipped, interrupted, stale, prerequisite-blocked or separately sampled result counts. Record both SHAs, commands, exit codes and artifact hashes; then close PRD-C158 and the code half of PRD-C016. **Accountable criterion: PRD-C016, PRD-C158.**

### F. PRD and evidence integrity

- **Remove stale status prose without deleting requirements.** **PARTIALLY CLOSED 2026-09-09.** The PRD-C156 half of this row was ITSELF stale: the narrative no longer describes PRD-C104 as red at 76/29 - it already reads "PRD-C104 is closed" and records the 76 -> 20 suppression drop with the ratchet lowered 29 -> 20. Verified by reading, no edit required. The remaining half is unchanged: reconcile the documents that each claim to be the single authoritative backlog so this file is the sole code-release checklist. **Accountable criterion: PRD-C017, PRD-C156.**
...
### G. Deferred deployed and human completion

- **Complete all 26 DEPLOYED criteria already listed under PRD-C010, PRD-C016, PRD-C162–PRD-C187 and PRD-C189–PRD-C195 as classified by the deferred-checkbox ledger.** This includes private storage/backfill, provider sandboxes, TLS/encryption/secrets, WAF, deployed Web Vitals, realistic load/headroom, isolated cells, replica/PITR, cost, observability, live alerts, deployment/recovery drills, privacy drills and final deployed release evidence. Do not close these from local mocks. **Accountable criterion: PRD-C010, PRD-C016.**
- **Complete all nine remaining HUMAN criteria.** Record named Product, Security, Privacy/DPO, Operations, Legal and Finance decisions/approvals, operator and break-glass policy, data-catalogue ownership, AI/provider policy, accepted residual risks and release authority. Automation may prepare records but may not sign for a human. **Accountable criterion: PRD-C016.**
- **Issue the final two-level verdict.** Code architecture may be certified 10/10 only after sections A–F and all immediate CODE criteria pass at one commit pair. Production readiness may be certified 10/10 only after section G and every deferred production gate also pass. CRM and Inventory must remain explicitly excluded from both claims. **Accountable criterion: PRD-C016.**
...
- [ ] **[PRD-C133]** Architecture/schema: verify spaces, memberships, documents/pages, immutable revisions, attachments, ingestion jobs, chunks/embeddings and deletion/reindex state have tenant-composite integrity.
      **CLOSED 2026-09-09 — measured on the co-located scratch stack (backend `8a6737df2`, `scratch_local` PG18.6, journal 708/708).** `db:verify-rls` → RLS VERIFIED (exit 0); `check-tenant-relationships` → zero actionable single-column tenant FKs (exit 0); live savepoint bite on `kb_page_versions` → substance UPDATE / DELETE / TRUNCATE all 42501, author redaction allowed. All eight entity classes carry non-null `org_id` + `(org_id,id)` anchor + composite child FKs. Carried design gaps (no reindex-state table; polymorphic `kb_ingestion_checkpoints`; RLS declared only in migrations) recorded, not defects. Evidence: [KB-DOCUMENTS-CERTIFICATION-2026-09-09.md](final-refactor/evidence/42-production-ops/release-authority/KB-DOCUMENTS-CERTIFICATION-2026-09-09.md).
- [ ] **[PRD-C134]** Queries/cache/workers: verify revision/search plans, ingestion leases/retries/DLQ, chunk dedupe, permission-aware cache keys, purge/reindex and realistic-corpus latency.
      **CLOSED 2026-09-09 — measured.** KB `*.db.spec.ts` 7 suites / 26 tests pass; KB backend units 122 suites / 895 tests pass (leases/heartbeat/fail-closed, outbox retry/DLQ DEAD@8, content-hash + delete-then-insert dedupe, ACL cache key, five-store purge, retrieval strategy). Realistic-corpus latency + vector recall referenced from [KB-RETRIEVAL-LATENCY-HARNESS.md](final-refactor/evidence/45-scale/KB-RETRIEVAL-LATENCY-HARNESS.md) / [KB-RETRIEVAL-RECALL-FLOOR.md](final-refactor/evidence/45-scale/KB-RETRIEVAL-RECALL-FLOOR.md); recall floor met ≤ 10,000-chunk exact-scan threshold, documented accepted gap above it. One stale-test-wiring regression fixed. Evidence: [KB-DOCUMENTS-CERTIFICATION-2026-09-09.md](final-refactor/evidence/42-production-ops/release-authority/KB-DOCUMENTS-CERTIFICATION-2026-09-09.md).
- [ ] **[PRD-C135]** Frontend/TanStack/tests: verify editor/revision conflicts, search cursors, permission changes, citations/source integrity, ingestion states and ACL/purge/reindex E2E.
      **CLOSED 2026-09-09 — measured.** KB frontend units 18 suites / 135 tests pass (autosave 409-latch revision conflict, versions/sources cursor pagination, ACL-versioned key discrimination + `useCan` gating, citation contracts, ingestion-state polling); KB seeded-e2e 3 suites / 8 tests pass (`kb-acl-purge-reindex`, `kb-page-visibility`, `kb-stuck-source-reaper-isolation`) via the DB-pinned helper against `scratch_local`. Named frontend-unit coverage gaps (citation render, `SourceStatusBadge` render, purge/reindex hooks, mid-session permission-change invalidation) recorded; the paths are covered end-to-end by the seeded E2E. Evidence: [KB-DOCUMENTS-CERTIFICATION-2026-09-09.md](final-refactor/evidence/42-production-ops/release-authority/KB-DOCUMENTS-CERTIFICATION-2026-09-09.md).
...
- [ ] **[PRD-C160]** No unresolved code-level P0/P1 finding remains.
      **CLOSED 2026-09-04 — measured. The 09-04 closure was WRONG, and is re-closed 2026-09-07.** The 09-04 text below stands as written, but it was not true when it was written: a code-level P0 was live and already measured failing. `decisions/operator-access-2026-09-03.md:182` records drill `C181-5c-self-approval-beneficiary` failing the day before (9/10, exit 1) — `approveGrant` compared `approverId` only against `grantedBy`, so whenever a third party filed the request, the operator named in `operatorUserId` could approve their own break-glass grant. Two-person approval held only when the operator asked for themselves.
      It survived this criterion because the only PRD lines naming no-self-approval are C180 and `PRD-10-10-TODO.md:132`, both classed as deployment-time **approvals**. A code defect carried as a policy signature is invisible to a code-level gate, which is exactly the failure mode this criterion exists to catch. Fixed 2026-09-07 (`platform-operator-access.service.ts`, four tests including a bite proof that the `grantedBy` rule alone misses it); the append-only trigger on `operator_access_log` (D2, same decision record) remains OPEN and is tracked under C180.
      *Original 09-04 text:* No unresolved code-level P0/P1 remains. The last one, chat mention delivery, is resolved by two consecutive passing runs of the live probe (exit 0) rather than by inspection; the previously recorded failure text was stale and predated the fix. Every other defect found during this closure pass was fixed in-session, not deferred — see PRD-C021 for the enumerated list.
