# Lane 2 — requests both ways

**Outbound (Lane 2 -> orchestrator)** is sections 1-3. **Inbound (-> Lane 2)** is at the end.
Nothing in the outbound sections was edited by Lane 2.

> Restored 2026-08-26: the outbound sections were overwritten when this file was reused for
> inbound requests. Both directions now live here - append rather than replace.
...
authoritative list and reproduces it on demand. **Snapshot, 2026-08-26 — re-run it. The totals drift
as other lanes add routes; only the UNDECLARED set below has held steady:**

| | count |
|---|---|
| total handlers | 3,517 |
| public | 202 |
| permissioned | 3,174 |
| in-service | 33 |
| universal | 1 |
...
**Lane 2, verified at source and accepted as a c25 finding.** Confirmed: `analytics` at
`leaves.service.ts:211-221` refuses only `none`, then calls `queryAnalytics(u.orgId, year)` whose
signature at `:223` is `(orgId: string, year: number)` — no scope parameter exists to thread. Your
reading is right, including that the cache is innocent. Recorded in
[`c25-02`](../c25-authorization-cannot-be-omitted/issues/02-object-access-and-scope-share-one-query-seam.md)
as a confirmed live instance of that ticket's defect, **not fixed** — the fix is to give
`queryAnalytics` a scope parameter and apply `applyScope(scope, u.userId, cols)` as a SQL predicate
rather than filtering after aggregation. Handing it back to Lane 4.
