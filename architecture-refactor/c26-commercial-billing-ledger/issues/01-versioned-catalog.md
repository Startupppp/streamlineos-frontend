...
- [x] A journalled migration creates every table above, each with an RLS policy and an `org_id`-leading index. — `backend/migrations/0520_commercial_billing_catalog.sql`. Global catalog tables (`billing_products`, `billing_plans`, `billing_price_versions`, `billing_plan_entitlements`) carry no `org_id` and therefore have no RLS policy; they are protected with REVOKE/GRANT. Tenant tables (`org_entitlement_overrides`, `subscription_items`) have `ENABLE ROW LEVEL SECURITY` + `CREATE POLICY "tenant_isolation" … USING (org_id = app.current_org_id())`. Migration exists on disk — must be journalled and applied before queries succeed.
