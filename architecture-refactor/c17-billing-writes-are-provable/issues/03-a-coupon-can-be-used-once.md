...
- [x] Redemptions are recorded so campaign performance is measurable. — every redemption writes `couponId`, `orgId`, `userId` and `redeemedAt`; `BillingCoupons.list` (`billing-coupons.ts`) returns coupons `with: { redemptions: true }` alongside `usedCount`. `coupon_redemptions.amount` is still null — it cannot be computed until `verifyPaymentSchema` carries `billingCycle`; see `lane-requests/s1.md` §2.
