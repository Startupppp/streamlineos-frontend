...
- **`check:module-di` rebuilt** into a real constructor-resolvability gate (undeclared token · non-injectable type · `import type` on an injected class). Its first resolver guessed a filename from the class name and silently skipped 76 classes whose files do not follow that convention; it now indexes every `export class` in `src/`. **1,626 classes checked, 0 skipped, 0 violations.** Bite-proven end to end on `cost-visibility.ts` — a file only the new resolver reaches.
- **The full suite is 1,367 suites / 11,674 tests**, and it was red — 20 suites / 66 tests. Earlier sessions reported green from path-filtered runs. **All 20 repaired**; the last five (accounting · crm · build · inventory · e-sign) were stale specs and incomplete mocks, not source defects, each bite-proven, with test counts unchanged so nothing was dropped to pass. Full-suite re-run is pending the running lanes.
