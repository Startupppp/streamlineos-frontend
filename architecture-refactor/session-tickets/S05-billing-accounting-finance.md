...
- [ ] Then **measure** the sweep in buffers as `streamline_app` with the tenant GUC set, and record before/after. A covering index on an RLS table must contain `org_id` — this one does. PENDING: requires live Neon DB with tenant GUC access; index contains `org_id` as required. (blocked on live DB; not part of S05b scope)
...
- [ ] Migrate finance tax payments, reminder policies and every other growing list to the shared cursor contract, cap 100, explicit `null` cursor on exhaustion. Remove legacy offset branches and migrate every caller. PENDING (blocked on S09 frontend hooks update — frontend still sends page/pageSize).
- [ ] Replace broad raw projections with explicit DTO projections. PENDING: spot-check found no `select *` on users; `credit-notes.service.ts` uses `select()` (select *) on `creditNotes` table; full audit of all finance/billing services remains.
