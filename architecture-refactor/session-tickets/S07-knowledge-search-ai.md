...
- [ ] Vector queries always hit an ANN (HNSW) index. NOT MEASURED: `idx_kb_chunks_embedding_hnsw` exists (`vector_cosine_ops`, migration 0016, RLS enabled with `tenant_isolation` policy). Dev DB has 0 rows — EXPLAIN ANALYZE is vacuous. Script at `backend/scripts/measure-hnsw-plan.mjs` ready; re-run when live data is present. Risk: index covers only `embedding` (not `org_id`), so under post-filter ANN semantics, high-org-count tables may see bleed at the limit boundary. S07c-report.
