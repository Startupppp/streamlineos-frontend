...
- [ ] Ingestion is asynchronous, resumable, deduplicated, observable and malware-scanned. Never re-embed unchanged content — hash the **source text**, not the rejoined chunks. PARTIAL: async via outbox ✓; dedup via `sha256(sourceText)` content hash ✓; observable via structured Logger signals ✓ (S07d: start/progress/completion/failure in consumer and indexing service — errors rethrown, never swallowed); hash is on source text not rejoined chunks ✓; chunk-level resumption DONE (migration 0701, `KbIngestionCheckpointService`, `embedWithResumption`). NOT DONE: malware scan — no scanner in environment; requires ClamAV sidecar, cloud scan API, or S3-Lambda quarantine pipeline gating BEFORE the attachment row commits (see S07c-report, S07d-report). S07c-report; S07d-report.
- [x] Vector queries always hit an ANN (HNSW) index. MEASURED 2026-08-30 on 30,000 clustered-vector chunks across 3 orgs as `streamline_app` with the tenant GUC. **Correctness guaranteed by migration 0703 (MATERIALIZED fence); clustered re-measurement settles the iterative-scan question.**
  - The correctness bug (0 rows for minority tenant on random-vector fixture): FIXED by migration 0703 (`app.search_kb_chunk_ids` SECURITY DEFINER + MATERIALIZED fence). Org-b returns 20 rows.
  - Re-measured 2026-08-30 with semantically clustered vectors (each org near its own random unit centroid, σ=0.05, query = avg(org-b embeddings)): **plain ANN returns 20 rows at 258 buffers; iterative scan returns 20 rows at 227 buffers; MATERIALIZED fence returns 20 rows at 22,688 buffers.** With real semantic embeddings the HNSW graph navigates correctly into the querying org's cluster — both (a) and (c) work. The negative result in HNSW3 was specific to the random-vector fixture (near-orthogonal, no clusters).
  - Iterative scan cannot be set in a function SET clause on Neon (42501) — only session-level SET LOCAL works — and the outer-query approach still fails on the random-vector case. The fence is the only correctness guarantee independent of embedding clustering quality; it stays.
  - Performance: fence costs 22,688 buffers vs 258 for plain ANN on a 7,500-row tenant. Acceptable at current scale; if tenant chunks exceed ~500k, partition `kb_article_chunks` by org_id for per-partition HNSW indexes. S07d-report.
