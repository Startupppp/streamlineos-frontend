...
- [ ] Ingestion is asynchronous, resumable, deduplicated, observable and malware-scanned. Never re-embed unchanged content — hash the **source text**, not the rejoined chunks. PARTIAL: async via outbox ✓; dedup via `sha256(sourceText)` content hash ✓; observable via Logger ✓; hash is on source text not rejoined chunks ✓; chunk-level resumption DONE (migration 0701, `KbIngestionCheckpointService`, `embedWithResumption`). NOT DONE: malware scan — no scanner in environment; requires ClamAV/cloud API/S3-Lambda pipeline (see S07c-report). S07c-report.
- [ ] Vector queries always hit an ANN (HNSW) index. MEASURED 2026-08-30 on 30,000 chunks across 3 orgs as `streamline_app` with the tenant GUC. **The correctness bug is FIXED but this row stays OPEN because the fix abandons the index.**
  - The bug was real: RLS post-filters ANN candidates, so a minority tenant holding 7,500 chunks returned **0 rows** for a LIMIT 20 query — 393 candidates traversed, all from the dominant tenant, all discarded. Under-retrieval, not leakage; the endpoint returned 200 throughout.
  - Fix shipped (migration `0703`, `app.search_kb_chunk_ids`): a SECURITY DEFINER function with a MATERIALIZED fence. Correct — org-b now returns 20 — but it sequentially scans the tenant's rows and sorts: **22,680 buffers vs 247**, and the cost grows LINEARLY with tenant size. That is why this is not ticked.
  - pgvector 0.8.6 `hnsw.iterative_scan` was tried and did NOT help (0 rows, 786 buffers). CAVEAT ON THAT NEGATIVE: the fixture is RANDOM 1,536-dim vectors, which are near-orthogonal and uniformly spread, so nearest-neighbour order is essentially arbitrary and follows row count (the dominant org holds 15,000 of 30,000). Real embeddings cluster semantically and a tenant's query lands near its own content, so iterative scan may well work in production. **Do not treat this negative as settled** — re-measure against real embeddings before ruling it out.
  - Neon platform limit found on the way: setting `hnsw.iterative_scan` in a function `SET` clause fails `42501` even as `neondb_owner`; only session-level `SET LOCAL` works. Same class of restriction as `ALTER FUNCTION … LEAKPROOF`.
  - Production options if the fence proves too slow: partition `kb_article_chunks` by org with per-partition HNSW indexes, or per-org partial indexes.
