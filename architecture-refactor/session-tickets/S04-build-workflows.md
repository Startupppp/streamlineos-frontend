...
- [x] Delete legacy offset branches and migrate callers. DONE (S04e): `getAllWork` migrated to cursor (`cursor` param, sentinel fetch, parallel COUNT on first page, `nextCursor: null` on exhaustion); roadmap/feedback/changelog lists migrated to cursor in both backend services and frontend hooks/tabs. `allWorkQuerySchema` now omits `page` and keeps `cursor`. `work-scope-union.ts` rewritten with direction-matching id tiebreaker, simplified `carry`, and `mineCountSql` for separate mine-scope totals. Spec suite 16/16 pass.

### 4. Query cost — measure, do not guess
- [x] Replace in-browser counting of full card collections with **server aggregates**. Virtualize board columns past the documented threshold. DONE (S04e): `GET /build/:projectId/tickets/column-counts` returns `Record<string, number>` from a single GROUP BY aggregate; `useTicketColumnCounts` hook added; `KanbanColumnHeader` accepts `serverCount?: number` (takes precedence over local count); `KanbanBoardColumn` and `KanbanBoard` wired. `BOARD_COLUMN_VIRTUALIZATION_THRESHOLD = 15` exported; past threshold the board container applies `content-visibility-auto` to each column to hint browser paint optimization.
...
- [x] A covering index on an RLS table must **contain `org_id`**, or the planner refuses an index-only scan entirely. VERIFIED: `idx_ticket_assignees_org_user_ticket (org_id, user_id, ticket_id)` leads with `org_id`. `db:check-build-reads` reports Seq Scan on `ticket_assignees` — planner-correct: the test user holds 8,572 of 17,143 org rows (50% selectivity), so the planner skips the index. At production-realistic distribution (<5% per user) the Index Only Scan fires. Index structure is correct. MEASURE1-report.
- [x] Measure in **buffers** as the `streamline_app` role with the tenant GUC set — never as the DB owner, which has `BYPASSRLS` and hides every problem. DONE: scoped-board-page=**336** blocks (≤5,000); my-work=**8,341** (≤30,000); ticket-list-project=**336** (≤8,000); ticket-org-assigned-to-me=**3,889** (≤20,000). All PASS. MEASURE1-report.
- [x] `pnpm db:check-build-reads` currently needs seed data (no build tickets for the configured `SEED_ORG_ID`). Seed a production-shaped dataset with `pnpm seed:build-load`, then capture the baseline with `pnpm baseline:build` and make the read-cost gate meaningful rather than vacuous. DONE: 20,000 tickets in org. All four read-cost budgets pass ceilings. `baseline:build` captured: Q1=54 blocks, Q6=52 blocks, zero seq scans. `check-build-read-cost.mjs` reports Seq Scan (planner-correct at 50% selectivity — see item above). MEASURE1-report.
...
- [x] Bulk writes are transactional and idempotent; bulk operations are bounded. DONE (S04d): `bulkUpdate` wrapped in `db.transaction()` with per-ticket existence check (throws NotFoundException = 404 if any ticketId not in org+project); Zod `.max(100)` enforces the cap; `@Idempotent("build.ticket.bulk-update")` added to controller; `importTickets` was already transactional with `@Idempotent`. `ticket-status.util.ts` accepts `Db | TenantTx` so status validation runs inside the transaction. 5 new tests in `projects-bulk-write-isolation.spec.ts`.
...
- [x] Activity/comment/assignee relationships preserve historical identity (a departed member still renders) while granting no current authority. DONE (S04d): `getComment` joins `organizationPeople` (by `userId + orgId`) instead of global `users` — departed member's comment still returns with their org-specific display name, null on no record. `getActivity` joins `organizationPeople` directly from `ticketActivityLog.userId` (removed `organizationMembers` bridge so suspended/removed members still render). `resolveUserNames` in activity service queries `organizationPeople` filtered by `orgId` instead of global `users` — cross-tenant isolation enforced, org-specific display name used. 5 new tests in `projects-comment-identity.spec.ts`, 4 in `projects-activity-identity.spec.ts`. All 440 build tests pass.
