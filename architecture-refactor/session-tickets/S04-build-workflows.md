...
- [x] `projectMembers` has no soft-delete (presence = active membership); org_id is included in all project membership queries. CAVEAT: `checkProjectAccess` direct-member path does not verify `organizationMembers.status=ACTIVE` — a suspended org member who is still in `projectMembers` can pass the direct check. `resolveUserPermissions` DOES check ACTIVE status (access.service.ts:395,521,548), so the permissions path is safe; only the direct project-member path is unguarded. Documented but not fixed — requires joining projectMembers with organizationMembers on every access check.
