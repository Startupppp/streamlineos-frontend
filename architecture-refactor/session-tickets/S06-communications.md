...
- [x] Complete membership-keyed actor migration for channels, participants, messages, reactions, mentions, reads and invites. DONE S06e: 12 chat user_id columns audited. 10 had membership columns already expanded (0660) and backfilled. 2 outstanding NOT VALID FKs (`fk_chat_channels_org_creator_membership`, `fk_chat_messages_org_sender_membership`) VALIDATED in 0712. `chat_user_presence.membership_id` backfilled + FK + VALIDATE in 0713. Service write paths updated in chat-presence, chat-huddles, chat-pins, chat-saved, chat-reply-reminders. S06e.md.
- [x] Backfill in resumable batches with duplicate/unmappable-row reporting **before** cutover; produce zero-use proof before dropping any legacy column. DONE S06e: 0660 backfilled 10 columns with unmappable reporting to `communication_backfill_issues`. 0713 backfills `chat_user_presence.membership_id` with same reporting pattern. Zero-use proof: NOT possible for 10 remaining legacy columns — all have active read paths in services; drops deferred to full read-path cutover session. S06e.md.
...
- [x] Protect administrative descendants of `/chat` and `/calendar` with exact permissions at both the route and hook level. DONE S06d: backend controllers already correct (class-level `@UseGuards(JwtAuthGuard, PermissionGuard)` + per-handler `@RequirePermission`). Frontend: `useKickParticipant` converted to `useAuthorizedMutation("chat:huddles:moderate")`; `/chat/settings` → `chat:org-settings:manage`, `/chat/moderation` → `chat:huddles:moderate`, `/calendar/settings` → `calendar:admin:manage` in `route-access-extensions.ts`. `calendar:admin:manage` confirmed in both catalogs (backend + frontend). Universal `/calendar` is exact-match only so `/calendar/settings` correctly falls through to extension registry. S06d-report.
...
### 6. Mail
- [x] Provider sync is checkpointed, resumable and idempotent; provider adapters normalize external ids. DONE S06d: `mail_sync_checkpoints` table created (Drizzle schema + migration 0704, journalled idx=532, when=1798000032000). `MailSyncCheckpointService` with `loadPosition` / `savePosition` (own `runInNewTenantTransaction`) / `clearPositions(tx)`. `mail.service.ts` fires `void this.checkpoints.savePosition(...)` fire-and-forget after each successful metadata upsert; loads checkpoint at fetch start. S06d-report.
...
- [x] Cover every uncovered service in your trees. DONE S06d: `ChatOrgSettingsService` (2 tests) added to `chat-services-tenant-isolation.spec.ts`; `MailSyncCheckpointService` (4 tests) in new `mail-sync-checkpoint-isolation.spec.ts`. All 14 services in chat/mail/calendar trees now have DENY+CONTROL isolation tests. 7 suites, 32 tests, all PASS. `resetAllMocks` used; `db.transaction` mock invokes callback via `runInNewTenantTransaction` mock. S06d-report.
