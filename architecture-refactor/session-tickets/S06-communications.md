...
- [ ] Preserve historical departed-member display without granting current channel access. OPEN: `assertMember` in huddle service checks channel membership by userId only (not active membership status); departed-member display names are not cached separately. Out of scope this session.
...
- [ ] Prove channel/thread BOLA, private-channel membership, cross-org denial, reconnect replay and duplicate-event behaviour. OPEN: no e2e specs found for these four cases; requires new test file in S06 exclusive tree.
