...
- [x] Cap and export large payroll datasets **asynchronously** behind an authorized expiring download that re-asserts object-level access and returns 404 (never 403) for another org's job id. DONE (S03c): `payroll_run_export_jobs` table + `PayrollRunExportService` (BOLA-safe `find` — orgId+membershipId in WHERE, NotFoundException never 403; `download` re-asserts at fetch time; 24h `expiresAt`; idempotent insert via `onConflictDoNothing`) + `PayrollRunExportWorkerService` + `PayrollExportController` (`@RequirePermission("payroll:reports:export")` + `@UseGuards(JwtAuthGuard, ModuleGuard, PermissionGuard)`). Migration 0679 journaled. Proof: `payroll-export-cross-tenant.spec.ts` — 7 tests, all pass.
...
- [x] Remove legacy offset branches in the same pass and migrate every caller (in-place removal is authorized; no external consumers). DONE (S03c): `listRuns` and `listRunEmployees` in `runs.service.ts` now use `buildCursorPage`/`decodeCursor`; `listRunsQuerySchema` and `listRunEmployeesQuerySchema` use `cursor`+`limit` (no `page`/`offset`); frontend hooks `usePayrollRuns`/`useRunEmployees` updated; `runs-page-content.tsx` and `employees-tab.tsx` migrated to cursor-based footer navigation; `prefetchPayrollRuns` updated; `hydration-contract.test.ts` and `runs-page-content.test.tsx` updated. `nextCursor` is explicit `null` (not `undefined`) when exhausted — proved by `cursor-pagination.spec.ts` (5 tests). 776 backend tests pass.
