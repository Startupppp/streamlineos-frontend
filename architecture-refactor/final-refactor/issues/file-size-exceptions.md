...
| `src/modules/organization/core/organization.controller.ts` | 536 | Cohesive controller | Organization | `OrganizationController` (31 public methods: validateInvitationToken, listOrganizations, listArchivedOrganizations, createOrganization, switchOrg, listMembers, updateMemberRole, removeMember, suspendMember, reactivateMember, getSettings, updateSettings, updateSecuritySettings, acceptInvitation, declineInvitation, listCustomDomains, addCustomDomain, verifyCustomDomain, removeCustomDomain, listHolidays, createHoliday, deleteHoliday, archiveOrg, restoreOrg, leaveOrg, deleteOrg, schedulePurge, cancelPurge, placeLegalHold, listLegalHolds, releaseLegalHold) | All organization lifecycle routes in one controller: membership, settings, security, invitations, custom domains, holidays, archive/restore, legal holds and purge scheduling. Two path-keyed specs prevent splitting: `organization-creation-policy.spec.ts` instantiates `OrganizationController` with all 13 constructor services and directly calls `createOrganization`/`switchOrg`, and `bola-org-purge-path-404.spec.ts` reads this file by path to assert `assertPathOrgIsCallerOrg` is present in `schedulePurge`/`cancelPurge`. Moving any handler to a sibling controller silently breaks both specs. | A two-controller split (lifecycle + security/legal) was rejected because the BOLA spec reads the file by absolute path and asserts the exact handler names — the spec would have to be rewritten, risking losing the bite proof; the creation-policy spec would also break because the constructor arity is pinned. A sub-controller per domain (membership, invitations, custom-domains, holidays) was rejected for the same reason. The spec constraints make this the exceptional case where fragmentation is the wrong answer. | 2026-12-01 | Drops to 500 lines or below, or the two path-keyed specs are rewritten to use the service layer rather than the controller constructor directly, removing the split barrier. |
