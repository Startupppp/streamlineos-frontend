...
- [ ] Require zero unjournalled/orphan/timestamp-regressed migrations, zero chain gaps and zero unexplained schema differences. Missing: `drizzle.__drizzle_migrations` holds 484 DB rows against 455 journal entries — 21 applied rows whose journal entry was later removed and 8 duplicate timestamps remain. "Zero unjournalled" would require removing these historical orphan rows from the migrations table.
