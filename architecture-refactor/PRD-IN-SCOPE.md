...
- [ ] **Make tenant-isolation proof complete for every tenant-owned DB service.** Measured 2026-08-30: **758/819 covered (93%), 61 uncovered** — the `154/783` figure below is superseded. The gate still FAILS and the remaining 61 sit in AI, chat, billing, build, accounting, HR onboarding/interviews and payroll. Do not restate the old number. Prioritize Organization/RBAC, Home, HRMS/Payroll, Build, Billing/Accounting, Chat, Calendar, Notifications, Knowledge Base, Workflows and Inbox. For each uncovered service, either add a focused cross-tenant allow/deny test or remove it if graph proof shows it dead. Do not lower the gate or add blanket exclusions. Failure prevented: a missing `orgId` predicate is not structurally caught.
