...
- [x] Every existing row's status is unchanged after migration. — `0543` is applied (journal idx 305) and its `DO $$` guard is what proves this: it aborts if any `hr_positions.status` is absent from the seeded lookup, so the migration completing *is* the verification. `hr_positions.status` is now `text`, 176 statuses are seeded, and no row's value changed because the guard would have refused the migration otherwise. **Stated honestly: `hr_positions` holds 0 rows with a status**, so the guard passed over an empty set — it is a real gate, not a demonstration at volume.
