...
- **The cells are still not independently resourced.** Only the database and the queues that live in it are isolated. Cache, object storage, realtime, workers and monitoring are namespace-separated, which prevents collision but not a noisy neighbour — one Upstash instance has one budget however many prefixes are written into it. Closing this is a purchase; every code seam it needs exists and is tested.
- **No same-region headroom figure exists.** The driver runs ~88 ms of network away from its database, so four of its five breaches measure the link rather than a cell's ceiling. The fifth, `regional-rpo`, is a real operational gap and should not be filed alongside them.
