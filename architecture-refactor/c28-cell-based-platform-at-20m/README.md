...
**Being "done" here does not mean 20M-ready.** The release rule below is unchanged and unmet, and one of its conditions moved from *unknown* to *known-failing*, which is progress of a kind. **A relocation has been exercised** (a real organization moved between cells, verified by reading the target, rolled back with the source intact), **the canary rollback has fired** on a real measured regression, and **the workload results are published** in [`WORKLOAD-RESULTS.md`](WORKLOAD-RESULTS.md) — now **all 14 latency objectives measured**, up from 7, with every figure carrying the conditions it was taken under. Two of them were briefly published on numbers that meant nothing and were corrected before they stuck.

What remains unmet, stated precisely:

- **Recovery has now been drilled end to end, and a cell can be rebuilt from cold.** Neither was true when this session opened. The cell was destroyed and restored in **19.6 minutes against a 60-minute target**, integrity verified with every digest matching, and the cold chain reaches head (`REACHED_HEAD 371/371`). What still misses: the **operational** recovery point is **6 hours against a 5-minute target**, because a logical dump reads every table and cannot be taken every five minutes. A scheduled backup now bounds it where previously it was simply "whenever someone last ran one" (measured at 791 minutes). Closing it needs Neon PITR and a `NEON_API_KEY`. The rebuilt cell also comes back with two tables lacking an RLS policy, which is tracked as its own cross-tenant defect rather than folded into the timing figure.
- **The cells are still not independently resourced.** Only the database and the queues that live in it are isolated. Cache, object storage, workers and monitoring are namespace-separated, and realtime and search are not even that — they are shared outright, which prevents collision but not a noisy neighbour — one Upstash instance has one budget however many prefixes are written into it. Closing this is a purchase; every code seam it needs exists and is tested.
- **No same-region headroom figure exists.** The driver runs ~80 ms of network away from its database, so four of its five breaches measure the link rather than a cell's ceiling. The fifth, `regional-rpo`, is a real operational gap and should not be filed alongside them.
- **The migration chain reports 9 gaps, down from 132.** All 124 were in one migration, `0591`, which protects 401 tables of which 124 did not exist yet; `0489_chain_creates_early` lifts the enum and table creation out of `0619` to where the chain needs it. The 9 that remain belong to two migrations other sessions added today — 8 reference Inventory, a module this PRD excludes. `cell:compare-schema` reports 144 differences, 55 of the 59 named objects being those same Inventory tables. CI now rejects unjournalled SQL, duplicate migration numbers, timestamp regressions, orphan journal entries and chain gaps, and its self-test proves it catches all five.
...
| # | S | Ticket | Blocked by | Status |
|---|---|---|---|---|
| 30 | S6 | [The workload envelope is a runnable load profile](issues/30-the-workload-envelope-is-runnable.md) | 26 | **partial** · results PUBLISHED; **all 14 objectives now measured** (9 met, 5 breached) — the 7 previously not driven were all driven under final-refactor ticket 45; headroom still needs a colocated deployment |
...
| 34 | S7 | [Where you land after signing in comes from the index](issues/34-where-you-land-comes-from-the-index.md) | — | **done** · 6/7 criteria closed; `users.lastActiveOrgId` drop deliberately open (contract half, needs real traffic). Found and fixed two defects: the activation stamp was written before the row existed, and a landing hint could fail an org switch |
