...
**Being "done" here does not mean 20M-ready.** The release rule below is unchanged and unmet, and one of its conditions moved from *unknown* to *known-failing*, which is progress of a kind. **A relocation has been exercised** (a real organization moved between cells, verified by reading the target, rolled back with the source intact), **the canary rollback has fired** on a real measured regression, and **the workload results are published** in [`WORKLOAD-RESULTS.md`](WORKLOAD-RESULTS.md) — now **13 of 14 latency objectives measured**, up from 7, with every figure carrying the conditions it was taken under.

What remains unmet, stated precisely:

- **Recovery has now been drilled, and it failed.** `regional-rpo` measured **791 minutes against a 5-minute target** — not because recovery is slow, but because no backup schedule exists at all; `cell:backup` is a manual dump, so the recovery point is the age of the last time somebody ran it. `cell-rto` could not be timed, because **a cell can no longer be rebuilt from cold**: the chain now dies at `0628_communication_actor_normalization`, which inserts with an `ON CONFLICT` 22 lines before creating the unique index that clause needs. A restore that cannot rebuild its schema is not a restore.
...
- **The migration chain still reports 124 gaps.** `differences=0` holds at the end state, but statements mid-chain still reference objects that only arrive at `0619`. CI now rejects unjournalled SQL, duplicate migration numbers, timestamp regressions, orphan journal entries and chain gaps, and its self-test proves it catches all five — so the chain can no longer silently drift even though it is not yet repaired.
