...
- [ ] Ageing out detaches a partition; no bulk delete is issued. — **BLOCKED on c21-04, and the blocker is now specific.** Everything on this side is done: `NotificationRetentionService` is registered in `CronModule` (`cron.module.ts:121`) and reachable at `/cron/notifications-retention-detach` (`cron-platform.controller.ts:119-128`); it was never a duplicate of `CronNotificationRetentionService`, which issues row-level DELETEs per org while this one detaches and drops; `notification-retention.spec.ts` asserts every statement it issues is free of `DELETE`. What is missing is partitions to detach.

  **c21-04 did not simply run out of time — it found that the cutover needs a decision first.** Seven inbound foreign keys across `chat_messages` and `notifications` must each denormalise the parent's `created_at` or be dropped, and partitioning `notification_outbox` would weaken `uniq_notification_outbox_dedupe` to per-partition, so a retry crossing a boundary enqueues twice. Until that is decided this criterion cannot move, and the identity widening c21-04 *did* ship (`0580`, `0581`) does not change it either way.
