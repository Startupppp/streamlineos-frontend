...
- [ ] The email provider client is constructed by a factory provider from injected configuration, not by a module-scope constant.
- [ ] Provider selection (`EMAIL_PROVIDER`, and the fallback when it is unset) behaves exactly as before, including which provider wins when both are configured. Pin that with a test first if it is not already pinned — this is the one behaviour here a reader could not reconstruct from the code.
