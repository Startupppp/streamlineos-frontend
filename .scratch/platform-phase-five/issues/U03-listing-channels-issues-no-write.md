...
- [ ] `chat-entity-actions.controller.e2e-spec.ts` passes under `pnpm test:e2e`, run explicitly — e2e specs are excluded from the default run. — **FAILS: 7/10 tests fail**. Most failures return 402 where the spec expects other status codes with `ALL_MODULES` enabled — the module gate appears to fire despite the token carrying all modules, suggesting a mismatch between the e2e harness token and the guard's module-key lookup. Real missing work.
