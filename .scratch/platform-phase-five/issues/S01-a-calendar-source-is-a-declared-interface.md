...
- [~] Sources register against an injection token, so a module contributes one without the calendar naming it. — SUPERSEDED (the goal holds — the calendar names no source — by a different mechanism): the concurrent session replaced the injection-token approach with `CalendarSourceRegistry.register(source)` self-registration called from `OnModuleInit`; sources push themselves in rather than being pulled by a token array.
- [x] The registry asks a source only when its module is available to the caller, **and says which**. **FIXED** (`caafbeea`) — it now calls `moduleAvailability` through `moduleAvailabilityResolver`, with the reason stated in the file: `isModuleEnabled` takes no `userId`, so a person denied a module still received its events. That was a live disclosure, not a style point: a user denied HR still got the org's leave and interview rows in their calendar.
