...
- [x] **The request runs as the application role, not the owning role.** (verified 2026-08-24 by running it: the BYPASSRLS self-test passes against a real database — `rolbypassrls = false` for `current_user` — and the whole seeded suite is 8/8.)
- [x] A fixture builder declares an organisation, people, standings, grants, projects and plan tier, and returns identifiers. **Now built and exercised**: `addMember(alias, { standing })`, `onPlan("PAID" | "ENTERPRISE")`, `addProject()` and `addProjectMember()`, with `projects` on the returned fixture. Seeding stays in the one transaction the deferred owner-membership FK requires.

  It was added inert first, and that is worth recording: the extension had no consumer, so nothing established the inserts matched the real schema. Two defects survived to the run — `spec.key ?? a() || "PROJ"` is a **SyntaxError** (`??` mixed with `||` unparenthesised) that stopped the whole harness loading, and `grantPermissions` was defined but left out of the returned object. **`tsc --noEmit` was clean through both**, because `backend/tsconfig.json` includes only `src/**` and `evals/**` — the `test/` tree is type-checked by nothing, and ts-jest sets `diagnostics: false`. A self-test now asserts standing, plan and project membership actually reach the database, through Drizzle rather than raw SQL (`project_members` lives in the `build` Postgres schema, so hand-written SQL naming the bare table fails).
...
- [x] **The harness proves it can fail, before any stream uses it.** Four self-tests, all **run** 2026-08-24 against a real database, not merely present:
  - [x] Seed a person without a permission → refused. Seed the permission → allowed. (alice 403 / bob 200; and carol 403 → granted mid-test → 200, which is the version-invalidation case.)
  - [x] Seed two organisations; request organisation B's identifier as A's member → **not-found, never forbidden**. (404, holding `settings:rbac:manage` in her own org.)
  - [x] Clear the tenant context deliberately → the request fails rather than returning the row. (`42501` on the error's `cause`, where Drizzle puts the driver error.)
  - [x] Assert the connection is not running as a `BYPASSRLS` role. (`rolbypassrls = false`.)
