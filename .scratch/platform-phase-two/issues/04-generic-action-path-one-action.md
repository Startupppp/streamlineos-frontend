...
Nine of ten criteria are met and verified: both routes exist, are batched, carry no module permission key, are gated on conversation membership, map the failure vocabulary, and refuse a disabled module. 10 e2e tests pass.

**Criterion 8 is not met:** *"The client renders text, date, user and choice inputs from the declaration. The user kind uses the existing shared person picker."* The client still renders two bespoke dialogs.
...
**Unblocked 2026-08-24 by ticket 16.** `EntityActionInput` can now name where a `user` input's options come from, and `useEntityActionOptions` resolves it. The declaration-driven form is buildable; it is not built. This ticket stays open until it is.
