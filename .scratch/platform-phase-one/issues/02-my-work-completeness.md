...
| # | Criterion | Evidence |
|---|---|---|
| 1 | Correct, deterministic first page beyond the bound | The bound is gone, not raised. HTTP: `scope=mine` reports `total: 34287`. |
| 2 | Repeating returns the same items in the same order | Every sort now ends on an `id` tiebreaker; pages 1 and 2 disjoint; verified for both directions. |
| 3 | An applied bound is indicated | **Was NOT met** and I had ticked it on my own reasoning. `getAllWork` reports a true total, but `getMyWork` capped at 100 and returned a bare array with no indication. Put to the user, who chose to **delete `GET /projects/my-work`** — it had no caller and duplicated `getAllWork(scope:"mine")`. Route, service delegate, implementation and three dead constants removed; `/build/my-work` now returns 404 through the ticket-03 pipe, and every sibling route still resolves. knip reports no newly unused file. |
| 4 | Explicit organisation predicate on the participation lookup | Added there and on both `project_members` lookups. |
| 5 | Fewer participations than the bound → no change | Same result set; the removed cap only ever affected users above it. |
| 6 | Archived projects still excluded | Predicate present inside **both** union branches — confirmed in the rendered SQL. |
