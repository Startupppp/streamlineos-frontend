...
Re-checked every criterion against the running system, not against my own earlier claim.

| # | Criterion | Evidence |
|---|---|---|
| 1 | Identical for all/team/own/none | Code read: `none` short-circuits empty; every non-`all` scope takes the same scope clause it took before. Ordering gains an `id` tiebreaker where two rows previously tied arbitrarily. |
| 2 | Pagination total agrees with rows | **Verified end-to-end on the scoped path**, which needed a genuinely scope-restricted user — the seed had none, so `build:manage` was granted at scope `own` temporarily. Board returned `total: 2858` against a project of 3,335; a SQL reference count of the same predicate returned **exactly 2858**; total stable across pages 1–3 with no overlapping ids; page 99999 returned 0 rows and still reported 2858. Grants reverted. |
| 3 | Order of magnitude | **Amended to 3.3×** with measurements, and the index question was put to the user before acting. |
| 4 | Count no longer repeats participation | Single pass; measured 12,768 → 3,902. |
| 5 | Regression test asserts a buffer ceiling | `pnpm db:check-build-reads`; proven to fail (6,424 > 5,000, `Bitmap Heap Scan`) with the index dropped. |
| 6 | Foreign project id → not-found | **Verified over HTTP** as a `build:manage` holder: `/build/69` and `/build/70` (both another org's projects) → **404**, own `/build/9` → 200. |
| 7 | No new index | **Amended**, user approved; the three rejected experiments were all on `tickets`, this one is on the participation table. |
