...
| # | Criterion | Evidence |
|---|---|---|
| 1 | Resolves member, non-member payee, person without a login | `person-seam.spec.ts` 8/8, plus live probes for the member and worker paths. |
| 2 | Unresolvable is a value, not a throw | Covered for all three subject kinds. |
| 3 | Payroll assertions keep signatures and errors, specs unedited | `payroll-payee-eligibility.spec.ts` **6/6, unedited**. |
| 4 | Tenant scope re-asserted every call | Proved **as the BYPASSRLS owner**, so only the seam's own predicate was filtering: own org resolves, another org does not; same for a worker. |
| 5 | Foreign subject → not-found, never forbidden | **Conflicted with criterion 3** and I had ticked it anyway. Put to the user, who chose to **keep `ForbiddenException` in the payroll wrappers** so a money path does not change and its spec stays unedited. Criterion 5 is therefore **amended to apply to the seam only** — it returns `unresolved`, and new callers are the ones that must surface 404. |
| 6 | Sensitive fields stay gated | By construction: the seam returns ids, flags and employment number/status only — no encrypted or bank field. |
| 7 | Canonical model written into the rules file | `backend/CLAUDE.md` §1, including the short-circuit contract behind `resolvedVia`. |
