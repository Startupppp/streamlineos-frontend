...
- [ ] p50/p95/p99 are recorded at the release commit for each declared budget, with measured fields populated.
  ...
  - **STALE AFTER THE CHAT FIX:** `GET /chat/channels` carries `measuredResponseBytes: 436371` from the pre-fix capture. The route now measures **6,876** bytes (reference) / 10,448 (minority) at head. Re-run `node test/perf/route-budget-http.seeded-e2e-spec.ts` full capture → `merge-http-measurement.mjs --write` → `merge-http-route-budgets.mjs --write` to refresh; until then that one breach line in `check:route-budgets` is a stale number, not a live defect.
