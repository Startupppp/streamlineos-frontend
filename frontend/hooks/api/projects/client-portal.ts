...
export function usePortalProjects() {
  return useQuery<ClientPortalProject[]>({
    ...
    queryFn: () => apiClient.get<ClientPortalProject[]>("/projects/portal/projects"),
    staleTime: 60_000,
  });
}
...
export function usePortalProjectOverview(projectId: number) {
  return useQuery<ClientPortalOverview>({
    ...
    queryFn: () =>
      apiClient.get<ClientPortalOverview>(`/projects/portal/projects/${projectId}/overview`),
    enabled: !!projectId,
    staleTime: 60_000,
  });
}
...
export function usePortalChangeRequests(projectId: number) {
  return useQuery<ChangeRequest[]>({
    ...
    queryFn: () =>
      apiClient.get<ChangeRequest[]>(`/projects/portal/projects/${projectId}/change-requests`),
    enabled: !!projectId,
    staleTime: 60_000,
  });
}
  ...
  return useMutation({
    ...
    mutationFn: (data: CreateChangeRequestInput) =>
      apiClient.post<ChangeRequest>(
        `/projects/portal/projects/${projectId}/change-requests`,
        data,
      ),
    onSuccess: () => {
...
export function useClientVisibility(projectId: number) {
  return useQuery<ClientVisibilitySummary>({
    ...
    queryFn: () =>
      apiClient.get<ClientVisibilitySummary>(`/projects/${projectId}/client-visibility`),
    enabled: !!projectId,
    staleTime: 30_000,
  });
}
  ...
  return useMutation({
    ...
    mutationFn: ({ id, clientVisible }: { id: number; clientVisible: boolean }) =>
      apiClient.patch<{ success: boolean }>(
        `/projects/${projectId}/client-visibility/tickets/${id}`,
        { clientVisible },
      ),
...
export function useUpdateMilestoneVisibility(projectId: number) {
  ...
  return useMutation({
    ...
    mutationFn: ({ id, clientVisible }: { id: number; clientVisible: boolean }) =>
      apiClient.patch<{ success: boolean }>(
        `/projects/${projectId}/client-visibility/milestones/${id}`,
        { clientVisible },
      ),
    onMutate: async ({ id, clientVisible }) => {
      const key = queryKeys.projects.clientPortal.visibility(projectId);
