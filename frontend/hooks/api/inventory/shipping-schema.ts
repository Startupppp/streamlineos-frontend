...
const shipmentContract = z.object({
  ...
  status: z.enum(["DRAFT", "PACKED", "LABEL_CREATED", "SHIPPED", "DELIVERED", "CANCELLED"]),
...
const packageContract = z.object({
  ...
  status: z.enum(["OPEN", "CLOSED", "SHIPPED"]),
  createdAt: z.string(),
  updatedAt: z.string(),
...
const loadContract = z.object({
  ...
  loadNumber: z.string(),
  carrierId: z.number().int().nullable(),
  status: z.enum(["DRAFT", "DISPATCHED", "ARRIVED", "CLOSED", "CANCELLED"]),
