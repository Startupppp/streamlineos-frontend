...
| Pagination | `TablePagination`, `useCursorPager` — `components/ui/table-pagination.tsx` · `DataTablePagination` — `components/shared/data-table-pagination.tsx` · `CursorPageControls` — `components/ui/cursor-page-controls.tsx` |
...
| Tabs | `Tabs`, `TABS_CONTENT_PAGE_BODY_CLASS` — `components/ui/tabs.tsx` · `PageTabsToolbar` — `components/ui/page-tabs-toolbar.tsx` |
...
- **Table density from the primitives:** `TableHead` `h-10 px-2 text-sm font-medium text-muted-foreground whitespace-nowrap` · `TableRow` `hover:bg-muted/50 data-[state=selected]:bg-muted border-b` · `DataTable` header `sticky top-0 z-10 bg-muted/50 border-b` · skeleton cell `px-2 py-2 text-sm`. **Anything denser than `h-10` is not implemented.**
