...
import VendorPortalPage from "@/app/(public)/vendor-portal/[vendorPortalToken]/page";
...
describe("public vendor portal first HTML", () => {
  ...
  it("renders vendor data into the first server HTML", async () => {
    ...
    const tree = await VendorPortalPage({ params: Promise.resolve({ vendorPortalToken: "vnd123" }) });
  ...
  it("calls notFound when the token is not found", async () => {
    ...
    await expect(
      VendorPortalPage({ params: Promise.resolve({ vendorPortalToken: "bad-token" }) }),
  ...
  it("renders expired state when the backend returns 410", async () => {
    ...
    const tree = await VendorPortalPage({ params: Promise.resolve({ vendorPortalToken: "old-token" }) });
    const html = renderToStaticMarkup(tree);
