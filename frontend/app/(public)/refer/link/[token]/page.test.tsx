...
import ExternalReferrerPortalPage from "@/app/(public)/refer/link/[token]/page";
...
describe("public referrer portal first HTML", () => {
  ...
  it("renders portal data into the first server HTML", async () => {
    ...
    const tree = await ExternalReferrerPortalPage({ params: Promise.resolve({ token: "ref123" }) });
  ...
  it("calls notFound when the token is not found", async () => {
    ...
    await expect(
      ExternalReferrerPortalPage({ params: Promise.resolve({ token: "bad-token" }) }),
  ...
  it("renders blocked state when the backend returns 403", async () => {
    ...
    const tree = await ExternalReferrerPortalPage({ params: Promise.resolve({ token: "blocked-tok" }) });
    const html = renderToStaticMarkup(tree);
