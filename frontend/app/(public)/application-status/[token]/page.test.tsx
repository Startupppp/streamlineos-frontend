...
import ApplicationStatusPage from "@/app/(public)/application-status/[token]/page";
...
describe("public application-status first HTML", () => {
  ...
  it("renders application data into the first server HTML", async () => {
    ...
    const tree = await ApplicationStatusPage({ params: Promise.resolve({ token: "abc123" }) });
  ...
  it("calls notFound when the token is not found", async () => {
    ...
    await expect(
      ApplicationStatusPage({ params: Promise.resolve({ token: "bad-token" }) }),
