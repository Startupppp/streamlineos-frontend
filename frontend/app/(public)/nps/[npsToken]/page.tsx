...
export default function PublicNpsPage() {
  const params = useParams<{ npsToken: string }>();
  const npsToken = params.npsToken;

  const surveyQuery = usePublicNpsSurvey(npsToken);
  const submitMutation = useSubmitNpsResponse(npsToken);
