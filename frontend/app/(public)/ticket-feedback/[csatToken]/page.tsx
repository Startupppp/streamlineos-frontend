...
export default function TicketFeedbackPage() {
  const params = useParams<{ csatToken: string }>();
  const csatToken = params.csatToken;

  const surveyQuery = usePublicCsatSurvey(csatToken);
  const submitMutation = useSubmitCsatResponse(csatToken);
