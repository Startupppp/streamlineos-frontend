...
import OfferAcceptancePage from "@/app/(public)/offer/[offerToken]/page";
...
describe("public offer acceptance first HTML", () => {
  ...
  it("renders active offer data into the first server HTML", async () => {
    ...
    const tree = await OfferAcceptancePage({ params: Promise.resolve({ offerToken: "tok123" }) });
  ...
  it("calls notFound when the token is not found", async () => {
    ...
    await expect(
      OfferAcceptancePage({ params: Promise.resolve({ offerToken: "bad-token" }) }),
  ...
  it("renders expired state when the backend returns 410", async () => {
    ...
    const tree = await OfferAcceptancePage({ params: Promise.resolve({ offerToken: "old-token" }) });
    const html = renderToStaticMarkup(tree);
