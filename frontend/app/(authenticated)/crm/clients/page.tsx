    ...
    <Link href={`/crm/clients/${account.id}`}>
      <Card className="hover:shadow-md transition-shadow cursor-pointer">
        <CardContent className="p-3 space-y-2">
          ...
          {account.estimatedInvestment &&
            parseFloat(account.estimatedInvestment) > 0 && (
          ...
          {account.convertedAt && (
            <p className="text-[10px] text-muted-foreground">
              {new Date(account.convertedAt).toLocaleDateString("en-IN", {
                day: "numeric",
                month: "short",
                year: "numeric",
              })}
...
function KanbanColumn({
  ...
  return (
    <div className="flex flex-col min-w-0 sm:min-w-[280px] sm:max-w-[320px] flex-1">
      <div
        className={cn(
          "flex items-center gap-2 px-3 py-2 rounded-t-lg border-t-2",
          config.color,
          config.bg,
        )}
      >
        ...
        <Badge
          variant="secondary"
          className="ml-auto text-[10px] h-5 min-w-5 px-1.5"
        >
          {count}
        </Badge>
      </div>
...
export default function ClientAccountsPage() {
  ...
  const setStatusFilter = useCallback(
    (s: string) => updateParams({ status: s === "all" ? null : s, page: null }),
    [updateParams],
  );
  ...
  const { data: allData, isLoading: statsLoading } = useClientAccounts({
    limit: 500,
  });
  const allAccounts = allData?.accounts ?? [];
  const stats = {
    total: allAccounts.length,
    accountOpening: allAccounts.filter((a) => a.status === "ACCOUNT_OPENING")
      .length,
    queries: allAccounts.filter((a) => a.status === "QUERIES").length,
    planSelected: allAccounts.filter((a) => a.status === "PLAN_SELECTED")
      .length,
    invested: allAccounts.filter((a) => a.status === "INVESTED").length,
  };

  const queryFilters: ClientAccountFilters =
    viewMode === "kanban"
      ? { search: debouncedSearch || undefined, limit: 200 }
      : {
          status:
            statusFilter !== "all"
              ? (statusFilter as ClientAccountStatus)
              : undefined,
  ...
  if (error) {
    return (
      <PageWrapper
        ...
        subtitle="Post-conversion client management"
      >
        <ErrorState
    ...
    >
      ...
      >
        ...
        >
          ...
          <StatCard
            label="Account Opening"
            value={statsLoading ? "…" : (stats?.accountOpening ?? 0)}
            icon={ClipboardList}
            color="cyan"
            index={1}
          />
          <StatCard
            label="Queries"
            value={statsLoading ? "…" : (stats?.queries ?? 0)}
            icon={MessageSquare}
            color="amber"
            index={2}
          />
          <StatCard
            label="Plan Selected"
            value={statsLoading ? "…" : (stats?.planSelected ?? 0)}
            icon={FileCheck}
            color="violet"
            index={3}
          />
          <StatCard
            label="Invested"
            value={statsLoading ? "…" : (stats?.invested ?? 0)}
            icon={IndianRupee}
            color="green"
            index={4}
          />
        ...
        {viewMode === "kanban" && (
          <motion.div variants={fadeUp}>
            {isLoading ? (
              ...
              <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 pb-4">
                {kanbanColumns.map(
                  ({ status, accounts: colAccounts, count }) => (
                    <KanbanColumn
                      key={status}
                      status={status}
