...
import { Building2, GitMerge, Globe, Plus, Users } from "lucide-react";
...
function getHealthBadgeClasses(score: number | null): string {
  ...
  if (score >= 70) return "bg-status-success-surface text-status-success-ink border-status-success-rule";
  if (score >= 40) return "bg-status-warning-surface text-status-warning-ink border-status-warning-rule";
  ...
  const columns: DataTableColumn<CrmOrganization>[] = [
    ...
    },
    {
      ...
      cell: (o) => (
        <Badge
          variant="outline"
          className={cn("text-micro px-1.5 py-0 h-4", getHealthBadgeClasses(o.healthScore))}
        >
