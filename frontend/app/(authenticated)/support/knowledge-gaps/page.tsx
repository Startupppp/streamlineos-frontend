...
import { KnowledgeGapCard } from "@/features/support/components/knowledge-gap-card";
...
export default function KnowledgeGapsPage() {
  ...
  function handleDraft(gapId: number) {
    draftMutation.mutate(
      ...
      {
        onSuccess: () =>
          toast.success("KB draft created — review & publish in KB"),
        onError: (err) => toast.error(getErrorMessage(err)),
      },
    );
  }
          ...
          {data.gaps.map((gap) => (
            <KnowledgeGapCard
              ...
              isDrafting={
                ...
                draftMutation.variables?.gapId === gap.id
              }
              isDismissing={
                dismissMutation.isPending &&
                dismissMutation.variables?.gapId === gap.id
              }
            />
          ))}
