...
import { ASSET_STATUS_OPTIONS, isAssetStatusFilter, type AssetStatusFilter } from "@/features/accounting/assets/asset-constants";
import { AssetTable } from "@/features/accounting/assets/asset-table";
import { CreateAssetSheet, type CreateAssetFormValues } from "@/features/accounting/assets/create-asset-sheet";
...
export default function FixedAssetsPage() {
  ...
  function handleCreateAsset(values: CreateAssetFormValues): void {
    createAsset.mutate(
      {
        name: values.name, categoryId: values.categoryId, acquisitionDate: values.acquisitionDate,
        acquisitionCost: Number.parseFloat(values.acquisitionCost),
        salvageValue: Number.parseFloat(values.salvageValue || "0"),
        usefulLifeMonths: values.usefulLifeMonths, depreciationMethod: values.depreciationMethod,
        vendorId: values.vendorId, billId: values.billId,
      },
      {
  ...
  return <>
    <PageWrapper title="Fixed Assets" subtitle="Track assets and depreciation" actions={<AssetPageActions canCreate={canCreate} onCreate={() => setCreateOpen(true)} />}>
      <Tabs defaultValue="assets" className="flex min-h-0 flex-1 flex-col">
        <TabsList><TabsTrigger value="assets">Assets</TabsTrigger><TabsTrigger value="categories">Categories</TabsTrigger></TabsList>
        ...
        <TabsContent value="categories" className={TABS_CONTENT_PAGE_BODY_CLASS}>
          ...
          {categoriesQuery.error ? <ErrorState title="Failed to load categories" description={getErrorMessage(categoriesQuery.error)} onRetry={() => void categoriesQuery.refetch()} /> :
            <DataTable<AssetCategory> className="flex-1 min-h-0" data={categories} columns={categoryColumns} getRowKey={(category) => category.id} isLoading={categoriesQuery.isLoading} minWidth="500px" emptyState={<EmptyState illustration={<EmptyReportIllustration />} title="No categories yet" description="Create a category to group your fixed assets." action={canCreate ? { label: "Add Category", onClick: handleOpenCreateCategory } : undefined} />} />}
        </TabsContent>
      </Tabs>
    </PageWrapper>
    <CreateAssetSheet open={createOpen} onOpenChange={setCreateOpen} categories={categories} isSubmitting={createAsset.isPending} onSubmit={handleCreateAsset} />
...
function AssetPageActions({ canCreate, onCreate }: { canCreate: boolean; onCreate: () => void }) {
  return <div className="flex items-center gap-2">
    <Button size="sm" variant="outline" asChild><Link href="/accounting/assets/depreciation">Depreciation Runs</Link></Button>
    {canCreate && <Button size="sm" onClick={onCreate}><Plus className="mr-1 size-4" />Add Asset</Button>}
  </div>;
}

interface AssetFiltersProps {
  status: AssetStatusFilter; categoryId: string; categories: AssetCategory[];
  onStatusChange: (value: string) => void; onCategoryChange: (value: string) => void;
}
function AssetFilters({ status, categoryId, categories, onStatusChange, onCategoryChange }: AssetFiltersProps) {
  return <div className={FILTER_TOOLBAR_ROW}>
    <Select value={status} onValueChange={onStatusChange}><SelectTrigger className={cn("w-[180px]", FILTER_SELECT_TRIGGER)}><SelectValue /></SelectTrigger><SelectContent>{ASSET_STATUS_OPTIONS.map((option) => <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>)}</SelectContent></Select>
    <Select value={categoryId} onValueChange={onCategoryChange}><SelectTrigger className={cn("w-[180px]", FILTER_SELECT_TRIGGER)}><SelectValue placeholder="All categories" /></SelectTrigger><SelectContent><SelectItem value="ALL">All categories</SelectItem>{categories.map((category) => <SelectItem key={category.id} value={String(category.id)}>{category.name}</SelectItem>)}</SelectContent></Select>
  </div>;
}
