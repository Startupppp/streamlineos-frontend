...
export default function PayrollRunDetailPage({ params }: RunDetailPageProps) {
  const { runId: runIdStr } = use(params);
  const runId = parseInt(runIdStr, 10);
  const searchParams = useSearchParams();
  const router = useRouter();
  const activeTab = searchParams.get("tab") ?? "employees";

  const { data, isLoading, refetch } = usePayrollRun(runId);
  const run = data?.run;
  ...
  const exceptionsDone = checklist.find((c) => c.key === "exceptions_resolved")?.done ?? false;
      ...
      <div className="space-y-4">
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange}>
          <TabsList className="h-8">
            <TabsTrigger value="employees" className="text-xs h-7">
              Employees
            </TabsTrigger>
            <TabsTrigger value="exceptions" className="text-xs h-7 flex items-center gap-1">
              Exceptions
              {openExceptions > 0 && (
                <span className="inline-flex items-center justify-center h-4 min-w-4 px-1 rounded-full bg-red-100 text-red-700 text-[9px] font-bold">
                  {openExceptions}
                </span>
              )}
            </TabsTrigger>
            <TabsTrigger value="inputs" className="text-xs h-7">
              Inputs
            </TabsTrigger>
            <TabsTrigger value="variance" className="text-xs h-7">
              Variance
            </TabsTrigger>
          </TabsList>
