...
export default function ShippingQueuePage() {
  ...
  const query = useSalesOrders({ status: "PACKED", limit: 50 });
  ...
  return (
    <PageWrapper
      ...
      subtitle={query.data ? `${total} ${total === 1 ? "order" : "orders"} to ship` : undefined}
      filters={filterBar}
    >
      <DataTable
        data={items}
        ...
        emptyState={
          query.error ? (
            ...
            <EmptyState
              ...
              description="Sales orders in Packed status will appear here ready for shipping."
              compact
            />
          )
        }
