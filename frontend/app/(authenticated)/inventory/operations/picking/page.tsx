...
export default function PickingQueuePage() {
  ...
  const query = useSalesOrders({ status: "RESERVED", limit: 50 });
  ...
  return (
    <PageWrapper
      ...
      title="Picking Queue"
      subtitle={query.data ? `${total} ${total === 1 ? "order" : "orders"} to pick` : undefined}
      filters={filterBar}
    >
      <DataTable
        data={items}
        ...
        emptyState={
          query.error ? (
            ...
            <EmptyState
              ...
              description="Sales orders in Reserved status will appear here ready for picking."
              compact
            />
          )
        }
