...
jest.mock("@/features/inventory/components/uom-create-form", () => ({
  UomCreateForm: () => <div data-testid="uom-create-form" />,
}));
...
import UomPage from "./page";

beforeEach(() => {
  jest.clearAllMocks();
  mockUseAccess.mockReturnValue(accessGranted);
  mockUseUom.mockReturnValue(emptyQuery());
});

describe("UomPage — access is three-valued, not a boolean", () => {
  ...
  it("renders the UOM page normally when access is granted", () => {
    mockUseAccess.mockReturnValue(accessGranted);

    render(<UomPage />);

    expect(screen.queryByText(/access restricted/i)).toBeNull();
    expect(screen.getByRole("heading", { name: /units of measure/i })).toBeInTheDocument();
  });
});
