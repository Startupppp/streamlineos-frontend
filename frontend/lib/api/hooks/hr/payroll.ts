...
export function useHrAllPayrolls(params: GetAllPayrollsInput) {
  const qk: Record<string, string> = {};
  if (params.month) qk.month = params.month;
  if (params.year) qk.year = params.year;
  return useQuery({
    queryKey: queryKeys.hr.payrolls(qk),
    queryFn: () => apiClient.get<PayrollWithUser[]>("/hr/payrolls/all", qk),
    staleTime: 2 * 60_000,
