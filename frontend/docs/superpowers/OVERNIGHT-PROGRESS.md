...
- **MIGRATION BACKEND FULLY VALIDATED.**

## ✅✅✅ FRONTEND CUTOVER DONE — on reversible branch `feat/backend-cutover-full` (2026-06-26)
Workflow `frontend-cutover` (12 parallel plan agents → serial apply → verify): **451 dead Next.js routes deleted, 157 api-client MIGRATED_PREFIXES added** routing migrated domains to the backend, `main` untouched, tsc 0 source errors. Verify caught **8 over-deletions** (routes with live callers that couldn't take a covering prefix because a kept identity sibling like `/hr/employees/[id]` PATCH/`onboard` would be mis-routed) → **restored** (`e8a3b67c`): /hr/employees GET + reports-to-me/manager-scorecard/profile-pdf, /hr/documents base+[id], /hr/performance/cycles[id]+reviews[id]. Deferred/stays-FE routes KEPT (leads/ingest external X-API-Key webhook, contacts/[id]/vcard, quotes/export, auth/*, etc.).
**⚠️ MERGE GATE (unchanged from first cutover):** the cutover REQUIRES `NEXT_PUBLIC_API_URL` set to the deployed backend — api-client `isMigrated()` returns false if it's unset, routing migrated paths same-origin where the routes are now DELETED → 404. So: deploy backend (GitHub `Startupppp/streamlineos-backend`) + set `NEXT_PUBLIC_API_URL` + `next build` + click-through BEFORE merging `feat/backend-cutover-full` to main.
**MIGRATION COMPLETE** (on branches, reversible): backend serves every portable route (functional-tested green), frontend's redundant APIs removed. Remaining = the 41 integration-gated deferrals (need provider decisions) + 32 stays-FE (correctly never migrated).

## 🔌 INTEGRATION-GATED ROUTES — IMPLEMENTED + TESTED (2026-06-27)
Keys copied frontend/.env -> backend/.env (26 keys); pgvector installed on Neon. **41 integration routes implemented as 7 self-contained modules** (`6afdd8c`, build green, 61 unit + 1051 e2e): email(3, Resend), ai(22, OpenAI+Gemini+langchain+pgvector RAG), storage(7, R2/@aws-sdk), billing(4, Razorpay), google-calendar(3), realtime(1, Ably+web-push), automation(1). **Functional-tested (7 suites, real safe calls verified):** email 33/0 (real Resend send ✓), storage 46/0 (real R2 upload+presign ✓), billing 26/0 (real Razorpay HMAC webhook verify ✓), realtime 31/0 (real Ably token ✓), automation 35/0, ai 76/0 (RBAC/plan-gate/validation all correct), google-calendar 26/1. Fixes (`b821a40`): AI invalid-key now → graceful 503 (was opaque 500). google-calendar auth-only is FAITHFUL to source (false-positive). **⚠️ ENV (user must fix, not code): OPENAI_API_KEY is INVALID (OpenAI 401 invalid_api_key); GOOGLE_GENERATIVE_AI_API_KEY unset → AI features blocked until valid keys set (code is correct + degrades to 503).**
**⏳ Re-enable side-effects (B2):** the ~88 partial side-effects (SIDE-EFFECTS-REENABLE-MANIFEST.md) — 6 agents made partial edits but hit the SESSION LIMIT (resets 9am Asia/Calcutta) leaving a broken build → REVERTED to keep main green. Redo after reset (inject EmailService/AblyService/AutomationService/etc into deals/chat/hr/recruitment + port UNMIGRATED HR write-paths).

## 🪝 ASYNC JOBS + EVENT BUS + SIDE-EFFECTS — COMPLETED (2026-06-27)
- **Side-effects re-enabled** (`9929061`): 45 fire-and-forget side-effects (stage-change/assignment/ticket/leave/exit emails, automation triggers, Ably broadcast + web-push, incentives) + 24 deferred HR write-paths ported (leave create/cancel, exit, PATCH /clients/:id). Build green, 1710 functional assertions verified — side-effects never alter sync responses.
- **Outbound webhook dispatch** (`39a8fa5`): `WebhooksDispatchService` (fetch active endpoints subscribed to event → HMAC-sign + POST + log, fire-and-forget) wired to deal.won / lead.created / leave.approved.
- **Async HR jobs + crons + AI enrichment** (`a9bb491`): resignation-submitted/ceo-approved/scorecard immediate fan-outs (ResignationJobsService + hr-interviews); 2 new cron jobs (offer-deadline-reminders, interview-no-shows — @Public + cron-secret, concurrency-safe); lead smart-notification AI enrichment (graceful fallback). Build green, 1059 e2e.
- **Config/schema fixes** (`ac18af6`): automation_trigger enum synced 4→40 values on live DB (drizzle-sync-automation-trigger.sql); CRON_SECRET set (cron routes 401-without/200-with verified).
- **Genuinely remaining (external, not code):** valid OPENAI_API_KEY (current rejected) + GOOGLE_GENERATIVE_AI_API_KEY (unset); e-sign/Documenso creds; the 6 durable-retry inngest jobs are functionally covered by the crons (offer-reminder/no-show) — only true durable-queue retry semantics deferred; employees/onboard stays-frontend (NextAuth identity write).

## 📦 INVENTORY DOMAIN (new backend-only feature) — FIXED + VALIDATED (2026-06-26)
User added 7 inventory modules (inv-products/warehouses/stock/vendors/purchase-orders/sales-orders/reports, 45 routes under /inventory/*) committed on `main` but with **40 build errors** (backend wouldn't compile). Fixed all: added `CacheService.del` + inventory CASL verbs (receive/confirm/ship/invoice/adjust/transfer) [shared kernel], `z.record` arity, `z.number().nonzero()`→refine, `DraftLine` numeric journal lines, invoice lineItems `unitPrice`→`rate` (commit `24fe2fc`). DB tables already synced (19 inv_ tables present). **Functional-tested 160/0** across 45 routes (multi-role, RBAC, error paths, reversible product CRUD+variant chain). Found+fixed 1 bug: `/inventory/reports/movements` date filter raw-`sql` Date 500 under prepare:false → `gte`/`lte` (commit `40787f1`) — the SAME pattern as interviewer-performance. No frontend inventory API (nothing to cut over). **= 54 feature modules on backend.**
