...
const EXPORT_VERDICTS = new Map([
  ...
  ["hooks/api/build/sprints.ts:useCreateSprint", { verdict: "WIRE", reason: "nothing creates a sprint: features/build has cycles, milestones and iterations sheets but no sprint-create control; backend POST build/:projectId/sprints exists (execution/iterations.controller.ts:78). Stale the day a sprint-create sheet calls it or the route is removed" }],
  ["hooks/api/hr/hr-org.ts:useOrgHeadcount", { verdict: "WIRE", reason: "no headcount surface on the HR org hub: features/hr/org renders org-catalog-table through org-hub-client and nothing else; its only consumer, headcount-stats.tsx, was never mounted by any route and was deleted in this pass. Backend GET hr/org/headcount exists (hr/core/hr-org-catalog.controller.ts:141). Stale the day the org hub renders headcount stats" }],
