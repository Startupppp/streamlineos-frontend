...
const EXPORT_VERDICTS = new Map([
  ...
  ["hooks/api/kb/kb-page-collection-schema.ts:kbPageCollectionResponseSchema|kbPageCollectionContract", { verdict: "KEEP", reason: "one base schema plus a per-route alias: kbPageCollectionResponseSchema defines the contract shape and types KbPageCollectionResponse; kbPageCollectionContract is the alias (= kbPageCollectionResponseSchema) passed to lazyContract at hooks/api/kb/page-collection.ts:27 via dynamic import — the static graph cannot follow the dynamic() call. Stale the day the alias stops being passed to a lazyContract seam" }],
  ...
  ["hooks/api/kb/pages.ts:useRestoreKbPage", { verdict: "WIRE", reason: "no per-page restore action in the KB trash UI; trash-page.tsx uses bulk restore useKbBulkRestorePages instead. Backend POST /kb/pages/:pageId/restore exists (requires kb:pages:update). Wire to a per-row restore action when individual-item operations are added to the trash surface" }],
  ["hooks/api/kb/pages.ts:useHardDeleteKbPage", { verdict: "WIRE", reason: "no per-page permanent-delete action in the KB trash UI; trash-page.tsx uses bulk purge useKbBulkPurgePages instead. Backend DELETE /kb/pages/:pageId/permanent exists (requires kb:pages:purge). Wire to a per-row purge action when individual-item operations are added to the trash surface" }],
  ["hooks/api/kb/pages.ts:useKbProjectPagesTree", { verdict: "WIRE", reason: "project-scoped page-tree hook added 2026-09-23 alongside wiki-home-page.test.tsx, which already mocks it — no component currently calls it. Backend GET /kb/pages/tree accepts a projectId query param. Wire to a project-filtered tree view when that surface is added to WikiHomePage or a project-detail panel" }],

  ["hooks/api/kb/spaces.ts:useKbSpaceArchiveImpact", { verdict: "WIRE", reason: "pre-flight read for an archive-space confirmation dialog — returns the count of pages and sub-spaces that would be affected; no dialog exists yet. Backend GET /kb/spaces/:spaceId/archive-impact exists (requires kb:spaces:manage). Wire alongside an archive-confirmation sheet when the spaces management UI adds a two-step archive flow" }],
  ["hooks/api/kb/spaces.ts:useDeleteKbSpace", { verdict: "WIRE", reason: "permanent-delete mutation for a KB space; spaces-page.tsx currently re-routes its delete button through useArchiveKbSpace (soft-delete). Backend DELETE /kb/spaces/:spaceId exists (requires kb:spaces:manage). Wire to a permanent-delete confirmation dialog when the spaces management UI separates archive from hard-delete" }],

  ["features/wiki/components/space-card.tsx:SpaceCardItem", { verdict: "WIRE", reason: "public type alias added 2026-09-23 when SpaceCard was refactored to accept KbSpaceListItem; provides a component-level import path so consumers can type their space state without reaching into hooks/api/kb/spaces. No consumer exists yet. Stale the day a component imports SpaceCardItem or the alias is removed" }],
