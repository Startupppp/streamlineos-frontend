...
const EXPORT_VERDICTS = new Map([
  ...
  ["hooks/api/dashboard.ts:useTodayActivities", { verdict: "WIRE", reason: "no surface renders today's activity feed; backend GET /dashboard/today-activities exists and is response-schema'd (dashboard.controller.ts:236, todayActivitiesSchema). Stale the day that route is deleted or a dashboard card consumes this hook" }],
  ["components/list-view/filter-types.ts:Sprint", { verdict: "WIRE", reason: "dead the moment Build's last sprint control was cut over to cycles: the shared list-filter controls are generic and this option shape had exactly one declarer (TICKET_FILTER_SPEC's sprint category) and one consumer chain (filter-category-submenu, filter-flat-search), all of which now read Cycle. Deletion is pending only because components/list-view is another branch's tree in this cycle; delete the interface, its index re-export, FilterState.sprintParam, the \"sprint\" member of BuildFilterCategory, its BUILD_CATEGORY_TITLES entry and the filter-category-row case together" }],
  ["components/list-view/index.ts:Sprint", { verdict: "WIRE", reason: "the barrel re-export of the same retired option shape; goes out with components/list-view/filter-types.ts:Sprint" }],
  ...
  ["lib/build/pm-workspace-path.ts:withPmWorkspacePath", { verdict: "WIRE", reason: "the write direction of this module has no caller: lib/build/build-scope.ts consumes both read helpers from this file (parsePmWorkspaceIdFromPath, stripPmWorkspacePrefix) to derive the active workspace, but nothing rebuilds the URL to switch workspace while preserving the current sub-path. Stale the day a workspace switcher calls it" }],
