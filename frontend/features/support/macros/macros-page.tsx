...
export function MacrosPage() {
  ...
  const query = debouncedSearch.trim() ? { search: debouncedSearch.trim() } : undefined;
  ...
  const categoryOptions = useMemo(() => getCategoryOptions(macros ?? []), [macros]);
  ...
  return (
    ...
    >
      <div className="flex min-h-0 flex-1 flex-col gap-4">
        <SearchInput placeholder="Search responses…" value={search} onValueChange={setSearch} />
      ...
      {editTarget ? (
        <MacroDialog macro={editTarget} categoryOptions={categoryOptions} onClose={() => setEditTarget(null)} />
      ) : null}
