...
export function WorkspaceRoadmapPage({ pmWorkspaceId }: WorkspaceRoadmapPageProps) {
  ...
  return (
    ...
    >
        ...
        <PmSection index={0} className="flex min-h-0 flex-1 flex-col gap-4">
          {gate === "loading" ? (
            ...
            <EmptyState
              ...
              action={{ label: "Add roadmap item", onClick: () => setCreateOpen(true) }}
            />
          ) : (
      ...
      {createOpen ? <RoadmapItemSheet onClose={() => setCreateOpen(false)} /> : null}
      {editTarget ? <RoadmapItemSheet item={editTarget} onClose={() => setEditTarget(null)} /> : null}
