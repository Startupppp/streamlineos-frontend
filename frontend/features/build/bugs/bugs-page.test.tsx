...
const mockUseDeleteBug = jest.fn();
const mockUseProjectMembers = jest.fn();
const mockUseCan = jest.fn();
const mockUseAccess = jest.fn();

jest.mock("@/hooks/api/build/bugs", () => ({
  useBugs: (...args: unknown[]) => mockUseBugs(...args),
  useDeleteBug: () => mockUseDeleteBug(),
}));
...
beforeEach(() => {
  ...
  mockUseBugs.mockReturnValue(baseQuery({ data: [] }));
  mockUseProjectMembers.mockReturnValue(baseQuery({ data: [] }));
  mockUseDeleteBug.mockReturnValue({ mutate: jest.fn(), isPending: false });
});

it("renders NoPermissionState when build:bugs:view is denied instead of the no-bugs empty state", () => {
  mockUseAccess.mockReturnValue(ACCESS_DENIED);
  mockUseBugs.mockReturnValue(baseQuery());
  render(<BugsPage projectId={1} />);
...
it("renders the upgrade path the backend sent with a 402 rather than a generic failure", () => {
  ...
  mockUseBugs.mockReturnValue(baseQuery({ isError: true, error: err }));
  render(<BugsPage projectId={1} />);
