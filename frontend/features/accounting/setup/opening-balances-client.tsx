...
import type { OpeningBalanceLineInput } from "@/types/accounting/accounting-kernel-ext";
  ...
  function handlePost(): void {
    postBalances.mutate(
      { asOfDate, lines },
      {
        onSuccess: (journal) => {
          toast.success(
            `Opening balances recorded as ${journal.journalNumber}`,
          );
          setAmounts({});
        },
        onError: (error) => toast.error(getErrorMessage(error)),
  ...
  if (bookQuery.isPending || accountsQuery.isPending) {
    return (
      <PageWrapper
        ...
        subtitle="Where the business stood when the books opened"
      >
      ...
      <div className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Enter a positive figure for something you own, and a negative one
              for something you owe. Anything left over becomes your accumulated
              earnings, so you do not have to work that out yourself.
            </CardDescription>
          </CardHeader>
        ...
        <Card className="overflow-hidden">
          <CardHeader>
            ...
            <CardDescription>
              Leave an account blank if it had nothing on it.
            </CardDescription>
          </CardHeader>
                      ...
                      >
                        ...
                        <td className="px-4 py-2 text-right">
                          <Input
                            ...
                            onChange={(event) =>
                              handleAmountChange(account.id, event.target.value)
                            }
            ...
            <div className="flex flex-wrap gap-6 font-mono tabular-nums">
              <div>
                ...
                <p className="text-lg font-semibold">
                  {formatMinorMoney(totals.debit, currency)}
                </p>
              </div>
              <div>
                <p className="text-dense text-muted-foreground">Owed</p>
                <p className="text-lg font-semibold">
                  {formatMinorMoney(totals.credit, currency)}
                </p>
              </div>
              <div>
        ...
        {preview.data ? (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Dated {preview.data.journalDate}, the day before your books
                open, so it never shows up as activity in your first month.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-1 text-sm">
              ...
              {preview.data.alreadyPosted ? (
                <p className="font-medium text-status-warning-ink">
                  Opening balances have already been recorded for this book.
                  Recording them again will return the original entry rather
                  than adding to it.
                </p>
              ) : null}
            </CardContent>
