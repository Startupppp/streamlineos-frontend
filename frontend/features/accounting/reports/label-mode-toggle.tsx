...
export function LabelModeToggle({ value, onValueChange, className }: LabelModeToggleProps) {
  function handleChange(next: string) {
    onValueChange(next === "accountant" ? "accountant" : "founder");
  }

  return (
    <Tabs value={value} onValueChange={handleChange} className={cn("shrink-0", className)}>
