    ...
    >
      <div className="flex flex-wrap items-center gap-x-6 gap-y-1 font-mono text-label tabular-nums">
        <span>
          ...
          <span className="font-semibold">{formatMoney(totals.debitMinor, currency)}</span>
        </span>
        <span>
          <span className="text-muted-foreground">Credits</span>{" "}
          <span className="font-semibold">{formatMoney(totals.creditMinor, currency)}</span>
        </span>
      </div>
      <p className={cn("text-label font-semibold", tone.ink)}>
        {totals.balanced
          ? "This entry balances."
          : `Out by ${formatMoney(Math.abs(totals.differenceMinor), currency)} — the two sides must match before it can be posted.`}
      </p>
    </div>
  );
}
