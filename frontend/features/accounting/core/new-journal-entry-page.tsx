...
import { journalEntrySchema, type JournalEntryFormValues } from "./new-journal-entry-schema";
...
export function NewJournalEntryPage() {
  ...
  function handleDebitChange(index: number, value: string): void {
    ...
    if (value && Number(value) > 0) form.setValue(`lines.${index}.credit`, "");
  }

  function handleCreditChange(index: number, value: string): void {
    form.setValue(`lines.${index}.credit`, value);
    if (value && Number(value) > 0) form.setValue(`lines.${index}.debit`, "");
  }
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="sm:col-span-3">
                    ...
                    <FormControl>
                      <Textarea id="entry-description" {...field} rows={2} placeholder="What does this entry record?" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
