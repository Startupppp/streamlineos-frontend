    ...
    <Card>
      <CardHeader className="px-4 py-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <CardTitle className="text-sm font-semibold">What this bill comes to</CardTitle>
          <SemanticBadge
      ...
      <CardContent className="space-y-3 p-4 pt-0">
        ...
        <div className="space-y-1 border-t border-border pt-2">
          <div className="flex items-center justify-between gap-3 py-1">
            <span className="text-label text-muted-foreground">Their bill number</span>
            <span className="text-sm">{document.vendorDocumentNumber ?? "—"}</span>
          </div>
          <div className="flex items-center justify-between gap-3 py-1">
            <span className="text-label text-muted-foreground">Dated</span>
            <span className="text-sm tabular-nums">
              {document.vendorDocumentDate ? formatShortDate(document.vendorDocumentDate) : "—"}
            </span>
          </div>
          <div className="flex items-center justify-between gap-3 py-1">
        ...
        {document.reverseCharge ? <FlagExplainer text={REVERSE_CHARGE_EXPLAINER} /> : null}
        {document.blockedInputTax ? <FlagExplainer text={BLOCKED_INPUT_TAX_EXPLAINER} /> : null}
      </CardContent>
    </Card>
  );
}
