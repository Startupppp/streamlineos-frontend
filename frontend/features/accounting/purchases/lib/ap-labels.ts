...
export const AP_PAYMENT_STATUS_LABELS: Readonly<Record<ApPaymentStatus, string>> = {
  POSTED: "Sent",
  REVERSED: "Reversed",
};

export const AP_PAYMENT_STATUS_TONES: Readonly<Record<ApPaymentStatus, BadgeTone>> = {
  POSTED: "success",
  REVERSED: "danger",
};
...
export const TAX_CATEGORY_OPTIONS: ReadonlyArray<{ value: TaxCategory; label: string }> =
  TAX_CATEGORY_ORDER.map((value) => ({ value, label: TAX_CATEGORY_LABELS[value] }));
...
export function withholdingExplainer(withheldMinor: number): string {
  if (withheldMinor === 0) return "Nothing is being withheld, so the vendor receives the full amount.";
