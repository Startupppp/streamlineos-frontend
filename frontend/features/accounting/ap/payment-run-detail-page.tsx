...
export function PaymentRunDetailPage({ runId }: PaymentRunDetailPageProps) {
  const query = usePaymentRun(runId);
  const canApprove = useCan("accounting:payment-runs:approve");
  const canManage = useCan("accounting:payment-runs:manage");

  const approveMutation = useApprovePaymentRun(runId);
  const executeMutation = useExecutePaymentRun(runId);
  const cancelMutation = useCancelPaymentRun(runId);
  ...
  const columns: DataTableColumn<PaymentRunItem>[] = [
    ...
    },
    {
      ...
      cell: (item) => (
        <ItemAmountCell item={item} runId={runId} isDraft={isDraft} />
      ),
    },
    {
      key: "status",
          ...
          <>
            ...
            <DataTable
              ...
              emptyState={
                <EmptyState
                  compact
                  title="No items in this payment run"
                  description="Add vendor bills to this run to process payments."
                />
              }
            />
          </>
