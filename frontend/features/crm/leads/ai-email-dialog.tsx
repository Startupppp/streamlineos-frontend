...
export function AIEmailDialog({
  ...
  const handleGenerate = useCallback(() => {
    if (!featureEnabled) {
      toast.error(
        `AI email drafting requires the ${requiredPlan ?? "PROFESSIONAL"} plan. Upgrade to unlock.`,
      );
      return;
    }
  ...
  const handleContextChange = useCallback(
    (e: React.ChangeEvent<HTMLTextAreaElement>) => setContext(e.target.value),
    [],
  );
