...
function buildClaimColumns(canManage: boolean, onReview: (claim: InsuranceClaim) => void): DataTableColumn<InsuranceClaim>[] {
  const columns: DataTableColumn<InsuranceClaim>[] = [
    ...
    { key: "amount", header: "Amount", cell: (claim) => <span className="text-sm font-medium">₹{(claim.amountCents / 100).toLocaleString("en-IN")}</span>, sortable: true, sortValue: (claim) => claim.amountCents },
    {
      key: "status",
      header: "Status",
      cell: (claim) => {
...
export function BenefitsClaimsTab({ canManage }: BenefitsClaimsTabProps) {
  ...
  if (isError) {
    return <ErrorState title="Couldn't load insurance claims" description="Failed to load claims. Please try again." onRetry={handleRetry} className="flex-1" />;
  }
