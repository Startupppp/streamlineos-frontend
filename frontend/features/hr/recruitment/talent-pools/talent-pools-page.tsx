...
function AddMemberSheet({ poolId }: { poolId: number }) {
  ...
  function handleCancel(): void { setOpen(false); }
  function handleCandidateChange(v: string): void { setCandidateId(v); }
...
function PoolButton({ pool, isSelected, onSelect }: PoolButtonProps) {
  function handleClick(): void { onSelect(pool.id); }
  return (
    <button
      type="button"
      onClick={handleClick}
...
function PoolMemberRow({ member: m, onRemove }: PoolMemberRowProps) {
  function handleRemoveClick(): void { onRemove(m.candidateId); }
  return (
...
function PoolMembersList({ poolId }: { poolId: number }) {
  ...
  function handleRemove(candidateId: number): void {
...
export function TalentPoolsPage() {
  ...
  function handleSelectPool(poolId: number): void { setSelectedPoolId(poolId); }
  function handleDeleteSelectedPool(): void { if (selectedPoolId !== null) setDeleteTarget(selectedPoolId); }
  function handleCloseDeleteDialog(open: boolean): void { if (!open) setDeleteTarget(null); }
