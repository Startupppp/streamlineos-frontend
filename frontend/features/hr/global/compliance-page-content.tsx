...
function WorkAuthStatusBadge({ status }: { status: WorkAuthorization["status"] }) {
  if (status === "expired") return <Badge className="bg-red-50 text-red-700 border-red-200">Expired</Badge>;
  if (status === "expiring") return <Badge className="bg-amber-50 text-amber-700 border-amber-200">Expiring</Badge>;
  if (status === "pending_renewal") return <Badge className="bg-blue-50 text-blue-700 border-blue-200">Pending renewal</Badge>;
