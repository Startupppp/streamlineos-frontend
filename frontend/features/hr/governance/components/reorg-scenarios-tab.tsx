      ...
      {selectedScenarioId && (
        <div className="mb-4 border border-blue-200 bg-blue-50 rounded-lg p-4">
          <div className="flex items-center justify-between mb-2">
            <h3 className="font-semibold text-sm text-blue-900">Simulation Preview</h3>
            <Button variant="ghost" size="sm" onClick={handleCloseSimulation} className="text-blue-700 h-6 px-2">
              Close
            </Button>
          </div>
          {simLoading ? (
            ...
            <div className="space-y-2 text-sm text-blue-800">
              ...
              <p className="text-xs text-blue-600 mt-2 italic">{simulation.warning}</p>
            </div>
          ) : null}
        </div>
      )}
