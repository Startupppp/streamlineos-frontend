...
export function HuddleChatPanel({
  ...
  return (
    <div className="flex flex-col max-h-56 w-full rounded-xl border border-border/60 bg-muted/30 overflow-hidden">
      <div className="h-10 px-3 border-b border-border/60 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          <MessageSquare className="h-4 w-4 text-muted-foreground" />
          <h3 className="text-label font-semibold text-foreground">Huddle chat</h3>
        </div>
        <button
          type="button"
          onClick={onClose}
        ...
        >
          <XIcon ref={closeIconRef} size={14} className="text-muted-foreground" />
        </button>
      </div>
      ...
      <ScrollArea hideScrollbar className="min-h-0 flex-1">
        ...
        {messages.map((msg) => (
          ...
          >
            <span className="text-micro text-muted-foreground mb-0.5">{msg.name}</span>
            <div
              className={cn(
      ...
      <div className="shrink-0 p-3 border-t border-border/60">
        <div className="flex items-center gap-2 bg-card border border-border/60 rounded-xl px-3 py-2">
          <input
            ...
            placeholder="Message…"
            aria-label="Huddle chat message"
          ...
          <button
            ...
            aria-label="Send"
          >
            <SendIcon ref={sendIconRef} size={12} className="text-primary-foreground" />
          </button>
        </div>
      </div>
    </div>
