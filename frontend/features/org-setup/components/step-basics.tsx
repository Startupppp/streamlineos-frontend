          ...
          {GOALS.map((goal) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "flex items-center gap-1.5 p-2 rounded-lg border text-left text-[11.5px] font-medium transition-colors",
                  selected ? "border-blue-500 bg-blue-50 text-blue-700" : "border-border bg-card text-foreground hover:border-blue-300",
                )}
              >
                ...
                {selected && <Check className="h-3 w-3 text-blue-600 ml-auto shrink-0" />}
              </button>
            );
          })}
        </div>
