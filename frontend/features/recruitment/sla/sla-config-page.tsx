...
export function SlaConfigPage() {
  ...
  const handleSave = useCallback(() => {
    ...
    if (!form.stage) { toast.error("Stage is required"); return; }
    if (!maxHours || maxHours <= 0) { toast.error("Max hours must be a positive number"); return; }
    if (!warningHours || warningHours <= 0) { toast.error("Warning hours must be a positive number"); return; }
    if (warningHours >= maxHours) { toast.error("Warning hours must be less than max hours"); return; }
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <label className="text-sm font-medium">Warning After (hours)</label>
              <Input
                type="number"
                min={1}
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium">Maximum Hours (Breach Threshold)</label>
              <Input
                type="number"
                min={1}
                value={form.maxHours}
