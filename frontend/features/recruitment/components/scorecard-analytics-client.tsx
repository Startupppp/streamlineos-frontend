...
import { useScorecardAnalytics } from "@/hooks/api/hr/recruitment/scorecard-analytics";

const ScorecardCharts = dynamic(
  () => import("@/features/recruitment/components/scorecard-charts").then((m) => ({ default: m.ScorecardCharts })),
  {
    ssr: false,
    loading: () => (
...
export function ScorecardAnalyticsClient() {
  const [days, setDays] = useState("90");
  const { data, isLoading, isError, refetch } = useScorecardAnalytics({ days });
