        ...
        <div className="flex items-start gap-4">
          ...
          >
            <Clock
              className={cn(
                "h-4 w-4",
                schedule.isEnabled ? "text-primary" : "text-muted-foreground",
              )}
            />
          </div>
          ...
          <div className="flex items-center gap-1 shrink-0">
            <Button
              ...
              disabled={isToggling}
              aria-label={
                schedule.isEnabled ? "Pause schedule" : "Activate schedule"
              }
            >
              {schedule.isEnabled ? (
...
export function SchedulerPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<WorkflowSchedule | null>(
    null,
  );
  ...
  function handleToggle(schedule: WorkflowSchedule) {
    ...
    updateSchedule.mutate(
      ...
      },
      {
        onSuccess: () =>
          toast.success(
            schedule.isEnabled ? "Schedule paused" : "Schedule activated",
          ),
