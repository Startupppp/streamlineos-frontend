...
const FieldValueInput = memo(function FieldValueInput({ field, currentValue, onSave }: FieldValueInputProps) {
  ...
  if (field.type === "select" && field.options?.length) {
    return (
      ...
      >
        <SelectTrigger className="h-8 text-xs border-0 bg-muted hover:bg-accent w-auto min-w-[120px]">
  ...
  return (
    <Input
      ...
      className="h-8 text-xs border-0 bg-muted hover:bg-accent focus:bg-background max-w-[200px]"
    />
  );
});
