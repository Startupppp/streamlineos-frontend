...
function CoverageNote({ shown, total, noun }: { shown: number; total: number; noun: string }) {
  ...
  return (
    <p className={cn("rounded-md border p-3 text-xs", warning.surface, warning.rule, warning.ink)}>
      Showing {shown} of {total} {noun}. The API returns the first 100 rows of a job, so anything numbered beyond that
      is not in this list.
    </p>
  );
}
...
export function BulkSendJobSheet({ jobId, templateName, open, onOpenChange }: BulkSendJobSheetProps) {
  ...
  const failedRows = errorReport.data ?? [];
          ...
          <Tabs defaultValue={job.failedCount > 0 ? "failed" : "all"}>
            ...
            <TabsContent value="failed" className="space-y-3">
              {errorReport.isLoading ? (
                ...
                <>
                  ...
                  <CoverageNote shown={failedRows.length} total={job.failedCount} noun="failed rows" />
            ...
            <TabsContent value="all" className="space-y-3">
              {rows.length === 0 ? (
                ...
                <>
                  <CoverageNote shown={rows.length} total={job.totalCount} noun="rows" />
                  <ul className="space-y-2">
                    {rows.map((row) => (
                      <RowCard key={row.id} row={row} mapping={mapping} />
                    ))}
