...
| Cursor (keyset) pagination | `useCursorPagination` — `hooks/common/use-cursor-pagination.ts`, paired with `DataTable`'s `pagination={{ mode: "cursor", … }}`. For a list the server pages by cursor (`hasMore` + `nextCursor`, no `total`) — the inventory ledger and its audit trail. A list that can report a real total stays on `mode: "server"`. |
