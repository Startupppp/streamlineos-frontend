...
**What a gated surface RENDERS is decided by `<PageState resolution={usePageState(...)}>`, never by a boolean.** `usePageState` (`hooks/api/use-page-state.ts`) → `PageState` (`components/shared/page-state.tsx`) is the one owner of the branch order: access-loading → module unavailable → denied → loading → error → empty → ready. Worked example `features/build/cycles/cycles-page.tsx`.
...
- **`error` is the whole 402 story.** `ModuleGuard` and `PermissionGuard` throw `ModuleDisabledException` — **402**, `code: "MODULE_NOT_ENABLED"`, `details: { moduleKey, reason, upgradePath }` — so the backend has already said *which* module, *why*, and *where to upgrade*. `resolvePageState` turns that into `plan-required` / `module-disabled` / `quota-exceeded` / `feature-locked`, and a 403 into `denied` carrying the backend's own message. **Omit `error` and every one of those becomes "Something went wrong" — the page still compiles and its tests still pass.** Measured 2026-09-20: fourteen Build pages did exactly this.
- **Never `useCan` for a page's own state.** `useCan` is `false` while the access snapshot is in flight, so `if (!canView) return <NoPermissionState/>` tells a user who *holds* the permission that access is restricted, until their rights land. `useCan` is for `enabled:` on a query, and for controls (below). `useCanState` is the three-valued answer when you need one outside `usePageState`.
- **The inverse for a mutation control: fail closed, so `useCan`.** Hide a create/edit/delete/approve control until access is *known*. `useCanState(key) !== "denied"` shows it during loading — a destructive button that appears and then vanishes offers authority the caller may not hold, which is worse than one that appears a beat late. The asymmetry is the point: a *surface* must not claim denial before it knows; a *control* must not claim authority before it knows.
- A disabled TanStack Query v5 read is `isPending: true, isFetching: false`, so `isLoading` is **false** — identical to an empty list. Branching on `isLoading`/`isError` alone is what makes a refusal read as "you have nothing". `lib/rbac/denial-is-not-emptiness.known.json` is the ratchet of unconverted surfaces: **it may only shrink**, and a surface not on it that reads a gated hook and renders an empty state fails the build.

**Destructive and lifecycle confirmations reuse `ConfirmDialog`** (`components/ui/confirm-dialog.tsx`) — never a hand-rolled `AlertDialog` + red `AlertDialogAction`. It owns the trigger-XOR-controlled union, `destructive`, `isPending`, and the pending-close guard, so a hand-rolled copy silently loses them. `HierarchyArchiveDialog` and the typed-name confirm (`features/build/sidebar/delete-project-dialog.tsx`, a validated form rather than a message) are the sanctioned variants; raw `AlertDialog` primitives are for building those, not for a delete prompt.
...
| **AP-9** | `if (!useCan(key)) return <NoPermissionState/>`, or `isError ? <ErrorState/>` as a page's whole failure branch | a page's state is `<PageState resolution={usePageState({ permission, isLoading, isError, error })}>`; the boolean flashes denial at permitted users and a raw `isError` discards the backend's 402 upgrade path (§2) |
| **AP-10** | a hand-rolled `AlertDialog` + destructive `AlertDialogAction` as a delete prompt | `ConfirmDialog destructive` (§10, §13) |
...
- [ ] Page state resolves through `usePageState` + `<PageState>` **with `error` passed**, so a 402 shows the plan/module denial the backend sent; mutation controls gate on `useCan` (fail closed); destructive prompts are `ConfirmDialog destructive`
...
| States | `EmptyState` — `components/ui/empty-state.tsx` · `ErrorState`, `LoadingState`, `AccessDenied`, `NoPermissionState` — `components/shared` · `AnonymitySuppressedNotice`, `anonymitySuppressedMessage` — `components/shared/anonymity-suppressed-notice.tsx` (an anonymised aggregate the backend answered `suppressed`/below `minResponses`: renders "Fewer than N responses — hidden to protect anonymity", never an empty chart; polls, mood trend, survey question analytics and the wellness pulse all use it) |
...
| Presence / availability | `PresenceDot`, `AvatarWithPresence` — `components/shared/presence-dot.tsx` · `PresenceStatusPicker` — `components/shared/presence-status-picker.tsx` · `usePresenceSelection` — `hooks/common/use-presence-selection.ts` · `usePresenceMap`, `usePresenceCustomStatus` — `hooks/api/chat-core-read.ts` · `PRESENCE_STATUSES`, `PRESENCE_CLEAR_AFTER_OPTIONS`, `PRESENCE_CLEAR_AFTER_LABELS`, `PRESENCE_STATUS_MESSAGE_MAX_LENGTH`, `presenceLabel`, `presenceDotClass`, `isPresenceStatus`, `isPresenceClearAfter` — `lib/presence.ts` |
...
- A workflow intentionally in two products renders ONE shared `features/` component through thin adapters with the route's base path passed in (`/directory/*` in Home vs `/directory/settings/*` in Administration) — never duplicate the page or import one `app/**/page.tsx` from another.
