...
|---|---|---|
| **S-16 RLS migration applied to production** | The production migration ledger holds 16 rows against 1,121 files; `db:migrate` would replay ~1,105. The migration is authored and journaled here; applying it is a separate, explicitly-authorised operation. | Decide when to apply |
...
| **S-16** | `1123_ai_action_proposals_rls.sql` enables RLS with a `tenant_isolation` policy, revokes PUBLIC, grants `streamline_app`, and verifies itself in a `DO` block. Registered in `_journal.json` (idx 1011) — a `.sql` outside the journal never runs while `db:migrate` still prints success. **Authored, not applied** (§5.2). |
