...
| ~~F-06~~ | **Closed 2026-09-21.** `global-ask-os.tsx:219-220` appends rather than overwriting the ref, and the renderer maps N cards. No wire-format change was needed. |
| F-07 | The confirm card does not survive a reload. ⚠ **The stated reason above was wrong** — the N-card renderer arrived with F-06 and the card *does* reappear. What is still broken is narrower and different: the reloaded card renders `mode="record"`, i.e. read-only, because the token is stripped at persist time (`streaming/ask-os-directive.ts:43`). The real work is a decline endpoint plus a token-bearing reload path, not a renderer. `cancelProposal` was deleted, so a live proposal dangles until expiry. |
| F-10 | Message-list windowing (`ask-os-chat-view.tsx:112` maps every row). ⚠ **The stated reason was false:** `react-window` is **not** unused — it has 8 production consumers, so this would not be the repo's first virtualized list. Follow the established pattern in `features/notifications/unified-inbox/inbox-virtual-list.tsx`, `features/mail/mail-virtual-list.tsx` or `features/chat/chat-user-virtual-list.tsx`; the variable-height/scroll-anchoring concern is real but already solved in-repo. The list still grows only when the user deliberately pages back at 30/page. |
| M6 | ⚠ **Restated 2026-09-21 — do NOT write this migration; it would fail `42710`.** It is true that no migration file creates `uniq_ai_action_proposals_org_id` (`src/db/schema/ai/ai-confirmation.ts:37`). It is **not** true that the constraint is missing: production already has it, `contype = 'u'`, `convalidated = true`, with its index, on a 10-row table with zero duplicate `(org_id, id)` pairs — because production was built by push, not by the chain. The `db:generate` rationale is also moot, since `backend/CLAUDE.md` §3 says `db:generate` is not usable on this schema. **What remains is cold-rebuild divergence** — a database built from migrations would lack a constraint production has. That belongs with the 869 below-watermark entries, not in this PRD. |
