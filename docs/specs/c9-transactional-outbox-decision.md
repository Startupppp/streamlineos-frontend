...
**Status: decision and relay mechanics implemented; notification-intent and external-effect ledgers intentionally retained as distinct semantic ledgers.** Verified at source 2026-08-27. The generic `outbox_events` relay is enabled by default, tenant-scoped, retrying, dead-lettering, observable, fenced by `InboxConsumer`, and callable through the leased platform worker endpoint. Chat external effects use the deployed, lease-fenced `external_effect_ledger`; its current report has zero effect rows, and provider-enforced exactly-once remains an operational gate. The decision and role split are recorded in [`c9-outbox-decision.md`](c9-outbox-decision.md); partitioning is an explicit threshold-triggered follow-up.
