...
**Status: seam and selected rollout locally verified.** Re-audited at source 2026-08-26. The authenticated adapter, access hydration, five selected authenticated route patterns, public Help Centre separation, hydration/fallback tests, production build, and route verifier are present. The verifier parses route calls to enforce permission-before-prefetch ordering and requires hydration/fallback coverage for every selected route. The two public Help Centre routes also have executable first-server-HTML tests. Live authenticated browser proof against a deployed session remains an operational gate.

**Current audit (2026-08-26):** the authenticated server adapter is implemented, access is hydrated at the authenticated layout, and the selected high-value route set covers workers, payroll runs, roles, HR assets, and HR documents. Each selected route has a permission-ordering test plus hydrated-row/no-fetch and empty-hydration/fallback coverage. The public Help Centre intentionally uses `publicGet` as a separate unauthenticated adapter; its landing and article routes are invoked as real async route components and serialized to HTML with backend responses mocked locally, proving their records are present before browser hydration. This does not replace a deployed authenticated-session capture showing protected rows in the network response body.

**Local evidence command:** `pnpm verify:server-data-seam` checks the built route manifest, authenticated/public adapter separation, AST-derived auth-before-prefetch ordering, and the existence of both hydration and fallback tests for all five authenticated routes. The focused c8 Jest matrix additionally runs `lib/prefetch/public-help-first-html.test.tsx`, all five route-ordering suites, and all five hydrated-row suites.
