...
| `modules/build/governance/risks.service.ts`, `decisions.service.ts` | **Verified fixed 2026-09-25** — both list services use cursor pagination with a `PAGE_LIMIT + 1` fetch and cursor response, and both collection pages pass the cursor result to `DataTable` pagination. |
| `features/build/governance/risks-page.tsx` | **Verified fixed 2026-09-25** — risk tiles and the matrix use the unfiltered risk-stats endpoint, while the status filter only narrows the table; `risks-page-aggregates.test.tsx` covers the Closed-filter case. |
