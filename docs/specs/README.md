...
| # | Candidate | Closed by |
|---|---|---|
| [c1](c1-kb-visibility-seam.md) | KB visibility predicate as a seam | Implemented: lifecycle-only indexing, bounded resumable backfill with explicit apply guard, seeded direct/keyword/vector parity coverage, and denial-of-wallet tests; live seeded E2E harness requires a full application test environment |
...
| [c5](c5-payment-provider-adapter.md) | Route the money through the adapter | All code-level criteria met: import-boundary spec enforces the seam at CI, `FakeProviderAdapter` covers order creation, wrong-signature no-write, forged webhook no-state-change, provider substitution (razorpay + stripe fake, same outcome), idempotent replay, and cross-tenant event-id collision; legacy Razorpay controller routes through the provider-neutral handler. Two permanent production-only gaps remain — live webhook delivery from Razorpay and provider-side duplicate suppression — both are external operational concerns identical in kind to c7/c9, not code defects |
