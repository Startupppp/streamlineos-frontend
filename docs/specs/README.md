...
| [c2](c2-calendar-source-registry.md) | Calendar source seam | Implemented: granular source adapters, production registration coverage, preference-isolation coverage, and a tenant-scoped read-only 44-organization integrity sweep; live HTTP/controller proof remains |
...
| [c6](c6-split-kb-help-centre-and-wiki.md) | Split the two products inside `kb/` | Implemented: split, namespace, permanent conversion-tool policy, failure-safe report, guarded conversion CLI, and live 44-tenant report; one published article remains migratable |
...
| [c9](c9-transactional-outbox-decision.md) | Decide what the outbox is for | Decision, leased worker, retry/fencing, complete report, stable effect keys, deployed durable external-effect ledger, and 44-tenant evidence implemented; six pending rows, provider-side dedupe, and fault-injection evidence remain |
