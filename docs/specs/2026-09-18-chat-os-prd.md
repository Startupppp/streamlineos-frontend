...
- [ ] **P7-1** Boot backend + frontend against a **named disposable/scratch** environment. Never load production credentials to make a test pass.
  ✖ **Blocked on this machine, established by probe rather than assumed (2026-09-20).** `backend/.env.localstack` already names the right target — `127.0.0.1:5432/scratch_local`, loopback and disposable — but nothing is listening on 5432 (`ECONNREFUSED`), there is no PostgreSQL service or install (`/c/Program Files/PostgreSQL` absent, no `psql`/`pg_ctl`/`initdb` on `PATH`), and no Docker. So the environment is configured but cannot be started here.
  ⚠ **Production is not a substitute, for two independent reasons.** This item's own acceptance forbids it. And `backend/.env` authenticates as `streamline_admin`, which holds `BYPASSRLS` — P7-2 requires a non-owner precisely because owner bypass masks non-owner 403s, so running the suite that way would report green without testing anything. Production *was* used, correctly, for the read-only evidence recorded against R-4 below.
