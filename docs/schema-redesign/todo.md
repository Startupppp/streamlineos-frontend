...
- [ ] **R-11 [SCHEMA]** Move organization-specific employee/payroll fields from global `users` to canonical org-scoped records, then retire legacy columns after dual-write parity. Migrations `0392_payroll_worker_subject` and `0393_payroll_lifecycle_worker_subject` establish worker-subject support for part of payroll, but global `users` fields and live readers remain; this is partial progress, not a completed cutover. The audited legacy tenant-scoped set includes `joining_date`, `tax_id`, `bank_details`, `org_department_id`, `designation`, `monthly_salary`, `employee_id`, `reporting_to`, `branch_id`, employment lifecycle timestamps/status, emergency contact, and onboarding employment state. `UsersService.updateUser` now atomically dual-writes the two unambiguous fields—designation and department—to the existing primary `hr_employments` row inside the tenant transaction and records `canonicalEmploymentSynced` parity telemetry; it never invents missing canonical records. The inventory production module has zero reads of these legacy fields. The reviewed field contract and read-only parity verifier live in `docs/schema-migration/hr-users-canonical-field-map.md` and `hr-users-canonical-parity-preflight.sql`: exact mappings cover person identity/contact, primary-employment employee number/designation/joining date/department, salary cents, and tax ID. Bank text-to-JSON, effective-dated reporting lines, branch/location, emergency-contact key normalization, lifecycle/onboarding semantics, and currently non-atomic bulk writers remain explicitly blocked rather than guessed. Do not drop legacy columns until all approved writers are dual-written, read cutover and mismatch telemetry remain clean, per-organization backfill reconciliation passes, and rollback is rehearsed.
