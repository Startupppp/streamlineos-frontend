...
- [ ] **R-11 [SCHEMA]** Move organization-specific employee/payroll fields from global `users` to canonical org-scoped records, then retire legacy columns after dual-write parity. Safe dual-write coverage now includes designation, department, and joining date into an existing tenant-correlated primary `hr_employments` row, plus monthly salary (validated integer cents) and encrypted tax ID into an existing `hr_employee_sensitive_fields` row. Missing canonical rows are never invented and produce audit parity telemetry. Employee detail reads designation, employee number, department, and joining date canonical-first with a migration fallback; statutory payroll resolves decrypted canonical tax ID/PAN first and decrypts the legacy fallback, so ciphertext cannot enter filings. An unused legacy salary projection was removed from payroll input building. `UserOpsService.bulkUpdateUsers` commits legacy user, canonical department, organization-unit, role, and structural-role changes in one transaction with set-based unit writes. The reviewed field contract, read-only parity verifier, and dry-run-by-default approved backfill/runbook live under `docs/schema-migration/`; the backfill fills only NULL designation/department/joining/salary values, is tenant-correlated and idempotent, and excludes tax because encryption keys must not be reproduced in SQL. Remaining important blockers are bank text-to-typed-JSON conversion, effective-dated reporting-line semantics, branch/location meaning, emergency-contact normalization, lifecycle/onboarding mapping, remaining read cutover, per-organization reconciliation, and rehearsed rollback. No legacy column may be dropped before those gates pass.
...
- [x] **R-14 [PERFORMANCE]** Redis request-path wildcard invalidation is fully removed: repository-wide source search returns zero `invalidatePattern(...)` references. Complete reader/writer families use tested O(1) versioned namespaces (`cachedVersioned` / `invalidateNamespace`), and cross-instance cache misses use a bounded, token-safe distributed fill lease with local single-flight as the first tier. Inventory regression coverage includes products, PO/GRN/SO, shipments, vendors, quality, jobs, counts, returns, stock/reservations, and traceability.
- [x] **R-15 [PERFORMANCE][IMPORTANT SCOPE]** High-impact request-path work is complete: batched reminders/chart seeding/contact import; referenced-account journal lookup; bounded streaming contact export; cursor-paged community/succession reads; O(1) cache invalidation; explicit public projections instead of secret-bearing global-user hydration; active-org correlation for direct reports and privileged-role joins; a single bounded auth organization-resolution query; a 100-row cap and no unused assignee join for AI CRM task suggestions; and set-based inventory cycle-count/physical-audit line updates instead of N sequential writes. Low-risk response-shape cleanup and arbitrary caps on established configuration APIs are intentionally excluded because they would add compatibility risk without measured benefit.
- [ ] **R-16 [VERIFY] (partial)** Frontend/backend Knip, typechecks, and isolated production builds pass on 2026-08-04. Full backend lint passes with zero warnings after dead test/import cleanup; focused migration/cache/contact/HR/timesheet/inventory/finance suites pass, including 6 inventory suites / 52 tests and the final cache suite / 3 tests. Remaining gates: full frontend lint, full test suites, schema cold rebuild, production-clone migration rehearsal, and load/soak/failover suites.
