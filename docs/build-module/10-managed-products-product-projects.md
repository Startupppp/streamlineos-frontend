...
- **Current/target route:** `/build/managed-products/[managedProductId]/projects`
...
- **Evidence:** `frontend/app/(authenticated)/build/managed-products/[managedProductId]/projects/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Browse and manage projects in the active scope.
- **Primary persona:** Project manager.
- **Success metric:** Project-open rate and time to locate a project.
- **Required density:** compact by default with a comfortable-density toggle.
- **Core fields:** name, key, status, manager, client, product, workspace, dates, health, progress.
...
Deep-linkable query parameters: `q`, `status`, `managerId`, `clientId`, `productId`, `workspaceId`, `health`, `sort`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/project-list/projects-page`
...
- **Endpoints:** GET /build; POST /build.
- **Client schema/hooks:** `frontend/hooks/api/build/projects.ts; build-project-schema.ts` where present.
...
- **Detail response:** `{ data: { name, key, status, manager, client, product, workspace, dates, health, progress, version, createdAt, updatedAt }, meta }`.
