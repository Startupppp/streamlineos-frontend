...
- **Current/target route:** `/build/[projectId]/reports`
- **Scope:** project
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** Explain flow, predictability, and schedule using drillable evidence.
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/reports/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Provide one trustworthy analytics owner.
- **Primary persona:** Project/program manager.
- **Success metric:** Metric drill-through and decision use.
- **Required density:** comfortable, with compact child collections.
- **Core fields:** overview, velocity, burnup, CFD, cycle/lead time, critical path, freshness.
...
Deep-linkable query parameters: `tab`, `from`, `to`, `cycleId`, `teamId`, `assigneeId`, `type`, `priority`, `status`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/reports/velocity-section`, `@/features/build/reports/burnup-section`, `@/features/build/reports/cfd-section`, `@/features/build/reports/critical-path-section`, `@/features/build/reports/cycle-time-section`, `@/features/build/reports/lead-time-section`
...
- **Endpoints:** report endpoints under /build/:projectId/reports.
- **Client schema/hooks:** `frontend/hooks/api/build/reports.ts; reports-schema.ts` where present.
...
- **Detail response:** `{ data: { overview, velocity, burnup, CFD, cycle/lead time, critical path, freshness, version, createdAt, updatedAt }, meta }`.
