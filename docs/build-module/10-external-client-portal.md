# External Client Portal Projects

## Route decision

- **Current route:** `/client-portal`
- **Scope:** external portal identity
- **Disposition:** **KEEP**
- **Decision:** Keep as the canonical external portal home.
- **User job:** See every project shared with me and nothing else.
- **Evidence:** `frontend/app/(portal)/client-portal/page.tsx`

## Product contract

- **Purpose:** List only projects explicitly shared with the current portal identity.
- **Primary persona:** External client
- **Success metric:** Successful project opens and zero cross-grant disclosure.
- **Core fields:** project name, state, summary, grant capabilities, last update.
...
Deep-linkable state: No filter parameters until the bounded list needs search; invitation/session reasons belong only on the auth route.. Selection, drafts, session credentials, open menus, and unsaved input do not belong in the URL.
...
| Build member | Only with valid portal token and active grant | Only explicit capability | Only explicit capability | No by default |
...
- Page/source: `frontend/app/(portal)/client-portal/page.tsx`.
...
- **Endpoints:** `GET /portal/v1/projects`.
- **Client evidence:** `frontend/hooks/api/portal/use-portal-projects.ts`, `frontend/hooks/api/portal/use-portal-guard.ts`.
...
- **P0:** Cards use `rounded-xl`, outside the Build 8 px maximum; production state was not mutation-tested.
