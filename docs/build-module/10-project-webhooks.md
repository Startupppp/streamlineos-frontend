...
- **Current/target route:** `/build/[projectId]/webhooks`
- **Scope:** project
- **Disposition:** **MOVE — EXECUTED 2026-09-22**
- **Decision:** The user job lives at `/build/[projectId]/settings/integrations/webhooks`. The physical page was deleted; the deep link is preserved by the `next.config.ts` redirect, which fires before the filesystem.
- **User job:** Integrate project changes with external systems reliably.
- **Evidence:** page deleted 2026-09-22; the job renders from `frontend/app/(authenticated)/build/[projectId]/settings/integrations/webhooks/page.tsx`. Deep link preserved by the `frontend/next.config.ts` redirect. Census: `docs/build-module/DEAD-BUILD-SURFACE-INVENTORY.md`.
...
- Existing feature evidence: `@/features/build/webhooks/project-webhooks-page`
