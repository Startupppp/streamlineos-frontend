...
- **Scope:** project
- **Disposition:** **MOVE**
- **Decision:** Migrate the user job to `/build/[projectId]/settings/integrations/webhooks`, preserve deep links temporarily, then remove this physical route.
- **User job:** Integrate project changes with external systems reliably.
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/webhooks/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.
