...
- **Current/target route:** `/build/programs`
- **Scope:** organization
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** Manage dependencies, milestones, risks, and status across projects.
- **Evidence:** `frontend/app/(authenticated)/build/programs/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Coordinate related projects toward one delivery outcome.
- **Primary persona:** Program manager.
- **Success metric:** Cross-project dependency resolution time.
- **Required density:** comfortable, with compact child collections.
- **Core fields:** name, owner, status, projects, milestones, dependencies, health.
...
Deep-linkable query parameters: `ownerId`, `status`, `health`, `projectId`, `q`, `sort`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/programs/programs-page`
...
- **Endpoints:** GET/POST /build/programs.
- **Client schema/hooks:** `frontend/hooks/api/build/programs.ts` where present.
...
- **Detail response:** `{ data: { name, owner, status, projects, milestones, dependencies, health, version, createdAt, updatedAt }, meta }`.
