# Public Shared Whiteboard

## Route decision

- **Current route:** `/board/[shareToken]`
- **Scope:** public share grant
- **Disposition:** **KEEP**
- **Decision:** Keep; capability scope must be encoded server-side, revocable, and independently rotatable.
- **User job:** Collaborate on the shared board without gaining broader project access.
- **Evidence:** `frontend/app/(public)/board/[shareToken]/page.tsx`

## Product contract

- **Purpose:** View or edit one explicitly shared whiteboard within token capabilities.
- **Primary persona:** External collaborator
- **Success metric:** Successful share opens and zero access beyond the shared board.
- **Core fields:** board snapshot, share capability, revision, expiry, attribution policy.
...
Deep-linkable state: No response-shaping query state; board viewport and selection are local. Selection, drafts, session credentials, open menus, and unsaved input do not belong in the URL.
...
| Build member | Only with valid unexpired share token with read or edit capability | Only explicit capability | Only explicit capability | No by default |
...
- Page/source: `frontend/app/(public)/board/[shareToken]/page.tsx`.
...
- **Endpoints:** `GET /public/whiteboard-links/:token`, `PATCH /public/whiteboard-links/:token`.
- **Client evidence:** `frontend/features/build/whiteboard/public-board-view.tsx`.
...
- **P0:** Production was not tested with a valid share token; concurrency, token rotation, rate limits, and stale-revision behavior need proof.
