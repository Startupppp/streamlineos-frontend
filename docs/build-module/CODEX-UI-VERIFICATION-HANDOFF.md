...
Also closed: the Intake list endpoint returned `{ items }` instead of the `{ data }` its own schema and the frontend both expected — root cause of the `/build/5/intake` crash named in the original baseline. Client-portal grant checks now enforce `expiresAt`, not just `status = "ACTIVE"`. `ApprovalsService.softDeleteApproval` now audit-logs, matching every sibling mutation.
