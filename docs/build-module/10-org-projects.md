...
- **Current/target route:** `/build`
- **Scope:** organization
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** Find the right project and understand its health before opening it.
- **Evidence:** `frontend/app/(authenticated)/build/page.tsx`; production rendered “All Projects” with filters, display controls, and a New Project action.

## Product contract

- **Purpose:** Browse and manage projects in the active organization or workspace scope.
- **Primary persona:** Project manager.
- **Success metric:** Project-open rate and median time to locate a project.
...
- **Client schema/hooks:** `frontend/hooks/api/build/projects.ts` and `build-project-schema.ts`.
