...
- **Current/target route:** `/build/client-access`
- **Scope:** organization
- **Disposition:** **MOVE — EXECUTED 2026-09-22**
- **Decision:** The user job lives at `/build/settings/client-access`. The physical page was deleted; the deep link is preserved by the `next.config.ts` redirect, which fires before the filesystem.
- **User job:** Grant least-privilege Build access and review it.
- **Evidence:** page deleted 2026-09-22; the job renders from `frontend/app/(authenticated)/build/settings/client-access/page.tsx`. Deep link preserved by the `frontend/next.config.ts` redirect. Census: `docs/build-module/DEAD-BUILD-SURFACE-INVENTORY.md`.

## Product contract

- **Purpose:** Administer Build access at organization scope.
- **Primary persona:** Organization administrator.
- **Success metric:** Overdue access review and unauthorized-fetch rate.
- **Required density:** comfortable, with compact child collections.
- **Core fields:** member, standing, permissions, scope, source, expiry, last review.
...
Deep-linkable query parameters: `q`, `role`, `permission`, `scope`, `state`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/portal-access/client-access-page`
...
- **Endpoints:** GET/POST /build/members; access endpoints.
- **Client schema/hooks:** `frontend/hooks/api/build/roster.ts; workspace-members.ts` where present.
...
- **Detail response:** `{ data: { member, standing, permissions, scope, source, expiry, last review, version, createdAt, updatedAt }, meta }`.
