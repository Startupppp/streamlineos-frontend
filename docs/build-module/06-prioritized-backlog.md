...
- **Evidence:** every screen exists today and the complete guided handoff does not — `/crm/quotes`, `/sign/envelopes`, `/timesheets/billing`, `/accounting/invoices` and `/accounting/payments-received` all resolve. Build now preserves the existing deal-to-project identity on retries in `backend/src/modules/build/core/projects-provision.service.ts`, and the merged `invoice_items.timesheet_entry_id` pointer carries approved-time traceability; quote-to-sign, sign-to-project, project-to-time, and payment handoffs remain open.
- **Effort:** 10–15 d
