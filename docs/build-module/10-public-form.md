...
- **Current route:** `/forms/[formToken]`
- **Scope:** public form grant
- **Disposition:** **KEEP**
- **Decision:** Keep as the canonical public form; forms own intake definitions and submissions.
- **User job:** Submit a structured request without needing an account.
- **Evidence:** `frontend/app/(public)/forms/[formToken]/page.tsx`

## Product contract

- **Purpose:** Render a published Build form and create one validated submission.
- **Primary persona:** External requester
- **Success metric:** Valid completion rate and spam/abuse rejection rate.
- **Core fields:** published title, description, ordered field definitions, validation, success response.
...
Deep-linkable state: No filter state; campaign attribution is allowed only through an explicit allowlist and must not alter authorization. Selection, drafts, session credentials, open menus, and unsaved input do not belong in the URL.
...
| Build member | Only with valid published form token; anonymous submit is rate-limited | Only explicit capability | Only explicit capability | No by default |
...
- Page/source: `frontend/app/(public)/forms/[formToken]/page.tsx`.
...
- **Endpoints:** `GET /public/forms/:token`, `POST /public/forms/:token/submit`.
- **Client evidence:** `frontend/hooks/api/build/public-form.ts`, `frontend/features/build/forms/public-form-api.ts`.
...
- **P0:** There is also `POST /public/build-forms/:publicToken/submissions`; consolidate the duplicate public submission contract and caller set.
