...
- **Current/target route:** `/build/[projectId]/files`
- **Scope:** project
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** Find the evidence attached to project work.
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/files/page.tsx`; Project 6 rendered only `Board`.

## Product contract

- **Purpose:** Expose project-linked files from the Files owner.
- **Primary persona:** Contributor.
- **Success metric:** Successful file retrieval and orphan-link rate.
- **Required density:** comfortable, with compact child collections.
- **Core fields:** name, type, size, owner, source record, version, updated.
...
Deep-linkable query parameters: `q`, `type`, `ownerId`, `sourceType`, `sort`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/files/files-page`
...
- **Endpoints:** files endpoints and signed download.
- **Client schema/hooks:** `frontend/hooks/api/build/project-files.ts; project-files-schema.ts` where present.
...
- **Detail response:** `{ data: { name, type, size, owner, source record, version, updated, version, createdAt, updatedAt }, meta }`.
