...
- **Purpose:** Configure event-driven project rules.
- **Primary persona:** Project administrator.
- **Success metric:** Successful run rate and manual time saved.
- **Required density:** compact by default with a comfortable-density toggle.
- **Core fields:** name, trigger, conditions, actions, owner, enabled, last run, failure.
...
Deep-linkable query parameters: `status`, `trigger`, `action`, `ownerId`, `q`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/automations/automations-page`
...
- **Client schema/hooks:** `frontend/hooks/api/build/automations.ts` where present.
...
- **Detail response:** `{ data: { name, trigger, conditions, actions, owner, enabled, last run, failure, version, createdAt, updatedAt }, meta }`.
