...
- **Current/target route:** `/build/workspaces/[pmWorkspaceId]/overview`
- **Scope:** workspace
- **Disposition:** **KEEP**
...
- **Evidence:** `frontend/app/(authenticated)/build/workspaces/[pmWorkspaceId]/overview/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.
...
- Existing feature evidence: `@/features/build/overview/workspace-overview-page`
...
- **Endpoints:** `GET /build/pm-workspaces/:pmWorkspaceId`, `GET /build/pm-workspaces/:pmWorkspaceId/members`, `GET /build/all-work?pmWorkspaceId=...`.
- **Client schema/hooks:** `frontend/hooks/api/build/pm-workspaces.ts`, `frontend/hooks/api/build/all-work.ts`.
