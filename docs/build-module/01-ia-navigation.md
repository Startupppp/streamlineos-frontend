...
| `/build/[projectId]/ai` | Propose cited project analysis and actions. | Project manager. | [CONSOLIDATE — EXECUTED](./10-command-center.md) | Job lives at `/build/command-center?projectId=...`; the physical page is gone and `next.config.ts` preserves the deep link. |
| `/build/[projectId]/analytics` | Provide one trustworthy analytics owner. | Project/program manager. | [CONSOLIDATE — EXECUTED](./10-project-reports.md) | Job lives at `/build/[projectId]/reports?tab=overview`; the physical page is gone and `next.config.ts` preserves the deep link. |
...
| `/build/[projectId]/automations` | Configure event-driven project rules. | Project administrator. | [MOVE — EXECUTED](./10-project-settings-automations.md) | Job lives at `/build/[projectId]/settings/automations`. Page deleted; the `next.config.ts` redirect preserves the deep link. |
...
| `/build/[projectId]/bugs` | Operate the canonical work-item collection. | Contributor and project manager. | [CONSOLIDATE — EXECUTED](./10-project-issues.md) | Job lives at `/build/[projectId]/issues?type=BUG`; the physical page is gone and `next.config.ts` preserves the deep link. |
...
| `/build/[projectId]/intake` | Review, accept, decline, or deduplicate incoming requests. | Product/project manager. | [KEEP](./10-project-intake.md) | Intake is the operational request queue; Forms owns form definitions and Triage owns broader evidence classification. All three are distinct live routes in the manifest. |
...
| `/build/[projectId]/my-tickets` | Unify work assigned to, reported by, or watched by the current actor. | Contributor. | [CONSOLIDATE — EXECUTED](./10-my-work.md) | Job lives at `/build/my-work?projectId=...`. Page deleted; the redirect moved into `next.config.ts`. |
...
| `/build/[projectId]/timeline` | Visualize dated issues and dependencies. | Project manager. | [CONSOLIDATE — EXECUTED](./10-project-issues.md) | Job lives at `/build/[projectId]/issues?view=timeline`; the physical page is gone and `next.config.ts` preserves the deep link. |
...
| `/build/[projectId]/views` | Summarize health and next actions for the active scope. | Project or product manager. | [CONSOLIDATE — EXECUTED](./10-project-issues.md) | Job lives at `/build/[projectId]/issues`; the physical page is gone and `next.config.ts` preserves the deep link. |
| `/build/[projectId]/webhooks` | Configure outbound project event delivery. | Project administrator. | [MOVE — EXECUTED](./10-project-settings-integrations-webhooks.md) | Job lives at `/build/[projectId]/settings/integrations/webhooks`. Page deleted; the `next.config.ts` redirect preserves the deep link. |
...
| `/build/client-access` | Administer client grants at organization scope. | Organization administrator. | [MOVE — EXECUTED](./10-settings-client-access.md) | Job lives at `/build/settings/client-access`. Page deleted; the `next.config.ts` redirect preserves the deep link. |
...
| `/build/customers` | Show CRM customers linked to delivery. | Client project manager. | [MOVE — EXECUTED](./99-kill-list.md) | CRM owns the customer master at `/crm`; Build keeps customer references on projects without a duplicate page. |
| `/build/drafts` | Collect actionable Build notifications and recover drafts. | Contributor. | [CONSOLIDATE — EXECUTED](./10-inbox.md) | Job lives at `/build/inbox?view=drafts`. Page deleted; the redirect moved into `next.config.ts` and the sidebar Drafts entry now links straight to the canonical URL. |
...
| `/build/members` | Administer Build access at organization scope. | Organization administrator. | [MOVE — EXECUTED](./10-settings-access.md) | Job lives at `/build/settings/access`. Page deleted; the `next.config.ts` redirect preserves the deep link. |
