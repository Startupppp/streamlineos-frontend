...
- **Current/target route:** `/build/workspaces/[pmWorkspaceId]/products`
- **Scope:** workspace
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** Understand progress, risk, and recent change without assembling a report.
- **Evidence:** `frontend/app/(authenticated)/build/workspaces/[pmWorkspaceId]/products/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Summarize health and next actions for the active scope.
- **Primary persona:** Project or product manager.
- **Success metric:** Action-through rate and status freshness.
- **Required density:** comfortable, with compact child collections.
- **Core fields:** health, progress, due work, risks, milestones, releases, activity.
...
- Existing feature evidence: `@/features/build/managed-products/managed-products-page`
...
- **Endpoints:** `GET|POST /build/managed-products` with `pmWorkspaceId`; `PATCH|DELETE /build/managed-products/:managedProductId`.
- **Client schema/hooks:** `frontend/hooks/api/build/managed-products.ts`, `frontend/hooks/api/build/managed-products-schema.ts`.
