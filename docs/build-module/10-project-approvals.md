...
- **Current/target route:** `/build/[projectId]/approvals`
- **Scope:** project
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** Make a traceable decision with enough source context.
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/approvals/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Review approval requests in scope.
- **Primary persona:** Approver.
- **Success metric:** Decision time and SLA breach rate.
- **Required density:** compact by default with a comfortable-density toggle.
- **Core fields:** request, type, source, requester, approvers, status, due/expiry, decision history.
...
Deep-linkable query parameters: `mine`, `status`, `type`, `actorId`, `from`, `to`, `q`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/approvals/project-approvals-page`
...
- **Endpoints:** GET/POST /build/:projectId/approvals.
- **Client schema/hooks:** `frontend/hooks/api/build/approvals.ts; approvals-schema.ts` where present.
...
- **Detail response:** `{ data: { request, type, source, requester, approvers, status, due/expiry, decision history, version, createdAt, updatedAt }, meta }`.
