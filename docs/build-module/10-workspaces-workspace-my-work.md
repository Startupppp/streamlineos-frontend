...
- **Current/target route:** `/build/workspaces/[pmWorkspaceId]/my-work`
- **Scope:** workspace
- **Disposition:** **CONSOLIDATE**
- **Decision:** Migrate the user job to `/build/my-work?pmWorkspaceId=...`, preserve deep links temporarily, then remove this physical route.
- **User job:** See and update all work that needs my attention.
- **Evidence:** `frontend/app/(authenticated)/build/workspaces/[pmWorkspaceId]/my-work/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Unify work assigned to, reported by, or watched by the current actor.
- **Primary persona:** Contributor.
- **Success metric:** Daily active use and overdue reduction.
- **Required density:** compact by default with a comfortable-density toggle.
- **Core fields:** key, title, project, status, priority, assignee, due, cycle, updated.
...
- Existing feature evidence: `@/features/build/my-work/my-work-page`
...
- **Endpoints:** GET /build/all-work?scope=mine.
- **Client schema/hooks:** `frontend/hooks/api/build/all-work.ts` where present.
...
- **Detail response:** `{ data: { key, title, project, status, priority, assignee, due, cycle, updated, version, createdAt, updatedAt }, meta }`.
