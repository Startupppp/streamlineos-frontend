...
Two migrations, both journalled, both with rollbacks, both carrying a verification `DO` block that raises rather than letting a silent no-op pass for success.
...
Its journal entry was removed with it and the remaining entry renumbered from 1038 to 1037, so the sequence has no gap. Both entries were new in this branch and unapplied anywhere, so renumbering them is not the renumbering of applied history that BE-59 forbids.
...
Neither migration has been executed. No database was available to this pass, so:
...
- No `EXPLAIN (ANALYZE, BUFFERS)` exists for 1154, so no speed claim is made for it anywhere.
