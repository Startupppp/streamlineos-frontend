...
Deep-linkable query parameters: `scope`, `owner`, `health`, `due`, `view`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
