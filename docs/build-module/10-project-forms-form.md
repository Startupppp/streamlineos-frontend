...
- **Current/target route:** `/build/[projectId]/forms/[formId]`
- **Scope:** project
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** Collect complete, routable requests from stakeholders.
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/forms/[formId]/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Design and publish structured intake.
- **Primary persona:** Project manager.
- **Success metric:** Valid-submission and conversion rate.
- **Required density:** comfortable, with compact child collections.
- **Core fields:** name, status, version, owner, fields, logic, submissions, automation.
...
Deep-linkable query parameters: `status`, `type`, `ownerId`, `q`, `sort`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/forms/form-detail-page`
...
- **Endpoints:** forms and submissions endpoints.
- **Client schema/hooks:** `frontend/hooks/api/build/forms.ts; forms-schema.ts` where present.
...
- **Detail response:** `{ data: { name, status, version, owner, fields, logic, submissions, automation, version, createdAt, updatedAt }, meta }`.
