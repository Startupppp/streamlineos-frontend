...
- **Current/target route:** `/build/[projectId]/backlog`
- **Scope:** project
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** Rank and refine ready work before commitment.
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/backlog/page.tsx`; Project 6 rendered the All Projects surface.

## Product contract

- **Purpose:** Prioritize unscheduled work and prepare cycles.
- **Primary persona:** Product owner.
- **Success metric:** Ready backlog age and cycle carry-over.
- **Required density:** comfortable, with compact child collections.
- **Core fields:** rank, key, title, type, priority, estimate, assignees, labels, readiness.
...
Deep-linkable query parameters: `q`, `type`, `priority`, `assigneeId`, `labelId`, `readiness`, `sort`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/backlog/project-backlog-page`
...
- **Endpoints:** GET /build/:projectId/tickets; PATCH .../rank; POST .../bulk.
...
- **Detail response:** `{ data: { rank, key, title, type, priority, estimate, assignees, labels, readiness, version, createdAt, updatedAt }, meta }`.
