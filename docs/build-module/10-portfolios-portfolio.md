...
- **Current/target route:** `/build/portfolios/[portfolioId]`
- **Scope:** portfolio
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** Compare strategic investment, health, and outcomes.
- **Evidence:** `frontend/app/(authenticated)/build/portfolios/[portfolioId]/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Group investments across projects and programs.
- **Primary persona:** Portfolio manager.
- **Success metric:** Portfolio review completion and stale health rate.
- **Required density:** compact by default with a comfortable-density toggle.
- **Core fields:** name, owner, status, projects/programs, budget, health, goals.
...
Deep-linkable query parameters: `ownerId`, `status`, `health`, `q`, `sort`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/portfolios/portfolio-detail-page`
...
- **Endpoints:** GET/POST /build/portfolios.
- **Client schema/hooks:** `frontend/hooks/api/build/portfolios.ts; portfolios-schema.ts` where present.
...
- **Detail response:** `{ data: { name, owner, status, projects/programs, budget, health, goals, version, createdAt, updatedAt }, meta }`.
