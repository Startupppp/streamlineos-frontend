...
- **Current/target route:** `/build/[projectId]/client-portal`
- **Scope:** project
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** Publish bounded progress while protecting internal data.
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/client-portal/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Control and preview the external client projection.
- **Primary persona:** Client project manager.
- **Success metric:** Grant adoption and zero disclosure incidents.
- **Required density:** comfortable, with compact child collections.
- **Core fields:** grant, publication state, visible sections/fields, expiry, preview content.
...
Deep-linkable query parameters: `grantId`, `section`, `status`, `from`, `to`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/client-portal/client-visibility-page`
...
- **Endpoints:** client portal and client visibility endpoints.
- **Client schema/hooks:** `frontend/hooks/api/build/client-portal.ts; client-portal-schema.ts` where present.
...
- **Detail response:** `{ data: { grant, publication state, visible sections/fields, expiry, preview content, version, createdAt, updatedAt }, meta }`.
