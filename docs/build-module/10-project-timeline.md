...
- **Current/target route:** `/build/[projectId]/timeline`
- **Scope:** project
- **Disposition:** **CONSOLIDATE**
- **Decision:** Migrate the user job to `/build/[projectId]/issues?layout=timeline`, preserve deep links temporarily, then remove this physical route.
- **User job:** Spot schedule conflict by arranging work over time.
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/timeline/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Visualize dated issues and dependencies.
- **Primary persona:** Project manager.
- **Success metric:** Dependency conflict resolution.
- **Required density:** comfortable, with compact child collections.
- **Core fields:** work items, start/due, dependency, milestone, owner, status.
...
Deep-linkable query parameters: `layout=timeline`, `from`, `to`, `zoom`, `group`, `filters`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/views/gantt-view`, `@/features/build/ticket-details/build-ticket-detail-url`
...
- **Endpoints:** GET /build/:projectId/tickets.
- **Client schema/hooks:** `frontend/hooks/api/build/tickets.ts` where present.
...
- **Detail response:** `{ data: { work items, start/due, dependency, milestone, owner, status, version, createdAt, updatedAt }, meta }`.
