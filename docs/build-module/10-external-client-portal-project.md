# External Client Portal Project

## Route decision

- **Current route:** `/client-portal/[projectId]`
- **Scope:** external project grant
- **Disposition:** **KEEP**
- **Decision:** Keep as the canonical external project view.
- **User job:** Understand project progress and request a change without asking for an internal account.
- **Evidence:** `frontend/app/(portal)/client-portal/[projectId]/page.tsx`

## Product contract

- **Purpose:** Review the bounded project projection and submit a change request.
- **Primary persona:** External client
- **Success metric:** Client self-service completion and zero hidden-field disclosure.
- **Core fields:** published project summary, milestones, tickets, updates, files, allowed request fields.
...
Deep-linkable state: `tab`; server cursor when any published child collection can exceed one page. Selection, drafts, session credentials, open menus, and unsaved input do not belong in the URL.
...
| Build member | Only with valid portal token plus active project grant and field-level visibility | Only explicit capability | Only explicit capability | No by default |
...
- Page/source: `frontend/app/(portal)/client-portal/[projectId]/page.tsx`.
...
- **Endpoints:** `GET /portal/v1/projects/:projectId/overview`, `POST /portal/v1/projects/:projectId/change-requests`.
- **Client evidence:** `frontend/hooks/api/portal/use-portal-project-overview.ts`, `frontend/hooks/api/portal/use-submit-change-request.ts`.
...
- **P0:** The current page delegates all detail behavior to one component; bounded pagination and field-level disclosure require contract tests.
