...
- **Current/target route:** `/build/[projectId]/risks`
- **Scope:** project
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** Identify and mitigate threats before impact.
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/risks/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Maintain a reviewable project risk register.
- **Primary persona:** Project manager.
- **Success metric:** Overdue-review and realized-risk rate.
- **Required density:** compact by default with a comfortable-density toggle.
- **Core fields:** title, category, probability, impact, exposure, owner, mitigation, review date, status.
...
Deep-linkable query parameters: `ownerId`, `category`, `probability`, `impact`, `status`, `overdue`, `q`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/governance/risks-page`
...
- **Endpoints:** risk endpoints.
- **Client schema/hooks:** `frontend/hooks/api/build/governance.ts; governance-schema.ts` where present.
...
- **Detail response:** `{ data: { title, category, probability, impact, exposure, owner, mitigation, review date, status, version, createdAt, updatedAt }, meta }`.
...
- [x] Keyboard, screen-reader, reduced-motion, 375 px mobile, and high-density desktop checks pass.
  `frontend/e2e/governance-qa-a11y.spec.ts` — 22/22 passing in Chromium against `/design-system/governance-qa`, which mounts the real `buildRiskColumns` and `RiskMobileCard` from `features/build/governance/risks-table-columns.tsx` (not a lookalike), so the measured table is the production 8-column one. Covers: no sideways scroll at 375/768/1280; every field control computed at exactly 36px; `table` role with `Title`/`Status` column headers; skeleton headers equal to `RISK_TABLE_HEADERS`; search input focusable; 9 `row`-role rows; pagination controls present; at 375px the status filter has no bounding box (Drawer collapse) while search stays painted, and `table` is hidden in favour of mobile cards. Reduced motion is asserted as a pair: with `reducedMotion: "reduce"` the shimmer computes `animation-name: none` (`globals.css:700`), and with `no-preference` the same node animates, so the assertion is not vacuous.
