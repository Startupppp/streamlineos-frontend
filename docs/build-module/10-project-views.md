...
- **Current/target route:** `/build/[projectId]/views`
- **Scope:** project
- **Disposition:** **CONSOLIDATE**
- **Decision:** Migrate the user job to `/build/[projectId]/issues and /settings/views`, preserve deep links temporarily, then remove this physical route.
- **User job:** Reopen, pin, edit, or remove a saved project view without rebuilding its query.
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/views/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Manage reusable project issue-view definitions.
- **Primary persona:** Project manager or frequent contributor.
- **Success metric:** Saved-view reuse rate and time to restore a working issue layout.
- **Required density:** comfortable, with compact child collections.
- **Core fields:** view name, owner, visibility, pinned state, filter/sort/group/layout definition, updated time.
...
Deep-linkable query parameters: `range`, `teamId`, `ownerId`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/views/views-page`
...
- **Endpoints:** `GET|POST /build/:projectId/views`, `PATCH|DELETE /build/:projectId/views/:viewId`.
- **Client schema/hooks:** `frontend/hooks/api/build/advanced.ts`.
...
- **Detail response:** `{ data: { health, progress, due work, risks, milestones, releases, activity, version, createdAt, updatedAt }, meta }`.
