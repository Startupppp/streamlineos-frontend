...
- **Current route:** `/roadmap/[orgId]`
- **Scope:** public organization projection
- **Disposition:** **KEEP**
- **Decision:** Keep, but replace public numeric organization identity with a stable public slug or publication token.
- **User job:** Understand what is planned and signal which outcomes matter to me.
- **Evidence:** `frontend/app/(public)/roadmap/[orgId]/page.tsx`

## Product contract

- **Purpose:** View published plans and changelog, vote, and submit feedback.
- **Primary persona:** Customer or prospect
- **Success metric:** Qualified feedback/votes and zero unpublished-item disclosure.
- **Core fields:** published roadmap lanes, feedback posts, changelog, vote totals, categories, target quarter.
...
Deep-linkable state: `tab`, `status`, `category`, `q`; voter identity remains local and is never accepted as authorization. Selection, drafts, session credentials, open menus, and unsaved input do not belong in the URL.
...
| Build member | Only with organization publication enabled; anonymous writes rate-limited and abuse-protected | Only explicit capability | Only explicit capability | No by default |
...
- Page/source: `frontend/app/(public)/roadmap/[orgId]/page.tsx`.
...
- **Endpoints:** `GET /public/roadmap?org=`, `POST /public/roadmap/vote?org=`, `POST /public/roadmap/feedback?org=`.
- **Client evidence:** `frontend/hooks/api/build/roadmap.ts`.
...
- **P0:** The client-generated voter key is not a security boundary; duplicate-vote controls and opaque organization addressing require hardening.
