...
- **Purpose:** Optionally group projects, products, and teams.
- **Primary persona:** Program administrator.
- **Success metric:** Workspace adoption without standalone-project failure.
- **Required density:** comfortable, with compact child collections.
- **Core fields:** name, owner, members, products, projects, teams, health.
...
Deep-linkable query parameters: `ownerId`, `status`, `q`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/pm-workspaces/pm-workspaces-page`
...
- **Endpoints:** GET/POST /build/pm-workspaces.
- **Client schema/hooks:** `frontend/hooks/api/build/pm-workspaces.ts; pm-workspaces-schema.ts` where present.
...
- **Detail response:** `{ data: { name, owner, members, products, projects, teams, health, version, createdAt, updatedAt }, meta }`.
