...
- **Current/target route:** `/build/[projectId]/modules`
- **Scope:** project
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** See ownership and progress by feature area.
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/modules/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Group work by durable product or system area.
- **Primary persona:** Engineering/product lead.
- **Success metric:** Module ownership coverage.
- **Required density:** compact by default with a comfortable-density toggle.
- **Core fields:** name, lead, status, dates, progress, open work.
...
Deep-linkable query parameters: `status`, `leadId`, `q`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/modules/modules-page`
...
- **Endpoints:** GET/POST/PATCH /build/:projectId/modules.
...
- **Detail response:** `{ data: { name, lead, status, dates, progress, open work, version, createdAt, updatedAt }, meta }`.
