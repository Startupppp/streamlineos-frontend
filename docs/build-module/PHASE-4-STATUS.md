...
| ~~`openapi.json` not regenerated~~ | **RETIRED 2026-09-22 — the premise was false.** The published contract already carried `status` on all four roadmap/feedback routes. A regeneration with placeholder env (no database contacted) returned a byte-identical artifact: same sha256, 0 paths changed, 0 operations changed, and **0** of the claimed 30 path-parameter rewrites. See `NEXT-CLOSURE-STATUS.md`. | n/a |
