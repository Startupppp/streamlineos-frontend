...
- **Current/target route:** `/build/[projectId]/triage`
- **Scope:** project
- **Disposition:** **KEEP**
...
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/triage/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Classify and convert incoming evidence into canonical work.
- **Primary persona:** Product/project manager.
- **Success metric:** Triage lead time and duplicate rate.
- **Required density:** comfortable, with compact child collections.
- **Core fields:** source, summary, submitter, age, owner, status, duplicate score, linked work.
...
Deep-linkable query parameters: `source`, `status`, `ownerId`, `age`, `q`, `sort`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/triage/triage-page`
...
- **Endpoints:** forms submissions and feedback endpoints.
- **Client schema/hooks:** `frontend/hooks/api/build/forms.ts; public-intake.ts` where present.
...
- **Detail response:** `{ data: { source, summary, submitter, age, owner, status, duplicate score, linked work, version, createdAt, updatedAt }, meta }`.
