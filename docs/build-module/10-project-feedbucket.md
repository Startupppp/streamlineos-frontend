...
- **Current/target route:** `/build/[projectId]/feedbucket`
- **Scope:** project
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** Turn contextual product feedback into triaged work.
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/feedbucket/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Collect screenshot and recording evidence from a project widget.
- **Primary persona:** Product or support manager.
- **Success metric:** Submission-to-triage time.
- **Required density:** compact by default with a comfortable-density toggle.
- **Core fields:** submission, screenshot/video, URL, browser, customer, status, owner, links.
...
Deep-linkable query parameters: `status`, `ownerId`, `linked`, `duplicate`, `from`, `to`, `q`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/feedbucket/project-feedbucket-page`
...
- **Endpoints:** Feedbucket APIs filtered by project.
- **Client schema/hooks:** `frontend/hooks/api/build/feedbucket hooks` where present.
...
- **Detail response:** `{ data: { submission, screenshot/video, URL, browser, customer, status, owner, links, version, createdAt, updatedAt }, meta }`.
