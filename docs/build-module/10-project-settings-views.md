...
- [ ] Every core field, action, overlay, query parameter, bulk action, shortcut, state, and permission above is implemented and tested.
  - PARTIAL (2026-09-26 R7): `useBuildListFilters` wired, `BuildListToolbar` with `SearchInput` added, `searchInputRef` created and passed to `useBuildListKeyboard` — `/` now focuses the real search input and `q` is URL-backed. Remaining BLOCKED: bulk delete (the only real repeated operation on the views list) is not yet implemented — no `BulkActionBar`, no checkbox selection state. The spec section "Child collections support selection only when a real repeated operation exists" names bulk delete as applicable here. Re-tick only once BulkActionBar is wired with a "Delete selected" action.
