...
- [ ] Lists are bounded/virtualized and remain usable at 10k work items and 1k members.
  BLOCKED — unbounded end to end: `hooks/api/build/advanced.ts:223` `useViews` returns a plain `ProjectView[]` from `GET /build/{projectId}/views` with no cursor or page envelope, and `features/build/settings/project-settings-views-page.tsx:113` renders `(views ?? []).map(...)` with no pagination, slice or windowing (FE-112). Saved views scale with member count, so the criterion's 1k-member case is not met.
