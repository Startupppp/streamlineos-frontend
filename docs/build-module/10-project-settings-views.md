...
- **Current/target route:** `/build/[projectId]/settings/views`
- **Scope:** project
- **Disposition:** **ADD**
...
- [x] Every core field, action, overlay, query parameter, bulk action, shortcut, state, and permission above is implemented and tested.
  - Closed 2026-09-26 Wave-B-02: Checkbox selection state added to `ViewCard` (optional `isSelected`/`onToggleSelect` props); `project-settings-views-page.tsx` tracks `selectedIds: Set<number>`, renders a bulk bar ("N selected · Delete N views · Clear") when selection is non-empty, and fires `ConfirmDialog destructive` before calling `deleteView.mutate` per selected id. Checkboxes are gated on `canManage`. 15 tests pass covering all states, keyboard shortcuts (c/e/j/k/Enter/Esc//), pagination, and the new bulk delete flow. ViewCard accessible names `Rename saved view` and `Delete saved view` preserved exactly.
- [x] Lists are bounded/virtualized and remain usable at 10k work items and 1k members.
  - Implemented 2026-09-26: `GET /build/:projectId/views` now accepts `cursor` + `limit` query params; `workspace.service.ts:listViews` uses `buildTupleCursorPage` with `(isPinned DESC, updatedAt DESC, id DESC)` triple-column keyset. `useViews` accepts a cursor param and returns `CursorPage<ProjectView>`. `project-settings-views-page.tsx` uses `useCursorPager` + `TablePagination mode="cursor"`. Contract: `viewPageContract = cursorPageContract(viewRowSchema)`. Tests: 7 passing in `workspace-schema-views.test.ts`.
