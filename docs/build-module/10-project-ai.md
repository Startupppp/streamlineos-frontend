...
- **Current/target route:** `/build/[projectId]/ai`
- **Scope:** project
- **Disposition:** **CONSOLIDATE**
- **Decision:** Migrate the user job to `/build/command-center?projectId=...`, preserve deep links temporarily, then remove this physical route.
- **User job:** Get grounded help while retaining approval over changes.
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/ai/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Propose cited project analysis and actions.
- **Primary persona:** Project manager.
- **Success metric:** Accepted proposal rate and citation validity.
- **Required density:** comfortable, with compact child collections.
- **Core fields:** prompt, scope, sources, freshness, proposal diff, cost, run state.
...
Deep-linkable query parameters: `run status and date only`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/ai/ai-assistant-page`
...
- **Endpoints:** AI project endpoints and durable run APIs.
- **Client schema/hooks:** `frontend/hooks/api/build/ai.ts; ai-schema.ts` where present.
...
- **Detail response:** `{ data: { prompt, scope, sources, freshness, proposal diff, cost, run state, version, createdAt, updatedAt }, meta }`.
