...
- **Current/target route:** `/build/managed-products/[managedProductId]/feedback`
- **Scope:** managed product
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** Connect feedback to insights, roadmap, and work.
- **Evidence:** `frontend/app/(authenticated)/build/managed-products/[managedProductId]/feedback/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Collect rich customer evidence.
- **Primary persona:** Product manager.
- **Success metric:** Evidence-to-decision linkage.
- **Required density:** compact by default with a comfortable-density toggle.
- **Core fields:** source, customer, summary, media, browser, status, owner, linked records.
...
- Existing feature evidence: `@/features/build/managed-products/product-feedback-page`
...
- **Endpoints:** feedback/feedbucket endpoints.
- **Client schema/hooks:** `frontend/hooks/api/build/roadmap.ts; feedbucket hooks` where present.
...
- **Detail response:** `{ data: { source, customer, summary, media, browser, status, owner, linked records, version, createdAt, updatedAt }, meta }`.
