...
- **Current/target route:** `/build/[projectId]/meetings/[meetingId]`
- **Scope:** project
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** Turn discussion into traceable actions.
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/meetings/[meetingId]/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Connect meetings, decisions, and action items to delivery.
- **Primary persona:** Project manager.
- **Success metric:** Action completion and notes-link coverage.
- **Required density:** comfortable, with compact child collections.
- **Core fields:** title, type, time, host, attendees, agenda, notes, actions, decisions.
...
Deep-linkable query parameters: `type`, `status`, `organizerId`, `attendeeId`, `from`, `to`, `q`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/meetings/meeting-detail-page`
...
- **Endpoints:** meetings and action-items endpoints.
- **Client schema/hooks:** `frontend/hooks/api/build/meetings.ts; meetings-schema.ts` where present.
...
- **Detail response:** `{ data: { title, type, time, host, attendees, agenda, notes, actions, decisions, version, createdAt, updatedAt }, meta }`.
