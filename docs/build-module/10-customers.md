...
- **Current/target route:** `/build/customers`
- **Scope:** organization
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** Move from customer context to active delivery without duplicating CRM.
- **Evidence:** `frontend/app/(authenticated)/build/customers/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Show CRM customers linked to Build delivery.
- **Primary persona:** Client project manager.
- **Success metric:** Linked-customer coverage.
- **Required density:** comfortable, with compact child collections.
- **Core fields:** company, CRM owner, active projects, health, open changes, next milestone.
...
Deep-linkable query parameters: `q`, `ownerId`, `health`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/customers/project-customers-page`
...
- **Endpoints:** GET /build/customers.
- **Client schema/hooks:** `frontend/hooks/api/build/customers.ts` where present.
...
- **Detail response:** `{ data: { company, CRM owner, active projects, health, open changes, next milestone, version, createdAt, updatedAt }, meta }`.
