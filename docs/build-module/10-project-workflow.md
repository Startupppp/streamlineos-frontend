...
- **Scope:** project
- **Disposition:** **MOVE**
- **Decision:** Migrate the user job to `/build/[projectId]/settings/workflow`, preserve deep links temporarily, then remove this physical route.
- **User job:** Make the team's process explicit and enforceable.
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/workflow/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.
