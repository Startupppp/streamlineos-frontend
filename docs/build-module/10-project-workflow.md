...
- **Current/target route:** `/build/[projectId]/workflow`
- **Scope:** project
- **Disposition:** **MOVE — EXECUTED 2026-09-22**
- **Decision:** The user job lives at `/build/[projectId]/settings/workflow`. The physical page was deleted; the deep link is preserved by the `next.config.ts` redirect, which fires before the filesystem.
- **User job:** Make the team's process explicit and enforceable.
- **Evidence:** page deleted 2026-09-22; the job renders from `frontend/app/(authenticated)/build/[projectId]/settings/workflow/page.tsx`. Deep link preserved by the `frontend/next.config.ts` redirect. Census: `docs/build-module/DEAD-BUILD-SURFACE-INVENTORY.md`.

## Product contract

- **Purpose:** Configure statuses, transitions, WIP, and required fields.
- **Primary persona:** Project administrator.
- **Success metric:** Invalid-transition and WIP-breach rate.
- **Required density:** compact by default with a comfortable-density toggle.
- **Core fields:** status identity, category, order, transitions, WIP, required fields, version.
...
Deep-linkable query parameters: `status and transition filters only`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/workflow/workflow-page`
...
- **Client schema/hooks:** `frontend/hooks/api/build/workflow.ts; workflow-schema.ts` where present.
...
- **Detail response:** `{ data: { status identity, category, order, transitions, WIP, required fields, version, version, createdAt, updatedAt }, meta }`.
