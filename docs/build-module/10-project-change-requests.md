...
- **Current/target route:** `/build/[projectId]/change-requests`
- **Scope:** project
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** Assess and approve change with visible impact.
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/change-requests/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Control proposed scope changes.
- **Primary persona:** Project manager or client.
- **Success metric:** Decision lead time and unplanned-scope rate.
- **Required density:** comfortable, with compact child collections.
- **Core fields:** title, requester, impact, affected work, cost/schedule change, approvers, status.
...
Deep-linkable query parameters: `status`, `impact`, `requesterId`, `approverId`, `releaseId`, `clientVisible`, `q`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/change-requests/change-requests-page`
...
- **Endpoints:** change-request endpoints.
- **Client schema/hooks:** `frontend/hooks/api/build/change-requests.ts` where present.
...
- **Detail response:** `{ data: { title, requester, impact, affected work, cost/schedule change, approvers, status, version, createdAt, updatedAt }, meta }`.
