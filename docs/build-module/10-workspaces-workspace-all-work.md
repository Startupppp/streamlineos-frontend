...
- **Current/target route:** `/build/workspaces/[pmWorkspaceId]/all-work`
- **Scope:** workspace
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** Find and bulk-update work across projects.
- **Evidence:** `frontend/app/(authenticated)/build/workspaces/[pmWorkspaceId]/all-work/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Search and operate across every authorized work item.
- **Primary persona:** Project manager.
- **Success metric:** Search success and bulk-action completion.
- **Required density:** compact by default with a comfortable-density toggle.
- **Core fields:** key, title, project, type, status, priority, assignees, cycle, due, estimate.
...
Deep-linkable query parameters: `q`, `projectId`, `workspaceId`, `productId`, `teamId`, `type`, `status`, `priority`, `assigneeId`, `group`, `sort`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/all-work/all-work-page`
...
- **Endpoints:** GET /build/all-work; POST /build/:projectId/tickets/bulk.
- **Client schema/hooks:** `frontend/hooks/api/build/all-work.ts; tickets.ts` where present.
...
- **Detail response:** `{ data: { key, title, project, type, status, priority, assignees, cycle, due, estimate, version, createdAt, updatedAt }, meta }`.
