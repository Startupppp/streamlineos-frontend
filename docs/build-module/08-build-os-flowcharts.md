...
Verified against the repository on 2026-09-22.
...
| Intake | Forms, public forms, Intake, Triage | `/forms`, `/forms/[formId]`, `/triage`, public `/forms/[formToken]` | LIVE; Intake is consolidated into Forms |
...
| Insight | Reports, Analytics, Workload | `/reports`, `/analytics`, `/workload` | LIVE; Workload is ticket-count based rather than full HR capacity |
...
The durable discovery chain is implemented as feedback to insight to roadmap to project to release. `docs/build-module/PHASE-4-STATUS.md` records the tenant-bound links and the browser fixes that make the chain traversable.
...
| Control scope | Budget, Approvals, Change Requests | `/build/[projectId]/budget`, `/approvals`, `/change-requests` | LIVE; release/client-visible/affected-work filters remain incomplete |
...
1. Complete the Sprint-to-Cycle application cutover, freeze legacy writers, and perform the approved contraction.
2. Complete the QA Bug-to-work-item application cutover, freeze legacy writers, and perform the approved contraction.
3. Add the missing Change Request fields and filters for release, client visibility, and affected work.
4. Keep the route, permission, contract, typecheck, and browser gates green after the two data-model cutovers.
...
2. Upgrade Workload from ticket counts to real capacity using HR leave, employment schedule, estimates, and Timesheets actuals.
3. Add durable offline drafts, reconnect reconciliation, version conflicts, and realtime gap recovery.
4. Add Feedbucket bulk actions and server-supported owner, linked, duplicate, date, and cursor filters.
5. Add project filtering to Inbox through the backend notification contract.
6. Add importers with preview, validation, mapping, and rollback reports for Jira, ClickUp, Linear, CSV, and Trello.
...
6. Optional freelancer payment protection or an escrow partner integration; do not imitate escrow with an unsafe internal balance.

## Recommended product order

```text
First:   finish Cycle and Bug cutovers
Second:  complete the freelancer cross-module handoff
Third:   make Workload use HR and Timesheets truth
Fourth:  close offline, realtime, Feedbucket, and import gaps
