...
- **Current/target route:** `/build/[projectId]`
- **Scope:** project
- **Disposition:** **KEEP**
...
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.
...
- Existing feature evidence: `@/features/build/overview/project-overview-page`
...
- **Endpoints:** GET /build/:projectId; bounded summary endpoints.
- **Client schema/hooks:** `frontend/hooks/api/build/projects.ts; reports.ts` where present.
