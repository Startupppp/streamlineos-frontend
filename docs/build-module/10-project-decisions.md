...
- **Current/target route:** `/build/[projectId]/decisions`
- **Scope:** project
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** Understand what was decided, by whom, and why.
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/decisions/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Preserve consequential decisions and their evidence.
- **Primary persona:** Project/product manager.
- **Success metric:** Decision evidence and supersession coverage.
- **Required density:** comfortable, with compact child collections.
- **Core fields:** title, context, options, outcome, owner, status, effective date, links, supersession.
...
Deep-linkable query parameters: `ownerId`, `status`, `category`, `from`, `to`, `linkedType`, `q`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/governance/decisions-page`
...
- **Detail response:** `{ data: { title, context, options, outcome, owner, status, effective date, links, supersession, version, createdAt, updatedAt }, meta }`.
