...
- **Current/target route:** `/build/[projectId]/qa/runs/[runId]`
- **Scope:** project
- **Disposition:** **KEEP**
- **Decision:** Retain as a canonical page, subject to the gaps and acceptance criteria below.
- **User job:** Plan tests, execute runs, and link failures to canonical bugs.
- **Evidence:** `frontend/app/(authenticated)/build/[projectId]/qa/runs/[runId]/page.tsx`; Route existence verified in the repository; live behavior not directly observed with a valid detail record. **ASSUMPTION:** the page follows its source component until browser evidence is captured.

## Product contract

- **Purpose:** Manage test evidence and execution.
- **Primary persona:** QA lead.
- **Success metric:** Coverage, pass rate, and defect escape rate.
- **Required density:** compact by default with a comfortable-density toggle.
- **Core fields:** suite, case, run, result, environment, release, assignee, linked bug.
...
Deep-linkable query parameters: `tab`, `suiteId`, `status`, `priority`, `assigneeId`, `releaseId`, `environment`, `q`, `cursor`. Cursor may be shared only when it is stable for the same normalized filter/sort/access revision. Selection, open menus, drafts, and unsaved form state are not placed in the URL.
...
- Existing feature evidence: `@/features/build/qa/runs/run-execution-page`
...
- **Endpoints:** test-suite/case/run endpoints.
- **Client schema/hooks:** `frontend/hooks/api/build/qa.ts; qa-schema.ts` where present.
...
- **Detail response:** `{ data: { suite, case, run, result, environment, release, assignee, linked bug, version, createdAt, updatedAt }, meta }`.
