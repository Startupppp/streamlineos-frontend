...
- **Read side scoped:** stock levels, availability, movements, transfers, adjustments, the warehouses list, valuation summary and valuation layers. Transfers require **both** ends in scope — seeing one leg would expose the counterpart warehouse's movement. Adjustments scope through `EXISTS` over their lines, since locations live on lines rather than the header. Valuation's FIFO subquery is scoped too: layers became location-keyed in 0400, so an unscoped subquery would have quietly summed warehouses the caller cannot see.
- **Cache keys carry the scope** on both the per-org stock-level list and the per-org warehouses list. Either would otherwise have served one operator's warehouses to the next.
- **Still unscoped:** quality holds, quality inspections, recalls, cycle counts, and the reports service.
