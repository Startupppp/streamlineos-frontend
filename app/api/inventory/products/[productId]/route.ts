  ...
  return withAbility("read", "inventory:products", async (session) => {
    try {
      ...
      return ok({
        ...
        uomName: product.uom?.abbreviation ?? product.uom?.name ?? null,
        isActive: product.status === "ACTIVE",
      });
    } catch (error) {
      return serverErr("Failed to load product", error);
    }
  });
}
...
export async function PATCH(req: NextRequest, ctx: Ctx) {
  ...
  return withAbility("update", "inventory:products", async (session) => {
    try {
      ...
      return serverErr("Failed to update product", error);
    }
  });
}
...
export async function DELETE(_req: NextRequest, ctx: Ctx) {
  ...
  return withAbility("delete", "inventory:products", async (session) => {
    try {
      ...
      return serverErr("Failed to delete product", error);
    }
  });
}
