...
export async function GET(req: NextRequest, { params }: Params) {
  ...
  return withModuleAbility("accounting", "read", "accounting:reports", async (session) => {
    ...
    const tag = entityScopedTag(CacheTag.customerLedger, `${session.orgId}:${id}`);
    const fetcher = unstable_cache(
      async (): Promise<CustomerLedger | null> => {
        const client = await loadClientSummary(session.orgId, id);
        if (!client) return null;

        const arAccountId = await loadArAccountId(session.orgId);
        const invoiceIds = await loadInvoiceIds(session.orgId, id);
        const paymentIds = await loadPaymentIds(session.orgId, invoiceIds);
        const totals = await loadInvoiceTotals(session.orgId, id);
      ...
      [tag, `customer-ledger:${id}:${from ?? ""}:${to ?? ""}`, orgScopedTag(CacheTag.customerLedger, session.orgId)],
      { tags: [tag, orgScopedTag(CacheTag.customerLedger, session.orgId)], revalidate: 60 },
    );
