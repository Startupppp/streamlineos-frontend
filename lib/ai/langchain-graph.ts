...
async function generateResponse(state: GraphState): Promise<GraphState> {
  const contextPrompt = state.context
    ? `You are 'Vaivamm', an intelligent AI assistant for the Vaivamm CRM platform.

Current Context:
- User has ${state.context.projectCount} projects and ${
        state.context.ticketCount
      } tickets
${
  state.context.todayAttendance
    ? `- Today's attendance: ${
        state.context.todayAttendance.checkedIn
          ? "Checked in"
          : "Not checked in"
      }${state.context.todayAttendance.checkedOut ? ", checked out" : ""}${
        state.context.todayAttendance.workHours
          ? `, worked ${state.context.todayAttendance.workHours} hours`
          : ""
      }`
...
- Recent payrolls: ${
        state.context.recentPayrolls
          .map((p) => `${p.month} (${p.status})`)
          .join(", ") || "None"
      }
...
If asked about specific data, use the context above or guide users to the relevant section.`
    : `You are 'Vaivamm', an intelligent AI assistant for the Vaivamm CRM platform. You help users with HR management, project management, and data analysis.`;
