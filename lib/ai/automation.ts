...
export async function predictProjectDelays(
  orgId: string,
  projectId: number
): Promise<{
  projectId: number;
  ...
  const completedTickets =
    project.tickets?.filter((t) => t.status === "DONE").length || 0;
  const inProgressTickets =
    project.tickets?.filter((t) => t.status === "IN_PROGRESS").length || 0;
  ...
  if (completionRate < 0.3 && daysRemaining !== null && daysRemaining < 30) {
    riskLevel = "HIGH";
    reason = `Low completion rate (${Math.round(
      completionRate * 100
    ...
    riskLevel = "MEDIUM";
    reason = `Moderate completion rate (${Math.round(
      completionRate * 100
    )}%) with ${daysRemaining} days remaining`;
  ...
  return {
    ...
    suggestedActions:
      suggestedActions.length > 0 ? suggestedActions : ["Continue monitoring"],
  };
}
