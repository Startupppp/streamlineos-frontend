...
export function getWeeklyAttendanceReportTemplate(
  ...
  const tableRows = rows
    .map(
      (r) => `
      <tr>
        <td style="border: 1px solid #d1d5db; padding: 8px 12px; color: #374151;">${r.name}</td>
        <td style="border: 1px solid #d1d5db; padding: 8px 12px; text-align: center; color: #111827;">${r.daysPresent}</td>
        <td style="border: 1px solid #d1d5db; padding: 8px 12px; text-align: center; color: #111827;">${r.totalHours}</td>
  ...
  const content = `
    ...
    <p class="email-text">
      Here is the weekly attendance summary for <strong>${orgName}</strong> for the period <strong>${weekRange}</strong>.
    </p>
...
export function getMonthlyExpenseReportTemplate(
  ...
  const tableRows = rows
    .map(
      (r) => `
      <tr>
        <td style="border: 1px solid #d1d5db; padding: 8px 12px; color: #374151;">${r.date}</td>
        <td style="border: 1px solid #d1d5db; padding: 8px 12px; color: #374151;">${r.employeeName}</td>
        <td style="border: 1px solid #d1d5db; padding: 8px 12px; color: #374151;">${r.category}</td>
        <td style="border: 1px solid #d1d5db; padding: 8px 12px; text-align: right; color: #111827;">${r.currency === "INR" ? "₹" : ""}${r.amount}</td>
        <td style="border: 1px solid #d1d5db; padding: 8px 12px; text-align: center; color: #111827;">${r.status}</td>
      </tr>`
    )
    .join("");
  ...
  const content = `
    ...
    <p class="email-text">
      Here is the expense summary for <strong>${orgName}</strong> for <strong>${monthLabel}</strong>.
    </p>
    ...
    <div style="background-color: #f8fafc; border-radius: 8px; padding: 16px; margin: 24px 0;">
      ...
      <p class="email-text" style="margin: 0; font-size: 14px;">Total amount: <strong>₹${summary.totalAmount}</strong> &nbsp;|&nbsp; Total expenses: <strong>${summary.totalCount}</strong> &nbsp;|&nbsp; Pending: ${summary.pendingCount} &nbsp;|&nbsp; Approved: ${summary.approvedCount} &nbsp;|&nbsp; Paid: ${summary.paidCount} &nbsp;|&nbsp; Rejected: ${summary.rejectedCount}</p>
    </div>
