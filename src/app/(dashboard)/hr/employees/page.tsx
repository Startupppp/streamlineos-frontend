    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
           ...
           <p className="text-zinc-400">Directory of all members in this organization.</p>
        </div>
      </div>
          ...
          <Card key={mem.id} className="bg-white/5 border-white/10">
            <CardHeader className="flex flex-row items-center gap-4">
              <Avatar className="h-12 w-12">
                ...
                <AvatarFallback>{mem.publicUserData?.firstName?.charAt(0)}{mem.publicUserData?.lastName?.charAt(0)}</AvatarFallback>
              </Avatar>
              <div>
                <CardTitle className="text-white text-lg">
                    {mem.publicUserData?.firstName} {mem.publicUserData?.lastName}
                </CardTitle>
                <div className="text-sm text-zinc-400">{mem.publicUserData?.identifier}</div>
              </div>
            </CardHeader>
            <CardContent>
