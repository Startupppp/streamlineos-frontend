...
export function BurndownChart({ sprintId }: BurndownChartProps) {
  ...
  if (isLoading) {
    return <Card><CardContent>Loading burndown chart...</CardContent></Card>;
  }
  ...
  const maxPoints = Math.max(...idealBurndown.map((d) => d.points), totalPoints);
        ...
        <div className="relative" style={{ height: `${chartHeight}px` }}>
          <svg width="100%" height={chartHeight} className="overflow-visible">
            ...
            {idealBurndown.map((point, index) => {
              ...
              const nextX = nextPoint ? ((index + 1) / (idealBurndown.length - 1)) * 100 : x;
            ...
            {actualBurndown.length > 0 && (
              <>
                {actualBurndown.map((point, index) => {
                  ...
                  const y = ((maxPoints - Number(point.points)) / maxPoints) * chartHeight;
                  const nextPoint = actualBurndown[index + 1];
                  const nextDateIndex = nextPoint
                    ? idealBurndown.findIndex(
                        (d) =>
                          format(d.date, "yyyy-MM-dd") === format(new Date(nextPoint.date), "yyyy-MM-dd")
                      )
                    : -1;

                  if (nextDateIndex === -1) return null;

                  const nextX = (nextDateIndex / (idealBurndown.length - 1)) * 100;
                  const nextY =
                    ((maxPoints - Number(nextPoint.points)) / maxPoints) * chartHeight;
          ...
          <div className="absolute bottom-0 left-0 right-0 flex justify-between text-xs text-muted-foreground">
            {idealBurndown.map((point, index) => {
              if (index % Math.ceil(idealBurndown.length / 5) !== 0) return null;
              return (
                <span key={index}>{format(point.date, "MMM dd")}</span>
              );
            })}
          </div>
        </div>
