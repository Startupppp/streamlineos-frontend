...
export function KanbanBoard({ tickets, projectId }: KanbanBoardProps) {
  ...
  const updateStatus = useUpdateTicketStatus({
    onMutate: async (newTicket) => {
      await queryClient.cancelQueries({ queryKey: vaivammKeys.project.project(projectId) });
                      ...
                      <CardContent className="p-4 space-y-3">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {ticket.priority && (
                            <Badge
                              variant="outline"
                              className={cn("text-[10px] h-5", PRIORITY_COLORS[ticket.priority] || "")}
                            >
                              {ticket.priority}
                            </Badge>
                          )}
                        ...
                        <div className="flex items-center justify-between pt-2">
                          {ticket.timeSpent && parseFloat(ticket.timeSpent) > 0 && (
