...
import { AttendancePageSkeleton } from "../ui/attendance-skeleton";
  ...
  return (
    <div className="p-8 space-y-8">
      ...
      <div className="grid gap-8 md:grid-cols-2">
        <div>
          <AttendanceTracker
            initialStatus={status as AttendanceStatus}
            todayLog={todayLog}
          />
        </div>
              ...
              {logs.map((log: AttendanceLog) => (
                ...
                >
                  ...
                  <div className="text-right">
                    <div
                      className={`font-bold ${
                        log.isOvertime ? "text-yellow-600" : "text-primary"
                      }`}
                    >
                      {log.workHours || "0"} hrs
                    </div>
                    <div className="text-xs text-muted-foreground capitalize">
                      {(log.status || "Unknown").toLowerCase()}
                    </div>
                  </div>
                </div>
              ))}
