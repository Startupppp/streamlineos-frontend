...
- [ ] `deals.stageChangedAt` — ADD if not present (for funnel timing)
- [ ] `deals.expectedCloseDate` index — speed up time-range queries
- [ ] `sales_dashboard_cache` — optional: nightly cache table for heavy aggregations

### API
- [x] `GET /api/sales/targets` — sales targets
- [x] `GET /api/sales/forecasts` — sales forecasting
- [x] `GET /api/sales/quotas` — quotas list (page exists)
- [ ] `GET /api/sales/dashboard/kpis` — Total MRR, Pipeline Value, Close Rate, Avg Deal Size (with date + rep filters)
- [ ] `GET /api/sales/dashboard/funnel` — stage counts for funnel chart
- [ ] `GET /api/sales/dashboard/leaderboard` — rep stats: deals won, win rate, revenue
- [ ] `GET /api/sales/dashboard/revenue-vs-goal` — closed revenue vs quota per month
- [ ] `POST /api/sales/quotas` — set monthly quota (Admin/CEO only)
- [ ] Redis cache for KPIs (TTL 5 min); cache key includes `orgId + dateRange + repId`
- [ ] Date-range filter: `startDate/endDate` query params on all dashboard endpoints
- [ ] Currency normalization: convert all deal values to org's `defaultCurrency`

### Frontend
- [x] `app/(dashboard)/sales/page.tsx` — sales dashboard
- [x] `app/(dashboard)/sales/quotas/page.tsx` — quotas management
- [x] `app/(dashboard)/sales/forecast-report/page.tsx` — forecast report
- [x] `app/(dashboard)/sales/commissions/page.tsx` — commissions
- [x] `app/(dashboard)/sales/person/[slug]/page.tsx` — rep profile
- [ ] Date filter dropdown: Today / This Week / This Month / Last Month / Q1-Q4 / YTD / Custom Range
- [ ] Rep filter dropdown: "All Reps" + individual rep selector
- [ ] KPI cards: Total MRR (large number, MoM trend %), Pipeline Value, Close Rate %, Avg Deal Size
- [ ] Funnel chart (Recharts or custom SVG): Lead → Qualified → Proposal → Won; shows counts + drop-off %
- [ ] Revenue vs Goal bar chart: grey bar (target) + green bar (actual) per month
- [ ] Rep leaderboard table: Rep Name, Open Deals, Deals Won, Win Rate, Revenue; sortable columns
- [ ] Click rep row → navigate to `/sales/person/[slug]`
- [ ] Export dashboard to PDF button (html2canvas + jsPDF)
- [ ] Trend indicator: ▲/▼ with % change vs previous period on each KPI

### New Features (Extended)
- [ ] **Deal velocity metric** — avg days from lead creation to close
- [ ] **Sales cycle length chart** — histogram of days-to-close distribution
- [ ] **Cohort analysis** — leads created in Month X: how many converted, when?
- [ ] **Sales rep comparison** — overlay two reps' performance on same chart
- [ ] **AI weekly narrative** — "This week: Pipeline grew 12%..." auto-generated by `/api/ai/report-narrator`
- [ ] **Goal setting UI** — drag slider to set monthly quota per rep
- [ ] **Real-time updates** — KPIs refresh when a deal is marked WON (Ably event)
- [ ] **Lost deal analysis** — reasons for LOST status; word cloud of loss reasons
- [ ] **Pipeline aging alerts** — deals stagnant > 14 days flagged in red on dashboard

### Verification
- [ ] KPIs load < 1.5s (Redis cached for repeated loads)
- [ ] Empty state shows gracefully when no data for selected range
