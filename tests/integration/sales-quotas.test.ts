...
describe("GET /api/sales/quotas", () => {
  ...
  it("returns 200 with quota list", async () => {
    const { GET } = await import("@/app/api/sales/quotas/route");
    const req = new NextRequest("http://localhost/api/sales/quotas");
  ...
  it("supports period filter", async () => {
    const { GET } = await import("@/app/api/sales/quotas/route");
    const req = new NextRequest("http://localhost/api/sales/quotas?period=monthly");
    const res = await GET(req);
    expect(res.status).toBe(200);
  });

  it("validates POST body with Zod", async () => {
    const { POST } = await import("@/app/api/sales/quotas/route");
    const req = new NextRequest("http://localhost/api/sales/quotas", {
      method: "POST",
  ...
  it("non-admin cannot create quotas", async () => {
