...
- **`/projects`**
  - [x] Create project (exists)
  - [x] Edit project inline (overflow menu + Edit Sheet on card and row)
  - [x] Archive/restore project (first-class action + confirm)
  - [x] Delete project inline (confirm + mutation)
  - [x] Filters persisted in URL (required)
- **`/projects/[projectId]` + views**
  - [x] View switcher stable with URL (filters preserved)
  - [x] Ticket deep-link open via `?ticket=` (exists)
  - [x] Ticket identity consistent as `PROJ-123` everywhere (header, list, table, backlog, my-tickets, board)
- **Ticket details Sheet**
  - [x] Ticket update flows invalidate all relevant views (onSuccess composition fixed; sprints/reports included)
  - [x] Comment permalink deep-link via `&comment=` (scroll + highlight; param cleared on close)
  - [x] “Restricted ticket” UX based on stable error codes (`PROJECTS_FORBIDDEN_TICKET` / `PROJECTS_TICKET_NOT_FOUND`)
- **Comments**
  - [x] Create/edit/delete/reply (edit author-only; delete author or `projects:manage`; replies cascade)
  - [x] Mentions + reactions (server-persisted; mention notifications deep-link `?ticket=&comment=`)
  - [x] Permalinks + copy link action (per comment and reply)
- **Chat (`/chat`)**
  - [x] Ticket mention picker (`#`) writes structured metadata (backend persists `metadata.entities`)
  - [x] Ticket pill renders from metadata (incl. fallback/restricted states)
  - [x] Status change action from chat with optimistic UI + revert (stable-coded errors; system message emitted)
  - [x] Comment permalink unfurl (internal ticket/comment preview cards)
  - [x] Offline queue preserves entity refs (metadata survives queue + drain)
- **Calendar (`/calendar`)**
  - [x] Link/unlink ticket on event create/edit/detail (unlink sends explicit nulls)
  - [x] Open linked ticket from event detail (enriched `linkedTicket` card navigates to project)
  - [x] Create ticket from calendar flows (header dropdown + slot-choice dialog)
  - [x] Ticket due dates appear as `source:"task"` (backend queries project tickets; `PROJ-123: title`, entityType/entityId)
- **Project settings (`/projects/[projectId]/settings`)**
  - [x] Workflow/status config stable and persisted (settings now CRUD `project_statuses`; board derives columns from it; service-level status validation `PROJECTS_INVALID_TICKET_STATUS`)
  - [ ] Custom fields reflected in table/list and ticket detail (ticket detail ✓; table/list columns still pending)
  - [x] Danger zone delete remains permission-gated (exists)
- **Saved views (`/projects/[projectId]/views`)**
  - [x] Save/apply a view with URL persistence (board applies `viewId` filters; “Save view” from board)
  - [ ] RBAC: shared vs personal views model (deferred — no `isShared` column yet; data re-queried under viewer permissions so no leak, but view metadata is org-visible)
