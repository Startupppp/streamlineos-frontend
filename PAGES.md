...
- [x] `/projects/whiteboards` — Whiteboards hub (org-wide ACL-filtered board grid via `GET /whiteboards`, cap 100, project-name join; cards deep-link `?board=`; "Whiteboards" entry added to the Projects & Time sidebar group; "New board" create dialog with project picker via `useCreateWhiteboardInProject`; migrations 0137/0138 APPLIED + `backfill:rbac` run + access versions bumped on 2026-07-03)
...
- [x] `/knowledge-base/recent` — Recently visited pages: compact list rows (icon, title link, relative updated-at); skeleton/empty/error states; live count subtitle
- [x] `/knowledge-base/favorites` — Favorited pages: list with hover remove-star action via toggleFavorite mutation; empty state CTA to browse pages
- [x] `/knowledge-base/trash` — Full-page trash: list rows (icon, title, deleted-at) with Restore + Delete forever (AlertDialog confirm) via existing hooks; skeleton/empty/error states
- [x] `/knowledge-base/templates` — Templates grid (icon, name, description); Use → create page from template → navigate to new page; Delete gated on kb:templates:manage; skeleton/empty/error states
- [x] `/knowledge-base/analytics` — KB analytics: StatCardGrid (total pages, views, searches, helpful votes, search success rate) + no-result searches table; client useCan("kb:analytics:view") gate with access-denied EmptyState; skeleton loading
...
- [x] `/knowledge-base/spaces` — Spaces list with card grid, create/edit Sheet, delete AlertDialog; manage gated by kb:spaces:manage
- [x] `/knowledge-base/spaces/[spaceId]` — Space detail with info card + honest EmptyState for page list (spaceId not in tree nodes)
- [x] `/knowledge-base/private` — Private pages view filtered from page tree (visibility==="private") via filterTreeWithAncestors
- [x] `/knowledge-base/shared` — Shared-with-me view: pages where createdById !== current user, shown as rows with status badge
