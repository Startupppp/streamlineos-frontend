...
> **PermissionGuard deny-by-default closure (2026-07-18)**: Closed the systemic RBAC fail-open follow-up from the 2026-07-03 CRM program. `PermissionGuard` now rejects guarded handlers missing `@RequirePermission`, explicitly exempts established `@Public` routes, and keeps owner/platform-admin bypass behind explicit permission metadata. Audited every backend controller: zero guarded non-public routes lack metadata and zero `@RequirePermission` routes lack guard wiring. Mixed controllers now apply `PermissionGuard` only to catalog-permission routes so intentional JWT-only self-service, dashboard, billing catalog, organization switch, RBAC catalog/template, and webhook reads preserve existing access. Focused unit and e2e regression suites cover missing metadata, allowed/denied permission, owner bypass ordering, and public route handling.

> **Free-seat pricing consistency (2026-07-18)**: Public pricing, landing copy, legal terms, and structured metadata now match the backend-enforced Free-plan limit of 5 members; added a cross-repo regression test.
