...
- [x] `/projects/command-center` — PM Command Center (cross-project dashboard: honest StatCard aggregates from `useProjects`/`useMyWork`, My Work + Projects sections, all 5 states)
...
- [x] `/projects/portal` — Client-facing portal: project cards grid (name, key, status badge, dates); gated `projects:portal:view`
...
- [x] `/projects/approvals` — Cross-project Approvals Inbox: my pending approvals across all projects, StatCards (pending/overdue), inline Decide (approve/reject/request-changes) gated `projects:approvals:decide`
...
- [x] `/projects/portfolios` (+`/[portfolioId]`) & `/projects/programs` (+`/[programId]`) — Portfolio/Program governance: workspace-level, health/status, M2M project links
